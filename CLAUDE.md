...
- **Dialog form validity** — always name the form validity ref `isEditFormValid`. Disable Save & Close via `:confirm-button-attrs="{ disabled: !isEditFormValid }"` (combined with other conditions as needed). Never use try/catch in submit handlers — prevent invalid submission through form validation rules so state is always consistent. Use `StyledEditFormDialogErrorIcon` with `:edit-form-ref :is-edit-form-valid` (plus optional `:schema :value` for Zod schema errors) in the `#prepend-actions` slot to surface field-level and schema validation errors. `formError` is **not** a store concern — stores do not watch `editedItem` to compute errors; schema validation is handled inside `StyledEditFormDialogErrorIcon` via `watchDeep`.
- **Vue macro ordering** — `defineSlots` → `defineModel` → `defineProps` → `defineEmits` (in this order).
