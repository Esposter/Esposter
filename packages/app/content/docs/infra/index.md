...
- **Event Grid dead-letter** — `deadletter` container + `deadLetterDestination` on all eight application subscriptions, tightened retry (10 attempts / 1h), 30-day lifecycle expiry, and a storage system topic that push-triggers the `ReplayDeadLetterEvent` function with a two-attempt cap and a quarantine prefix. → [Event Grid dead-letter](/docs/infra/eventgrid-dead-letter)
