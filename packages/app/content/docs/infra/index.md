...
- **Event Grid dead-letter** — `deadletter` container + `deadLetterDestination` on all six subscriptions, tightened retry (10 attempts / 1h), 30-day lifecycle expiry, and a `replayDeadLetterEvents` script. → [Event Grid dead-letter](/docs/infra/eventgrid-dead-letter)
