...
| Observability    | Log Analytics 30-day retention, App Insights 90-day, both environments; `dailyQuotaGb: 0.5` daily ingestion cap with a cap-reached alert; adaptive sampling in the Functions' `host.json`; public ingestion/query | A telemetry burst could outrun the `$0.01` budget guard before the Logic App reacts, so the cap is deliberately conservative rather than baseline-derived — see /docs/infra/observability-caps; no private access path exists yet                               |
