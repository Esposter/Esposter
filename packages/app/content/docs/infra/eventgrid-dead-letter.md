...
- `sqr…002` matches the `ReplayDeadLetterEvent quarantined` prefix. Quarantine is the only outcome that logs through `context.error` with that prefix, so nothing else can trip that alert. The quarantine copy and its log deliberately precede the fatal republish — a poison payload must be out of the resend batch before anything is resent — so a republish failure re-runs the whole prefix on redelivery. `writeDeadLetterBlob` therefore reports whether _this_ delivery created the copy, and the log is gated on that: one poison payload pages exactly once, however many times its batch is redelivered.
