---
title: File Table Editor
description: The grid editor for CSV/JSON/XLSX File resources — inline editing, data cleaning, computed columns, statistics, and Excel-style clipboard.
---

# File Table Editor

The file table editor is the grid editor of the **File resource** — a spreadsheet-like editor for CSV, JSON, and XLSX files with inline editing, data cleaning, computed columns, statistics, and an Excel-style clipboard. It is surfaced as the Data blade of a File resource in the Resource Explorer (`/resources/[id]/[[blade]]`).
...
- **Computed columns** — read-only columns whose value is derived lazily from other columns through a transformation (math expressions, string operations, type conversion, date parts, regex extraction, dataset aggregations). See [computed columns](/docs/file-table-editor/computed-columns).
