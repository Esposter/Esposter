---
title: Clicker
description: The Cookie-Clicker-style idle game — buildings, upgrades, a data-driven effect engine, and a single save blob per user.
---
...
- **One save blob per user** — the whole game state is a single `Clicker` entity persisted through the generic blob-state procedures (auth) or localStorage (unauth). Games deliberately stay off the resource layer ([games integration](/docs/platform/rejected/games-integration)). See [game loop and saves](/docs/clicker/game-loop-and-saves).
