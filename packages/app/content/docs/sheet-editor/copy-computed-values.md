...
A cell range indexes the **displayed** columns, while `computeValue` resolves a computed column's source by id against the columns it is handed and answers `null` for a source it cannot find — so the two sets are not interchangeable. `filterDataSourceRange` derives both from the one `DataSource`: the displayed columns select which cells the range covers, and the full column set stays the compute context. Hiding the source column of a computed column therefore changes nothing about what copy produces, matching the grid, which has always passed every column.
