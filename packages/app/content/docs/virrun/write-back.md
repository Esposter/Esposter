...
So the persist call takes `maskedPaths` — prepare outputs everywhere, and on win32 the whole mirror exclude set (a superset of them) — matched by the same `isExcludedPath` the mirror walk uses, so the two directions of the boundary cannot drift. `createVirrun` hands the outputs it already resolved to `resolveMirrorExcludes` rather than letting it re-read the config, so an `environment` passed programmatically cannot leave the mask narrower than the prepare layer it built; both shapes are root-anchored, since an output naming one directory (`.nuxt`) would otherwise mask that name at every depth. This is not a name-based smart filter: the rule is still structural (_did the sandbox receive this path from the host?_), and the excluded set is derived, never enumerated per command.
...
| `services/exec/util/isExcludedPath.ts`                  | pure: the one exclude matcher, shared with the source-mirror walk so the boundary's two directions agree            |
