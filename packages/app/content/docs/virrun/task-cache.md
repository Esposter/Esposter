...
The key is `sha256(lockfile-hash + working-tree-hash + command)` (`computeTaskCacheKey`); the working-tree hash is git-based (`git ls-files -s` + `git diff --binary` + untracked walk, `computeSourceTreeHash`). Two runs share a key iff all three match, so a hit is safe to replay verbatim. When the key can't be computed (not a git repo, no lockfile), the run falls back to a plain uncached persist rather than keying on partial state.
...
The key is only honest if the command is deterministic in its three inputs — and the network is not one of them. Two guards keep it so, neither needing a second "replay" run to compare:

- **Read-network → hermetic execution.** A cacheable command's fork runs with the network namespace **unshared** (`isNetworkEnabled: false` — the sandbox is `--unshare-all` by default and virrun simply stops re-adding network for the cached run; deps are already provisioned, so `createSnapshot`/`createPrepareLayer` keep network). This doesn't _detect_ network use; it _removes_ network as a hidden input, so the key becomes complete by construction. A pure task (`tsgo`/`eslint`/`vitest`) is unaffected; a read-network command (`pnpm outdated`/`audit`) can't reach the registry, exits non-zero, and is never recorded. virrun makes that failure legible: `isNetworkFailure` matches the network-error signature (undici's `fetch failed`, `getaddrinfo`/`ENETUNREACH`, …) in the failed run's streams and prints a "did you mean `--no-cache`" hint (`formatVirrunNetworkHint`). A command _known_ to need the registry simply runs native — no prefix.
...
| File                              | Role                                                                        |
| --------------------------------- | --------------------------------------------------------------------------- |
| `computeTaskCacheKey.ts`          | sha256 over lockfile hash + source-tree hash + command; null → run uncached |
