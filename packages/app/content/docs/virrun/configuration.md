...
- **Resolution** — walk up from cwd to the first `virrun.config.*`, delegated to unconfig's `loadConfigSync`: nearest directory wins, and within a directory the candidate order is `ts > mts > js > mjs > json`. Absent file = the backend defaults to `auto` (native today). No config is a valid, fully-functional state.
- **Every field is optional** in the file: `parseVirrunConfiguration` defaults `backend → auto` and leaves `environment → undefined` (no preset), so a minimal `{}` — or no file at all — is valid. Unknown keys throw (a typo fails loud, not silent), and the resolved `VirrunConfiguration` stays fully populated — optionality is the input surface, not the internal contract.
