...
The "Rust speed" win is real but lives one layer up: swap the slow JS tools for their native rewrites. The repo already does this — oxlint, rolldown, oxfmt (Rust), tsgo (Go) — and virrun's job is to run those isolated + warm, not to be fast itself. Revisit only if a profile of a real run shows a CPU-bound hotspot in virrun's own code that is a material fraction of wall-clock — realistically only a parallel directory walk for a huge in-memory source tree, which the `os` backend hot path never touches.
