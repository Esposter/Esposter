...
```text
packages/app             → apps/web         @esposter/app → @esposter/web
packages/azure-functions → apps/functions   @esposter/azure-functions → @esposter/functions
packages/infra           → apps/infra       @esposter/infra (unchanged)
...
The last row is a test, not a courtesy: `scripts/ledgerScopes.test.ts` resolves every sweep scope against the working tree, precisely because a pathspec that matches nothing reports a swept tree instead of failing. A ledger scope left pointing at `packages/app` fails that suite on the move commit, which is the behaviour to want.
...
3. **The prose.** The hand-written trees that name `packages/app`: the docs, the skills, the ledger bodies, the READMEs. Split by tree to stay inside a review window, and reviewed for the sentences that need rewording rather than renaming — a page saying "the app package" is fine, a page saying "under `packages/app`" is a path.
...
| `packages/app/content/docs/index.test.ts` | Checks that every Key Files path in the docs exists                         |
