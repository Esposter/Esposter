...
| Cluster              | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PDF rendering        | Three packages serve one component — a thumbnail embed, a full viewer, and the renderer both are built on, which we ship regardless. One renderer copy is installed, because an `overrides` entry forces the full viewer onto a major above the one it declares; the embed already tracks ours. The renderer is a permanent keep, the embed is what the dialog should render through too, and retiring the full viewer retires the override with it |
...
2. **Consolidate PDF onto the renderer.** Removes the adapter that declares an older renderer major, the question of which of the two answers what, and the override holding it above what it declares. The scope has to be stated before it starts: a page canvas with navigation and zoom is bounded, and in-document search is where it stops being bounded. Specified in [PDF viewer consolidation](/docs/proposals/refactors/pdf-viewer-consolidation).
