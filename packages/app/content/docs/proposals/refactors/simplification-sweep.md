...
- **A shared `key` across two `useMutation()` instances does not queue.** Queues live on the instance, so `blockUser`/`unblockUser`, `SendButton`/`useCancelScheduledMessageJob` and the seven instances in `useResource` all key on the same target while running concurrently — the opposite of what the key reads as. The one-instance-per-mutation convention and per-target queueing are in direct tension; the fix is either a shared instance per target or an explicit statement that writes merge the fields they own instead of queueing. The rollback audit made every site safe under concurrency, which is what turns this from a bug into a design decision.
- **`callSession`'s media toggles hand-roll their own async.** `setCameraEnabled` and `setMuteEnabled` apply optimistically and unwind with `getResultAsync` instead of `useMutation`, so they have no key, no per-target queue, and they re-throw. Same shape as the rule that a store never orders its own async work.
...
- **Optimistic counters beside server totals.** `useDeleteResources` nudges `count` by the rows it removed while `count` is the server's total, and `useVotePoll`'s `isVoting` is the executor's global `isPending`, so every poll on screen shows as voting while any one is in flight. Both are derivable state kept by hand.
