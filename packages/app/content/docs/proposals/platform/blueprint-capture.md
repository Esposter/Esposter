...
**Today** (once the Blueprint resource ships): authoring a manifest means writing entry content by hand — accurate but tedious, and exactly backwards from how people work. You configure a live dashboard against a live sheet until it's right; _then_ you want it templated.

**This proposal adds** one owner procedure plus a Save-as-blueprint command. No new storage, no new services — the output is an ordinary Blueprint resource.

## How it works

```mermaid
flowchart LR
  SEL["explorer multi-select or<br/>resource-page command"] -->|"captureBlueprint { ids, name }"| READ["read each resource's<br/>content blob (owner-gated)"]
...
- **References to unselected resources stay as raw ids** — the captured blueprint still points at the live resource (deploys will share it). The capture result dialog lists these outbound ids so the user can extend the selection and re-capture if sharing wasn't intended. Dangling-on-deploy is the same fail-soft posture every bare-id link already has.
- **Entry keys** derive from resource names (kebab-cased, deduped); **entry names** keep the original resource names. Parameters start empty — parameterizing ("this survey title should be `{{parameter:client}}`") is a manual edit in the Editor blade afterwards. Auto-parameterization is deliberately out: guessing which strings are variable is AI territory, not manifest mechanics.
- **UX**: multi-select in the explorer list → **Save as blueprint** command (alongside the existing bulk commands); also on a single resource's command bar, capturing just that one (a one-entry blueprint is a personal starter template).
...
| File                                                     | Role                                          |
| -------------------------------------------------------- | --------------------------------------------- |
| `server/trpc/routers/blueprint.ts`                       | `captureBlueprint` joins the blueprint router |
| `server/services/blueprint/rewriteIdsToAliases.ts` (new) | the deep-walk exact-match rewrite             |
| explorer list bulk commands + resource command bar       | Save-as-blueprint entry points                |
