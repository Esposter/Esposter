...
- **Delete** — `deleteSurveyResponse(partitionKey, rowKey)`, owner-gated via `getOwnerProcedure(ResourceType.Survey, …)`, deleting the Azure Table entity. Destructive confirm follows [resource page parity](/docs/platform/resource-page-parity) guard patterns.
- **Count** — `countSurveyResponses(id)`, owner-gated, surfaced on the Survey Overview summary slot ("N responses" linking to the Responses blade). Azure Table has no cheap server-side count, so this counts keys-only pages up to the existing `AZURE_MAX_PAGE_SIZE` cap and renders `1000+` beyond it — consistent with the dataset row cap ([warning proposal](/docs/proposals/platform/dataset-row-cap-warning)).
