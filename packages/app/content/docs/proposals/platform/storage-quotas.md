...
4. **Release** (abandoned reservation). If a reservation's `expiresAt` (the SAS TTL) passes with no `BlobCreated`, a sweep decrements the counter by `declaredBytes` — the rollback half. Deletion of a stored blob decrements by its actual size through the existing `publishBlobDeletion` path.

Steps 3 and 4 race, and the release must not be the step that decides the outcome: Event Grid delivery is at-least-once and unordered, so a `BlobCreated` for an upload that landed just before its TTL can arrive after the sweep has already given the bytes back. Applying `actual − declared` to a counter the release has emptied undercounts by the whole declared amount, and an orphan sweep later decrementing the same blob undercounts again. So the release **settles the row rather than dropping it** — `releasedAt` stamped, `declaredBytes` retained — and each transition is written as a conditional update keyed on the state it expects (`WHERE confirmedAt IS NULL`, `WHERE reconciledAt IS NULL`), which makes a redelivered event a no-op instead of a second decrement. A `BlobCreated` finding a released row re-adds `actual` rather than the difference, since the declared bytes are no longer in the counter. The row is dropped only once both signals are in, or by a retention sweep well past the point either can still arrive.
