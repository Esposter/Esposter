...
- The canonical motivating manifest: **survey funnel wave** — audience Sheet + Identified Survey + tokened Email + Program + funnel Dashboard, one `client` parameter in names and email subject. Deploy per client; each wave lands fully wired (`surveyFunnel.integration.test.ts` gains a blueprint-deployed variant once this ships).
