...
**Today**: every multi-resource setup is manual. The [survey funnel](/docs/proposals/platform/program-resource) — audience Sheet + Survey + invite Email + Program + results Dashboard — is five creates, five content edits, and four cross-resource bindings, repeated per wave or per client. Duplicate copies one resource; nothing copies a wired _set_.
...
- **Deliberately no engine.** No conditions, loops, expression functions, or nested-deploy — every rejected complexity of the full ARM/Logic-Apps shape. If a manifest needs logic, the answer is a second blueprint, not a language. The extension seam is the manifest schema (typed parameters, entry-count growth), same posture as the [Program](/docs/proposals/platform/program-resource) content schema.
...
- The canonical motivating manifest: **survey funnel wave** — audience Sheet + Invited Survey + tokened Email + Program + funnel Dashboard, one `client` parameter in names and email subject. Deploy per client; each wave lands fully wired ([funnel tests](/docs/proposals/platform/funnel-integration-tests) gain a blueprint-deployed variant once both ship).
