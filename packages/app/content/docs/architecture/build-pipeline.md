...
The barrel is generated, never committed: `ctix` runs against `tsconfig.build.json` as the build starts, which is why CI caches the generated `src/**/index.ts` files alongside `dist`, and the fingerprint below beside them so the skip fires there too. It is skipped when the source file list, each file's export bit and the generator are all unchanged — a list the two generated barrels are named out of exactly, because every other `index.ts` under `src` is a hand-written directory barrel the root barrel carries a line for, and excluding those by shape would hide the one addition that has to regenerate. A barrel names no symbol, so it cannot depend on _what_ a file exports — but ctix omits a file exporting nothing, so it does depend on whether each one exports at all, which is the single transition a path list would report as no change while the barrel silently lost a line. And it depends entirely on what wrote it, which is why ctix's cli, its `.ctirc-*` configs and the tsconfigs those point `project` at are all hashed by content alongside the list — a preset's `include` and `exclude` decide which files ctix is shown, so widening one changes the barrel without moving a single path or export bit. `export:gen` is still a script per package, and running it regenerates unconditionally.
