...
Shard count therefore trades feedback latency against duplicated work, and the duplicate is the part that is easy to stop counting: per-shard setup is roughly a third of a shard's runtime and every shard pays it in full, so the marginal shard adds a whole setup to the run's total to shorten a chain that already ends before the app build does. Coverage runs 8 shards. Bench is unsharded — it is a smoke test that every `*.bench.ts` still executes rather than a walltime measurement, so splitting it only multiplies setup. Changing the coverage count means changing `developMainStatusChecks`' `coverageShardCount` in the same commit: each shard publishes its own `Coverage (n)` required check, and a stale count either requires a context that never reports (blocking every PR) or silently stops requiring a live one.

### What a CI proposal has to beat

**Correctness first, then total consumption.** A check exists to be believed, so a proposal that cannot be shown to fail when the tree is broken is rejected whatever it saves — and the failure worth fearing is the quiet one, a job reporting green over work it did not do. Everything that clears that bar is then judged on the **total** it burns: runner minutes, CPU and memory across every job in the run, not the moment the slowest one happens to finish. Wall-clock is a poor proxy for it and the two often disagree — work that finishes inside another job's shadow still costs everything it consumed, and a saving invisible on the critical path is a real saving. That is why duplicated setup across matrix shards counts, why a job that installs what it never reads counts, and why the gates below hang off the disk rather than off a cache's own verdict: the cheapest run is the one that does not repeat work it already has, and the only run worth having is one whose green means something.
