...
What a miss costs is narrowed by a second entry under the same content hash — `barrel-generation-<hash>`, reached by a `restore-keys: barrel-generation-` prefix so a miss on the exact key still finds the previous build's copy. It exists because barrel generation is the expensive half of a cold build: bundling a package is quick, generating its barrel is a TypeScript program, and the same tree builds an order of magnitude faster once every fingerprint is current. [Generation is skipped](/docs/architecture/build-pipeline) per package when `node_modules/.cache/ctix-source-fingerprint` still matches, but nothing restored that file, so the skip fired only on a developer's machine while every CI miss regenerated all of them, including for the packages the commit never touched.

**The split is what makes the prefix safe, and the reason the `package-builds` entry keeps an exact key.** That entry's `packages/*/src/**/index.ts` glob also catches the hand-written directory barrel a package may keep under `src` — a tracked file, covered by the key, so an exact hit proves the restored copy is the checked-out copy. Restored by prefix it would be another commit's version of that file, extracted over the checkout and built, green. The second entry therefore holds only paths no commit authors: the generated root barrel, the generated component barrel of the one package whose extra ctix pass writes one, and the fingerprints. Staleness decides nothing there either — `cache-hit` is true only on an exact match, so a prefix hit still builds every package, and a restored barrel survives only where the fingerprint's source list, export bits and generator are all identical. Losing those files costs a cold build and never a wrong barrel. A prefix hit can also carry a package deleted since, as a directory holding nothing but generated output; a manifest is what declares a package, so its absence prunes the directory before the build. None of this reaches 🏗️ Pulumi or the two Functions deploys, which restore the exact `package-builds` entry only: they skip their build when a `dist` is on disk, so a prefix hit would hand them another commit's output to deploy.
