...
Nuxt's own `experimental.buildCache` is the same non-starter one layer down, and the resemblance is the argument against it. It tars `.nuxt` under a hash of `srcDir/**` plus config, and on a hit skips the Vue bundler outright — which is the marker's shape, not an incremental one, so it misses on exactly the commits the marker misses on. The sliver it would add is a commit confined to `server/**` or `public/**`, which its hash excludes and the marker's does not. That sliver is bought with the wrong currency: the marker's key is a content hash over every tracked file the build could read, so a key that is wrong costs a rebuild, while a tar restored against Nuxt's hash of config objects ships a bundle that does not match the source. There is nothing underneath it either — `optimizeDeps` is the dev server's, and a production Rolldown build keeps no incremental state — so the bundler layer has no cache to enable at all.

Caching `*.tsbuildinfo` across runs is retired by the same rule, and it is worth naming because it measures so well. Restoring the packages' incremental state takes their typecheck from most of two minutes to seconds, and the app's roughly halves — but the typecheck job already finishes with a third of the app build still ahead of it, so the whole saving lands inside another job's shadow and the workflow ends at the same moment. It fails on a second count too, which is why no version of it is worth reaching for. Only a prefix restore gives that saving — an exact-key hit means the tree already typechecked green, which a marker answers without a compiler — and a prefix hit is the one cache here whose correctness would rest on a compiler's own dependency graph rather than on a hash of its inputs. TypeScript does revalidate each file by content hash, so a stale entry reports a newly introduced error rather than hiding it; what is unproven is that every _indirect_ invalidation is caught, on a native compiler far younger than the format. The failure that would buy is a silent green on the gate that exists to be believed, and the repository's caches are built the other way round: a hit means this exact tree already passed.
