...
The key is deliberately whole-set rather than per-package: any real input change rebuilds all of them. Per-package keying — what Turborepo and Nx derive automatically from the workspace graph — is [rejected](/docs/architecture/rejected/monorepo-task-runners), because the whole set builds in seconds and a third content-hash cache beside virrun's and this one buys a fraction of a single job.
