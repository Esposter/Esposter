...
Shard count is chosen so the `coverage` → `merge coverage` chain lands inside the app build, and it is a boundary rather than a preference: halve the shards and each one's slice roughly doubles, which puts the chain past the app build and makes coverage the job everything else waits on. Coverage runs 8 shards. What a shard repeats is its setup, about a quarter of its runtime, and the larger half of that is provisioning the toolchain rather than the install — `pnpm i` and the artifact download together are the cheap part, because the pnpm store cache absorbs one and the single `build-packages` job already paid for the other. Bench is unsharded — it is a smoke test that every `*.bench.ts` still executes rather than a walltime measurement, so splitting it only multiplies setup. Changing the coverage count means changing `developMainStatusChecks`' `coverageShardCount` in the same commit: each shard publishes its own `Coverage (n)` required check, and a stale count either requires a context that never reports (blocking every PR) or silently stops requiring a live one.

### What a CI proposal has to beat

**Correctness first, then total consumption.** A check exists to be believed, so a proposal that cannot be shown to fail when the tree is broken is rejected whatever it saves — and the failure worth fearing is the quiet one, a job reporting green over work it did not do. Everything that clears that bar is then judged on the **total** it burns: runner minutes, CPU and memory across every job in the run, not the moment the slowest one happens to finish. Wall-clock is a poor proxy for it and the two often disagree — work that finishes inside another job's shadow still costs everything it consumed, so a saving invisible on the critical path is a real saving. What that counts against is **repeated** work, not distributed work: fanning distinct work across runners consumes no more than running it in one place and returns the feedback sooner, so a matrix is never the waste. Rebuilding what a cache already holds is, and so is a job installing what it never reads. Wall-clock re-enters only as a constraint — a job may not be shortened into becoming the one every other job waits on — which is what sizes the coverage matrix above. Hence the gates below hanging off the disk rather than off a cache's own verdict: the cheapest run is the one that does not repeat work it already has, and the only run worth having is one whose green means something.
...
Nuxt's own `experimental.buildCache` is the same non-starter one layer down, and the resemblance is the argument against it. It tars `.nuxt` under a hash of `srcDir/**` plus config, and on a hit skips the Vue bundler outright — which is the marker's shape, not an incremental one, so it misses on exactly the commits the marker misses on. The sliver it would add is a commit confined to `server/**` or `public/**`, which its hash excludes and the marker's does not. That sliver is bought against the first rule: the marker's key is a content hash over every tracked file the build could read, so a key that is wrong costs a rebuild, while a tar restored against Nuxt's hash of config objects ships a bundle that does not match the source and says nothing about it. There is nothing underneath it either — `optimizeDeps` is the dev server's, and a production Rolldown build keeps no incremental state — so the bundler layer has no cache to enable at all.

Caching `*.tsbuildinfo` across runs is the one that gets past the second rule and dies on the first, which is why it is named here rather than quietly not done. The saving is real and large in exactly the currency that counts — restoring the packages' incremental state takes their typecheck from most of two minutes to seconds, and the app's roughly halves — so the fact that it all lands inside the app build's shadow is beside the point. It is rejected because it cannot be shown correct. Only a **prefix** restore saves anything: an exact-key hit means this tree already typechecked green, which is a marker's answer and needs no compiler to give it. A prefix hit is then the one cache here whose correctness rests on a compiler's own dependency graph rather than on a hash of its inputs — every other gate can be checked by hashing what went in and looking at what came out, and this one cannot. TypeScript does version each file by content, so a stale entry still reports an error introduced in a file someone edited; what no amount of sampling establishes is that every _indirect_ invalidation is caught, and the compiler here is a native implementation far younger than the format it writes. The ecosystem does not lean on it either — the monorepo caches people do trust, this repository's own included, key inputs to outputs and never restore a compiler's internal state. Getting it wrong buys a silent green on a gate, which is the failure the first rule exists to refuse.
...
The `build-packages` job builds the packages once and uploads `packages/*/dist` plus the generated `packages/*/src/**/index.ts` barrels as a single `package-builds` artifact (both share the `packages/` common ancestor, so consumers download into `packages`; neither path is a dotfile, so no `include-hidden-files`). The build itself is gated by an `actions/cache` whose key is a content hash — mode, blob hash and path — computed by the `get-build-cache-keys` composite action so the save and every restore derive it from one definition (🚀 Pulumi and both Functions deploys restore this same entry). It hashes every tracked file under `packages/` and **subtracts** three things. Both keys drop `*.test.ts` / `*.test-d.ts` / `*.bench.ts`, which no entrypoint reaches and which change far more often than anything they sit beside, and the committed bench reports beside them — `*.bench.json`, `*.bench.md` and their platform-suffixed variants — which a `pnpm bench` run rewrites in `packages/app` as readily as in `virrun`, so hashing them spent a whole app build on a number no build reads. This key alone drops `packages/app`, which no consumer of this cache builds — the app key above is the same hash without that subtraction — together with every `.md`: markdown is a build input for the app, which renders `content/docs` through @nuxt/content, and for no package, so a README edit and the per-package CHANGELOGs `lerna publish` rewrites no longer discard the whole set. The root manifest, lockfile, catalog and `virrun.config.ts` are added as blobs — infra bundles the first via `import packageJson from "../../../../package.json"`, and the last decides which backend `pnpm build:app` runs under, which the script line naming `virrun` does not.
