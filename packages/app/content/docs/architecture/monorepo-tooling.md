...
The `build-packages` job builds the packages once and uploads `packages/*/dist` plus the generated `packages/*/src/**/index.ts` barrels as a single `package-builds` artifact (both share the `packages/` common ancestor, so consumers download into `packages`; neither path is a dotfile, so no `include-hidden-files`). The build itself is gated by an `actions/cache` whose key is a content hash — mode, blob hash and path — over an **allowlist** of the tracked files that actually feed `pnpm build:packages`: per non-app package its `package.json`, `tsdown.config.ts`, `auto-imports.d.ts`, `tsconfig*.json`, `src/**` and `types/**` (minus `*.test.ts` / `*.test-d.ts` / `*.bench.ts`), plus `configuration/.ctirc-*` and the root manifest, lockfile and catalog blobs. It reads `git ls-tree`, so only tracked files are hashed and generated output (`dist`, barrels, `*.tsbuildinfo`, `node_modules`) is excluded by construction. An allowlist rather than a whole-tree hash because every non-app package builds with the same fixed stack, which makes the inputs a small stable set worth enumerating: a README, CHANGELOG, lint-config, deploy-config or test-only commit hits the cache instead of rebuilding, as do app-only commits (the app is excluded). The cost is a standing obligation — **a build input of a new shape, such as a new root-level config a package's build reads, has to be added to the allowlist**, or a real input change reuses a stale build with nothing to say so. On a cache hit `pnpm build:packages` is skipped and the restored output is uploaded as-is; on a miss the job builds and seeds the cache. The downstream `setup-packages` composite installs dependencies and downloads the artifact — no build, no per-job cache.
