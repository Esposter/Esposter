...
What a miss costs is narrowed by a `restore-keys: package-builds-` prefix instead, which restores the newest previous entry — and the entry carries `packages/*/node_modules/.cache/ctix-source-fingerprint` alongside `dist` and the barrels for exactly that reason. Barrel generation is a TypeScript program per package and dominates a cold build, and [it is skipped](/docs/architecture/build-pipeline) when a package's fingerprint still matches; nothing restored that file, so the skip fired only on a developer's machine while every CI miss regenerated all of them, including for the packages the commit never touched. The prefix does not decide anything: `cache-hit` is true only on an exact match, so a prefix hit still builds every package, over a `dist` tsdown cleans first and barrels the fingerprint re-derives from the source list, each file's export bit and the generator. Losing the fingerprints costs a cold build and never a wrong barrel, which is why the prefix stays out of 🏗️ Pulumi and the two Functions deploys — those skip their build when a `dist` is on disk, so a prefix hit would hand them another commit's output to deploy.
