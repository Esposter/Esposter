...
| `ResourceAssets` · `{id}/files/…`                         | unique per upload  | resource asset upload, `restoreSnapshotVersion`'s clone of a published snapshot's assets | resource `deleteFile` (one asset), `purgeResource` (whole `{id}/` directory)                                                                                                                                      |
