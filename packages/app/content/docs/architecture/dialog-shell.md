...
Every dialog that asks for something renders through one shell, `StyledDialog`. It owns the whole frame — the `v-dialog`, the `StyledCard` inside it, the full-screen toggle in the card's append slot, the padded scrollable body, and the actions row — so a consumer supplies only three things: what the header says (`cardProps`), what the confirm button says (`confirmButtonProps`), and bare body content in the default slot. Nothing else in the app hand-rolls a `v-dialog` + confirm-button pair.
