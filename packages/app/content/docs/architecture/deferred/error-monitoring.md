...
- `app/plugins/errorHandler.ts` hooks Nuxt's `vue:error` and writes the error to the console with its `info` string. That is the whole global client handler — nothing is transmitted anywhere.
