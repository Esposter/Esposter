...
description: Deferred — inspecting an uploaded file's bytes server-side instead of trusting the content type its write SAS was signed with.
---
...
- What the bytes could do is bounded by what serves them. A blob is read through a signed url from a container that serves it as the content type its SAS was signed with, nothing on the origin executes it, and every surface that renders one renders it as an `<img>` or a media element. The realistic exposure is a decoder bug in the reader's own browser.
...
Keep the two properties that already bound it: a write SAS is minted for exactly one derived blob name with the size and content type already checked, and a blob is only ever reachable through a short-lived read SAS. Neither costs a request hop, and both hold for every upload path without anybody remembering.
