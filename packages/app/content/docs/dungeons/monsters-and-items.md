...
**Monsters** — `MonstersDataMap` (5 species) defines each species' sprite asset, attack kit, base statistics (`level`, `maxHealth`, `attack`, `defense`, `baseExperience`) and initial status (`health`, `experience`). Each species plays a distinct role: Aquavalor is the tank (low attack, high defense and HP), Carnodusk is fast and fragile (highest attack, lowest defense and HP), Frostsaber is the rare prize encounter (strong on every axis, highest `baseExperience`), Ignivolt is a glass cannon, and Iguanignite is the balanced starter. `new Monster(key)` deep-clones the species data and assigns a UUID, so every caught monster is an independent instance whose stats then grow by [level-ups](/docs/dungeons/battle). The player's party lives in `save.player.monsters`.
