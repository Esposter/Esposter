...
- **Publish** — `transformPublishedBlobUrls` → `cloneContentAssets`: referenced working-copy asset blobs are cloned under `{id}/published/{publishId}/files/…` and the content rewritten to the clones' stable urls, so a published snapshot survives the owner replacing or deleting the working-copy asset. Already-published urls in the content are immutable snapshot references and are carried as-is — re-cloning one under the publish directory would nest an unparseable `published/…/published/…` path. **`publishId` is minted per publish attempt, not the `publishVersion`** — the clone runs before the transaction that claims the version, so a version-keyed directory could only be predicted, and two concurrent publishes predict the same one ([publishing](/docs/architecture/publishing)).
