...
- **URL grammar** — a blob url is matched from the container base url up to the first `"`, `\` or `?`, then an optional SAS query bounded to its own charset (`[\w%&=+:.,~-]`). Blob names carry the uploaded filename verbatim, so `'`, `(`, `)` and spaces are all legal path characters; quotes and backslashes are not, which is what lets one regex match urls embedded in raw HTML/CSS and in JSON-serialized content alike. Splitting the query charset off the path is what stops a match inside `url('…?sig=…')` swallowing the closing `')` and the rest of the stylesheet. A string that does not match is left untouched, never rewritten and never an error.
