...
- **Duplicate** — `duplicateResource` runs the same `cloneContentAssets` against `{newId}` with published-reference cloning on: **every** referenced asset — working-copy and published alike — is cloned under the copy with its source-relative path preserved (`{newId}/files/…`, `{newId}/published/{n}/files/…`), so the copy is fully self-contained: its editor can delete files, and deleting or unpublishing the original never strands it. A cloned published-path asset serves owner-only until the copy itself publishes (the authorization matrix requires a publication row for anonymous reads — the copy starts as Draft). The clone drops the _source_ resource id from the destination path, which is also why the rewrite is a per-url map, never a prefix replace — content can legitimately carry foreign-id urls (duplicated or blueprint-deployed content).
