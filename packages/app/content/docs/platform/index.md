...
- [Storage quotas](/docs/platform/storage-quotas) — per-user blob allowance reserved atomically at SAS issuance, settled against the stored objects by a timer sweep
...
- **Storage quotas** — a per-user blob allowance (Free = 10 GiB) reserved by a compare-and-swap at SAS issuance, ledgered per blob, and settled against the stored objects by a 15-minute timer sweep; deletion and purge decrement through the same ledger, and `/user/settings` grows a Gmail-style usage bar. One Postgres migration, one timer function, no new Azure services.
