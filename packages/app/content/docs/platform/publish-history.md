...
- **A restore is a content write like any other**, so it goes through the shared `saveResourceContent` service ([/docs/architecture/resources](/docs/architecture/resources)): an editor open in another tab adopts the restored content and its new `contentVersion` off the subscription, instead of holding the pre-restore draft and having every further save in that tab rejected as stale until a hard reload; the type's after-save hook re-derives what the restored content declares; and the [activity](/docs/platform/activity-log) trail records a `Restored` entry, as the recycle bin's own restore does.
