...
Two persistence mechanisms (blob + localStorage) double every save/load path in `useResource` for a niche flow. The consolidation keeps exactly one mechanism; `/resource-explorer` is auth-gated.
