...
The cap counts **holds that declare bytes**, which is one per upload. A thumbnail rider declares zero and takes no slot: the client downscales it and its upload is best-effort, so a thumbnail that is never generated would otherwise hold a slot until its SAS expires an hour later, while the upload it belongs to succeeded — a handful of undecodable images would lock a user out of uploading anything. The consequence is that a user can have up to twice the cap in ledger rows in flight, each rider minted only alongside a file that did take a slot, so nothing unbounded rides in on it. The same number bounds each upload request's `files` array, because a batch larger than the cap can never pass the reserve however long the client waits.
...
| `packages/infra/.../eventSubscriptions/prodEvgsEsposterAe007.ts`                | the subscription, filtered to resource assets     |
