...
**Collection is a different, later bound, and the difference is load-bearing.** A hold that stopped counting is not a hold that may be deleted: a `BlobCreated` for a blob that _did_ land can still be inside Event Grid's retry window, and a reconcile that finds no ledger row charges nothing and reports nothing — the bytes would be stored, attributed to nobody, with no dead letter to show for it. So a row is kept until no event naming it can still arrive: the last PUT a write SAS authorizes happens at `expiresAt`, and Event Grid retries that PUT's event for `EVENT_GRID_DELIVERY_TTL_MS` after it, so the reserve collects only rows whose `expiresAt` is older than that window. The bound is derived from the subscription's own `retryPolicy` rather than restated beside it, because two hard-coded hours that drift apart fail silently in exactly this direction. The rows are dropped opportunistically by the next reserve that user makes, which already holds their rows under lock.
