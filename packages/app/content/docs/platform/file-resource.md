...
Each imported file (CSV/JSON/XLSX) is its own resource. The old table editor fused three concerns on one entity (`ADataSourceItem`: identity, parse settings, and the data itself, all inside one multi-item `TableEditorConfiguration` blob); the File resource keeps them separate:
...
- **Data** — the entire grid editor: inline editing, computed columns, statistics, clipboard, find/replace, undo/redo. Components live under `Resource/File/*`; the grid's feature set is documented in the [file table editor](/docs/file-table-editor) area.
...
| File                                              | Role                                                |
| ------------------------------------------------- | --------------------------------------------------- |
| `app/shared/models/resource/file/FileResource.ts` | content blob schema (`data` + `settings`)           |
| `app/components/Resource/File/Data.vue`           | Data blade (grid editor)                            |
| `app/components/Resource/File/Settings.vue`       | Settings blade (parse configuration form)           |
| `app/store/resource/file/`                        | grid state + command/undo stack over `content.data` |
...
- The store's command/undo stack (`ADataSourceCommand`, `fileHistory`) operates on a single `DataSource` — exactly one File resource's `content.data`; it initializes from `useResource(id)` content.
- Existing table-editor data was discarded, not migrated (consistent with the no-production-data stance). The multi-item models (`TableEditorConfiguration`, `ADataSourceItem`, `vuetifyComponent/` — see [decision](/docs/platform/decisions/vuetify-component-resource)) were deleted with the fold.
