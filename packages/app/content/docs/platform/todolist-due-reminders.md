...
- **Delivery** — the reminder pushes `『{item}』 is due` to the owner's push subscriptions through the same `sendWebPushNotifications` path every other notification uses. Clicking the notification opens the resource's Items blade (`/resource-explorer/{id}/items`). TodoLists are single-owner resources, so the recipient set is just the owner's subscriptions — no fan-out.
