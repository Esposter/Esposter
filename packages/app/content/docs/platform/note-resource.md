...
- **Extensibility cost.** Note landed as a live test of the one-`ResourceType`-plus-one-`ResourceDefinitionMap`-entry extensibility claim, and the claim is understated. Beyond the enum value, the definition entry, the editor component, and the view component, a new type also needs, one per file: a per-type tRPC router (`note.ts`) plus its registration in the root router, an arm in the client `useResourceMutations` dispatch switch, and a `[ResourceType.Note]: []` entry in the exhaustive `ResourceBladeDefinitionMap`. Making the type creatable from the explorer adds three more hand-maintained lists: `CreatableResourceTypes`, the `ResourceCreateProcedureMap` in `useCreateResource`, and `ResourceTypeDescriptionMap`. None of these are the definition map — they are the router-per-type topology and the switch-based client dispatch, the honest friction the test was meant to surface. A fully map-driven registration (deriving routers and client dispatch from `ResourceDefinitionMap`) would collapse them back toward the claimed two.
