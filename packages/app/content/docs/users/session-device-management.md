...
- **The browser and device**, from `getDeviceLabel`, which parses the stored `userAgent` with `bowser` on the server: a browser and its major version (`Chrome 141`), then the most specific device the string still knows. Apple is the last vendor publishing a model, so everything else lands on the OS name — Chrome froze the Android model to `K`. No OS version is shown, because none can be trusted: Windows 11 still sends `NT 10.0`, and Safari and Chrome both freeze macOS at `10_15_7`. A string bowser recognises nothing in reads as `Unknown device` rather than being echoed back raw.
