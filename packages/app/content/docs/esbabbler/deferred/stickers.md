...
- A sticker is not a glyph. It has no reaction path, no `:` autocomplete, and no inline render inside text, so it shares almost nothing with [custom emoji](/docs/proposals/esbabbler/custom-emoji) beyond the upload — it is a new message type, with its own render component, its own per-item weight in the message list, and its own picker surface.
...
**Revisit when:** custom emoji has shipped and rooms want a bigger, standalone image than an inline glyph — at which point the sticker set reuses the emoji table's shape, cap and permission, and the only new work is the message type.
