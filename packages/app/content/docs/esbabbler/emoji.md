...
- **Categories are data, not the enum.** `getEmojiCategories` pins Frequently Used ahead of the nine CLDR groups when it has anything in it. That is also the seam [custom emoji](/docs/proposals/esbabbler/custom-emoji) appends its room category to.
