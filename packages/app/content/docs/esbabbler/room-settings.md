...
**Roles** (previously a combined "Permissions" panel) edits roles and their permission bitfields; **Members** (previously a tab inside it) assigns/revokes member roles; **Invites** manages your invite link — the same manager component (`MessageModelRoomInviteManager`) the header's Add Friends dialog uses; **Attachments** edits the room's upload limits, described in [/docs/esbabbler/file-media](/docs/esbabbler/file-media).
