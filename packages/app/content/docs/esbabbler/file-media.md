...
The renderer records **which thumbnail urls failed, as a set** — not a single "the current one failed" flag. The two say the same thing only while a url is stable, and these are not: the hourly sweep re-mints every read SAS, so a flag reset on each new url makes the renderer re-attempt a thumbnail it already knows is missing on every refresh, which is what the set replaced. The cost is that entries accumulate for as long as the message list stays mounted — one dead string per failed thumbnail per refresh cycle — and nothing prunes them. Accepted, and deliberately not reverted to a single ref: that swap has now been made in both directions across reviews, and the set is the direction that keeps the renderer's behaviour correct.
