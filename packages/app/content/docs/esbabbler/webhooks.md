...
The url a member copies points at the **app**, not at Azure: `server/api/webhooks/[id]/[token].post.ts` validates the two route parameters against the schema shapes, spends a point from the per-webhook budget described in [rate limiting](/docs/architecture/rate-limiting), and forwards the body to the Function App with the function key. Keying the budget on the webhook id rather than on a caller is the point — a webhook is a machine identity, and one misconfigured integration should exhaust only its own allowance.
