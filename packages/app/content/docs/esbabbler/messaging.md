...
- **An ascending page stops at an index row whose entity has not landed.** `createMessage` writes `MessagesAscending` first (so a rejection always means nothing is readable, which is what every caller's rollback assumes), and the two tables cannot be written atomically — so the index can name a message the join does not find. The cursor comes from the index, so stepping over that hole would advance past a message whose entity lands a moment later and drop it from the rest of that scroll. The page therefore truncates there and resumes from the same spot. Only holes older than `MESSAGE_INDEX_WRITE_GRACE_NANOSECONDS` are skipped — past that window the row can only be an orphan of a failed write, and waiting on one would stall the scroll forever.
