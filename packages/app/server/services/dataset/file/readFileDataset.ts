...
export const readFileDataset: DatasetProvider = async (ctx, reference) => {
  ...
  const { data } = fileResourceSchema.parse(jsonDateParse(await streamToText(readableStreamBody)));
