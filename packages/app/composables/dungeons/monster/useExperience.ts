...
export const useExperience = (monster: Ref<Monster>) => {
  const totalExperienceToCurrentLevel = computed(() => calculateTotalExperienceToLevel(monster.value.status.level));
  const totalExperienceToNextLevel = computed(() => calculateTotalExperienceToLevel(monster.value.status.level + 1));
