...
describe(useCursorPaginationCache, () => {
  ...
  const cursorPaginationData = ref(
    new CursorPaginationData<IndexedDbDatabaseSchema[IndexedDbStoreName.Messages]["value"]>(),
  );
  const initializeCursorPaginationData = (
    data: CursorPaginationData<IndexedDbDatabaseSchema[IndexedDbStoreName.Messages]["value"]>,
  ) => {
  ...
  const mountCache = async (initialKey: string = partitionKey) => {
    ...
    wrapper = await mountSuspended(
      defineComponent({
        render: () => h("div"),
        setup: () => {
          ({ flush } = useCursorPaginationCache({
  ...
  beforeEach(() => {
    items.value = [];
    cursorPaginationData.value = new CursorPaginationData();
    goOffline();
  });
