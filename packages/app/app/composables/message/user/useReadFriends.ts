...
export const useReadFriends = () => {
  ...
  const { friendRequests, sentFriendRequests } = storeToRefs(friendRequestStore);
  const { friends } = storeToRefs(friendStore);

  const readFriends = async () => {
    const [readBlockedUsers, readFriends, readFriendRequests, readSentFriendRequests] = await Promise.all([
      $trpc.block.readBlockedUsers.query(),
      $trpc.friend.readFriends.query(),
      $trpc.friendRequest.readFriendRequests.query(),
      $trpc.friendRequest.readSentFriendRequests.query(),
    ]);
    blockedUsers.value = readBlockedUsers;
    friends.value = readFriends;
    friendRequests.value = readFriendRequests;
    sentFriendRequests.value = readSentFriendRequests;
  };
