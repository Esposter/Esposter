...
export class CsvParser implements IParser {
  async parse(file: File): Promise<DataSource> {
    ...
    const sourceFieldNames = parseCsvLine(takeOne(lines));
    ...
    const rows = rawRows.map((rawRow) =>
      Object.fromEntries(columns.map((col, index) => [col.fieldName, coerceValue(rawRow[index] ?? "", col.type)])),
    );
