...
import { ArrowKeyDeltaMap } from "@/services/resource/file/ArrowKeyDeltaMap";
...
<template>
  <v-card flat>
    <template #text>
      <ResourceFileRowTextSlot :data-source />
    </template>
    <VueDraggable v-model="dragRows" target="tbody" :disabled="!isDraggable" :handle="`.${DRAG_HANDLE_CLASS}`">
      ...
      >
        ...
        >
          <ResourceFileRowHeaderSlot :column :get-sort-icon :header-column :is-sorted :toggle-sort />
        </template>
        <template v-for="column of displayColumns" :key="column.id" #[`item.${toColumnKey(column.name)}`]="{ item }">
          <ResourceFileRowItemSlot
            :column
