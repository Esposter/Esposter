...
import { BOOLEAN_VALUES, DATE_FORMATS } from "@/services/tableEditor/file/constants";
...
export const inferColumnType = (values: string[]): ColumnType => {
  ...
  else if (trimmedValues.every((v) => DATE_FORMATS.some((format) => dayjs(v, format, true).isValid())))
