...
export const writeCachedMessages = async (roomId: string, messages: MessageEntity[]) => {
  try {
    ...
    const index = store.index("partitionKey");
    ...
    // Write the most recent messages (up to limit), excluding optimistic messages
    ...
    // Best-effort cache — fall back to network-only behavior on error
  }
};
