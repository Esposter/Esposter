...
<template>
  <NuxtLayout hide-global-scrollbar>
    ...
    <div flex flex-col h-full p-6 overflow-y-auto>
      <h1 text-h5 font-bold mb-6>Friends</h1>

      <section mb-8>
        <h2 text-h6 mb-3>Add Friend</h2>
        <div flex gap-2>
          <v-text-field
            v-model="searchQuery"
      ...
      <section v-if="pendingRequests.length > 0" mb-8>
        <h2 text-h6 mb-3>Pending Requests — {{ pendingRequests.length }}</h2>
        <v-list rounded>
      ...
      <section>
        <h2 text-h6 mb-3>Friends — {{ friends.length }}</h2>
        ...
        <p v-else text-medium-emphasis>No friends yet. Search for users above to add them.</p>
      </section>
    </div>
  </NuxtLayout>
</template>
