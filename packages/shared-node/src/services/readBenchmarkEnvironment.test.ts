...
vi.mock(import("node:os"), async (importOriginal) => ({ ...(await importOriginal()), cpus: vi.fn<typeof cpus>() }));

describe(readBenchmarkEnvironment, () => {
  test("trims the width padding a platform reports its cpu model with", () => {
    expect.hasAssertions();

    const core: CpuInfo = { model: "a ", speed: 0, times: { idle: 0, irq: 0, nice: 0, sys: 0, user: 0 } };
