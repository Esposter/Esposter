...
The plugin settles a character for the day at the first session start — by lore through a typed decision when a TypeSafe key is configured, by the nearest birthday otherwise — and every session that day meets them; this skill is how the user reads and overrides that. Every verb runs the plugin's script and relays its output — the roster is game data the script reads, and nothing about it is known without running it.
...
| `today`          | The card of the day's pick, or of the pinned character. Settles the day's pick if no session has yet, so what it prints is what the sessions get. Reports a pin that names nobody in the roster; that pin is ignored.                                                                                       |
| `pin <name>`     | Fixes the character for every session until unpinned. The name is matched whole, ignoring case.                                                                                                                                                                                                             |
| `unpin`          | Removes the pin; the day's pick stands again.                                                                                                                                                                                                                                                               |
