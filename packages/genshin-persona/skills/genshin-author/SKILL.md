...
Only what a person reads is a language's to write. The habits and the sign-off reach the model alone and are never translated — a model reading them in English answers in whatever language it was asked to, with the register intact. The greeting is the card's one line the person reads, printed by the hook as the welcome before the model has spoken, which is why it is the language's; a greeting in the module is the card's line as that character would say it in that language, in their own register, never the English line pasted through — the module's test refuses that. Once a module has one it is the only greeting that shows: the welcome and the card the model reads both carry it, because a verb printing a card writes one stream the person and the model share.
...
**The greeting is the one line a card performs rather than describes.** The session-start hook shows it to the person as the welcome, and it is the model's example of a spoken line — how the character says hello, which no reply repeats, since the welcome already has — so it is one or two short sentences in Latin script, spoken words and nothing else, since a stage direction or a mark of emphasis in the example is written into lines the engine then reads aloud, and it is written as the character would say hello — a fresh line in our words that echoes how their own hello line moves (who they name themselves as, what they ask first), never that line reworded closely enough to be recognised.
