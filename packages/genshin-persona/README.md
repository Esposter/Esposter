...
The Stop hook hands each reply's prose to a resident synthesizer — one process per machine that holds the loaded engine, woken by the session-start hook so the first reply is warm, and gone again after half an hour idle — which reads it in units — the first sentence alone, then a few sentences at a time — playing each while the next is synthesized. `mute` and `unmute` decide whether the hook asks it at all, and so does a `volume` of zero, which is silence the engine should not be woken for. `volume <number>` is a whole number from 0 to 100, applied as a gain, from the next reply on:
