...
| `src/services/baseSpinnerContent.ts`   | The base Teyvat verbs every spinner shows before the character's own, and the tips a character with no tip or line falls back to.                                                                       |
...
The spinner replaces the built-in verbs and tips with the session's character's: the base Teyvat verbs in `baseSpinnerContent.ts` with the character's own behind them, and under the character's name their card's tips followed by every line of theirs, off the game data or the community wiki — the tool shows one label over every tip, so the base tips, nobody's line, appear only for a character with neither, under the tool's own "Tip". The session-start hook rewrites the two settings from a detached process whenever the character changes, and so do `use`, `pin` and `unpin`; Claude Code reads the spinner keys once per process, so a rewrite shows from the next session. A status line that is not the plugin's is left alone.
