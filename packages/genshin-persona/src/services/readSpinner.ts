...
// The spinner for one character, off their card and every line of theirs — the game data, else the wiki, so
...
export const readSpinner = async (name: string, personaCard: PersonaCard | undefined): Promise<Spinner> => {
  ...
  return getSpinner(BASE_SPINNER_CONTENT, name, personaCard, lines);
};
