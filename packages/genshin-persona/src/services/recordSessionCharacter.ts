...
// The session's record is what every later reader trusts — the start hook on a clear, compact or resume, the status
// Line, the speech hook — so whoever changes a session's character writes it here
export const recordSessionCharacter = ({ element, name }: Nameplate, sessionId: string, todayIsoDate: string): void => {
  ...
  writePickRecords([...otherRecords, { element, isoDate: todayIsoDate, name, sessionId }]);
};
