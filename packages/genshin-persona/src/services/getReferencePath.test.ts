...
describe(getReferencePath, () => {
  ...
  test("caches the clip under its dub by its own stem", () => {
    ...
    expect(getReferencePath("stem", VoiceLanguage.English)).toBe(join(languageDirectory, "stem.ogg"));
  });
  ...
  test.each(["../a", String.raw`..\a`])("spends the stem %s on one name inside the dub", (stem) => {
    expect.hasAssertions();

    expect(getReferencePath(stem, VoiceLanguage.English)).toBe(join(languageDirectory, "..-a.ogg"));
  });
});
