...
const nameKey: keyof BlockEntity["properties"] = "https://blockprotocol.org/@blockprotocol/types/property-type/name/";
...
export class BlockElement extends BlockElementBase<BlockEntity> {
  ...
  private async handleInput(event: InputEvent) {
     ...
     */
    try {
      await this.graphModule.updateEntity<BlockEntity["properties"]>({
        data: {
      ...
      console.error(`Error updating self: ${e}`);
    }
  }
  ...
  render() {
    return html`<h1>Hello, ${this.getBlockEntity().properties[nameKey]}</h1>
      <p>
        The entityId of this block is ${this.getBlockEntity().metadata.recordId.entityId}. Use it to update its data
        when calling updateEntity.
      </p>
      <!-- @see https://lit.dev/docs/components/events -->
      <input @change=${this.handleInput} value=${this.getBlockEntity().properties[nameKey]} />`;
  }
}
