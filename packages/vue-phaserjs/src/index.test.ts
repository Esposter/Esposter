...
describe("vue-phaserjs", () => {
  ...
  test.skipIf(process.platform !== "win32")("bundle size (Windows)", () => {
    ...
    expect(getFileSize(distFile)).toMatchInlineSnapshot(`"index.js: 32.38 KB (33156 bytes)"`);
  });

  test.skipIf(process.platform === "win32")("bundle size (POSIX)", () => {
    expect.hasAssertions();
    expect(getFileSize(distFile)).toMatchInlineSnapshot(`"index.js: 32.38 KB (33158 bytes)"`);
  });
