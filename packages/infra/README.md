...
- `src/index.ts` is the generated Pulumi program entrypoint. Run `pnpm export:gen` before invoking Pulumi directly.
- `Pulumi.yaml` defines the Pulumi project.
- `Pulumi.dev.yaml` and `Pulumi.prod.yaml` hold stack-specific configuration.
- `src/resources/` contains Pulumi resource declarations. Each tracked file owns exactly one Azure resource.
...
- package scripts run `pnpm export:gen` before Pulumi stack operations;
...
```bash
pnpm build             # generate exports and type check the Pulumi project
