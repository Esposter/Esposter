...
- **A Linux `node` inside your default WSL2 distro** (Windows hosts only) — Windows `node.exe` can't run in the Linux sandbox, so without it the sandbox still mounts but node-based commands (`pnpm`, `vitest`, …) can't resolve `node` inside it. virrun reads it from your real WSL login shell, so a version manager (fnm/nvm) counts. If that login capture is unavailable it probes the guest's default-`PATH` node (`wsl.exe --exec node --version`) instead — it never falls back to your Windows node, because the sandbox cannot run it and a snapshot labelled with the wrong node major is a wrong cache hit, not a stale one. The version it resolved is printed in the run banner — that, not your Windows `node -v`, is what the sandbox runs. Both captures are cached for a few hours, so a fresh `fnm use` shows up on the next capture (or immediately after `virrun cache clean --all`).
