...
// A recoverable runtime condition. Rebuilt so the editor-only `$schema` pointer never leaks into the runtime config;
// The `environment` key is only carried when a preset is set, so no `undefined` value is ever synthesized (absence is
// The missing key).
