...
const { crossSpawn } = vi.hoisted(() => ({ crossSpawn: vi.fn<typeof baseSpawn>() }));
...
describe(spawnBackground, () => {
  ...
  beforeEach(() => {
    crossSpawn.mockReturnValue(child as unknown as ChildProcess);
  });
  ...
  test("spawns a hidden, stdio-ignored child and never detaches", () => {
    ...
    expect(crossSpawn).toHaveBeenCalledExactlyOnceWith("", [], { stdio: "ignore", windowsHide: true });
  });
