...
const UNSUPPORTED_HOST_MESSAGE = "requires Linux/WSL + bubblewrap";
...
export const createOsBackend = (environment?: Environment): ExecBackend => {
  if (!checkIsOsBackendSupported())
    throw new InvalidOperationError(Operation.Create, createOsBackend.name, UNSUPPORTED_HOST_MESSAGE);
