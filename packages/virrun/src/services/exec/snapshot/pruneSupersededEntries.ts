...
// The shared eviction behind pruneStaleSnapshots / pruneStalePrepareLayers: only `directory/<currentName>` is reused by
...
export const pruneSupersededEntries = (directory: string, currentName: string): void => {
  sweepStaleEntries(directory, (name) => name !== currentName && !checkHasLiveLease(join(directory, name)));
};
