...
// Upper is removed so it is never read back as a usable snapshot. The capture run's result is returned
// Alongside the location: it is a real execution, so the cold-path fork returns it directly rather than
// Re-running the command over the frozen upper.
export const createSnapshot = (
  ...
  return getResultAsync(async () => {
    ...
    const result = await backend.exec(command, { ...options, overlayLayers: { upperDir, workDir } });
