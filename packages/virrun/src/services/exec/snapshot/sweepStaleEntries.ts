...
// Absent or concurrently-removed directory sweeps nothing instead of throwing. The callers differ only in what they
// Select — a superseded hash/key (`name !== current`), a hard-killed run's mkdtemp corpse (a temp prefix), a mirror
// Whose repo is gone — so the readdir + guarded teardown lives here once instead of in each.
