...
export const removeSnapshotDirectoriesDetached = (directories: readonly string[]): void => {
  ...
  for (const directory of directories)
    if (WSL_UNC_REGEX.test(directory)) linuxDirectories.push(readWslPath(directory));
    else
      getResult(() => {
        removeSnapshotDirectory(directory);
      }).match(noop, ({ message }) => {
        writeVirrunDebug(`stale cache directory ${directory} not removed — ${message}`);
      });
  if (linuxDirectories.length === 0) return;
  ...
  getResult(() => {
    ...
    writeVirrunDebug(`detached teardown of ${linuxDirectories.length} directories not staged — ${message}`);
  });
};
