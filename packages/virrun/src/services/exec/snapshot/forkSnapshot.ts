...
// `extraLowerDirectories` stack above the deps snapshot (e.g. the source-keyed prepare layer owning `.nuxt`), so their
...
export const forkSnapshot = (
  ...
  const { exists, upperDirectory } = resolveSnapshotLocation(options.cwd);
  if (!exists)
  ...
  return backend.exec(command, {
    ...options,
    overlayLayers: { lowerDirectories: [upperDirectory, ...extraLowerDirectories] },
  });
};
