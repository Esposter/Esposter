...
describe(removeSnapshotDirectory, () => {
  ...
  // The WSL-side teardown removeSnapshotDirectory shells out for a UNC snapshot directory: chmod traversable, then rm -rf.
  ...
  test(`tears down a ${TEST_WSL_UNC_PREFIX} UNC snapshot directory inside WSL via a chmod + rm -rf`, () => {
    expect.hasAssertions();
