...
// Take a live-user lease on a snapshot/prepare hash directory by writing `leases/<pid>`, so a concurrent run on a different
...
export const createLease = (hashDirectory: string): Lease => {
  ...
  reapDeadLeases(leasesDirectory);
  const leaseFile = join(leasesDirectory, String(process.pid));
