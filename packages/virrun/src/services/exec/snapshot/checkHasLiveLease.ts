...
// True when a live run still holds a lease on this snapshot/prepare hash directory. Reaps dead-pid leases as a side
// Effect (reapDeadLeases), so pruneStale* both spares a directory another run is reading and clears a hard-killed run's
// Stale lease.
