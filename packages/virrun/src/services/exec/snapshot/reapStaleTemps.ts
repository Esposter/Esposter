...
// *Superseded* hash directories, so a corpse in the *live* directory would accumulate forever. Reap it beside the prune
// — but read the owner pid back out of the name (parseTempOwnerPid) and reclaim only a *dead* owner's corpse, so a
// Concurrent run whose temp shares this hash directory (same lockfile) is never deleted mid-exec. The published
// Bare `upper`/`work` and the `leases/` sibling carry no owner pid and are always kept. (Not for the
// Shared `os.tmpdir()` source-clone root — that is concurrent with no per-entry owner, so it is left to the OS's tmp
// Reaping.)
