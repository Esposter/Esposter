...
// The sandbox never starts on that path, so no status block reaches stderr and the close handler would otherwise read
// The missing block as a sandbox-setup failure — createBwrapBackend keys on this marker to surface the sync failure as
