...
// `apps/web/.nuxt`) matches from the tree root — either way the pattern's whole subtree matches, since excluding
...
// Sibling (`.gitignore` vs `.git`, `apps/web-e2e` vs `apps/web`) is never a match.
