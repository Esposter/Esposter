...
// Underneath it), or a value older than PROBE_CACHE_MAX_AGE_MS (state the key cannot fingerprint drifted). The age
// Bound is applied here rather than passed in, so no probe cache can be read without it. Every undefined branch simply falls through to a fresh probe, so a bad cache is self-healing, never
