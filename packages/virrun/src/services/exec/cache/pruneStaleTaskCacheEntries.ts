...
// Days → milliseconds without a raw ms literal; dayjs is test-only here, kept out of the ~1 MB runtime bundle.
...
export const pruneStaleTaskCacheEntries = (tasksRoot: string): void => {
  const cutoffMs = Date.now() - TASK_CACHE_MAX_AGE_DAYS * MILLISECONDS_PER_DAY;
