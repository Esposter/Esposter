...
// Three-tier like readWslLoginEnvironment / isOsBackendSupported so a fresh `virrun -- <cmd>` process skips the two
...
export const getWslNativeCacheRoot = (): string => {
  ...
  if (process.env[VIRRUN_FORCE_PROBE_KEY] === undefined) {
    const cached = readWslEnvironmentCache(WSL_CACHE_ROOT_CACHE_FILENAME, z.string(), key);
    if (cached !== undefined) {
