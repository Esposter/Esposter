...
export const reapAbandonedSourceMirrors = (liveEntryName: string): void => {
  getResult(() => {
    const sourcesDirectory = join(getWslNativeCacheRoot(), VIRRUN_SOURCES_DIRECTORY_NAME);
    sweepStaleEntries(sourcesDirectory, (name) => {
      if (name === liveEntryName) return false;

      const entryPath = join(sourcesDirectory, name);
