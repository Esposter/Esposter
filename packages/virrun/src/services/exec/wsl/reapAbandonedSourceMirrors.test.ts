...
describe(reapAbandonedSourceMirrors, () => {
  ...
  test("reaps a mirror whose origin host directory no longer exists", () => {
  ...
  test("keeps a mirror whose origin host directory still exists", () => {
    expect.hasAssertions();
  ...
  // The marker is written the instant the entry directory exists, so an aged unmarked entry is a corpse of a sync that
