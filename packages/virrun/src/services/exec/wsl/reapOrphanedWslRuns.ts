...
// `cache clean`, which then removes the directories those trees hold open. It runs the reaper synchronously and has it
// Wait for the TERMed trees to exit (buildWslReapCommand's wait arm). A blocking reap that fails or times out keeps its
