...
// Probe whose subject can drift without changing the key passes. Stored where getWslEnvironmentCachePath says.
export const readWslEnvironmentCache = <TValue>(
  filename: string,
  valueSchema: z.ZodType<TValue>,
  key: string,
  maxAgeMs?: number,
): TValue | undefined => readKeyedCache(getWslEnvironmentCachePath(filename), valueSchema, key, maxAgeMs);
