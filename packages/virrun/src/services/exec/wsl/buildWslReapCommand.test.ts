...
describe(buildWslReapCommand, () => {
  ...
  // Rather than the markers, since the marker is on the shell TERM kills first while the bwrap holding the directories
  // Open is still unwinding. An expired deadline exits nonzero (`alive` pre-set so exit 0 cannot mean "never looked"),
