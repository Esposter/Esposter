...
// Blocking single-directory teardown (removeSnapshotDirectory); a sweep of many directories uses WSL_REMOVE_LIST_SCRIPT
// Instead, because an argv has a ceiling and a sweep's size has none.
...
// Directories are still stale, so the next run's sweep enumerates them again and stages a fresh list. Retaining the
// Failed one buys diagnostics nothing reads and leaks a file per failure. A removal
