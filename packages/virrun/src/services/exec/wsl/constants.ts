...
// A source mirror is a self-contained entry directory (`sources/<sha256(hostCwd)>/`) like `snapshots/<hash>`: the mirrored
...
// Pid-tagged temp leaves (`<prefix><pid>.<uuid>`) the planner stages into the entry directory for the folded sync script to
...
// Node, since the 9p bridge identity can't chmod/remove the overlay workDirectory's namespaced-root scratch.
...
// Missing directory is a no-op). Paths are passed as positional args and iterated, never interpolated into the body — a
// Cache path containing a single quote would otherwise break the quoting and inject extra shell syntax. Used by the
// Blocking single-directory teardown (removeSnapshotDirectory); a sweep of many directories uses WSL_REMOVE_LIST_SCRIPT instead,
// Because an argv has a ceiling and a sweep's size has none.
export const WSL_REMOVE_SCRIPT =
...
// ONE wsl.exe launch however many directories it holds: a launch is a service RPC plus a relay process, and the per-entry
...
// Directories are still stale, so the next run's sweep enumerates them again and stages a fresh list. Retaining the failed
