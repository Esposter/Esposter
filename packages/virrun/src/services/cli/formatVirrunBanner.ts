...
export const formatVirrunBanner = ({
  ...
  `${colorize(colorize("[virrun]", Color.Cyan), Color.Bold)} running "${colorize(command.join(" "), Color.Yellow)}" (backend=${colorize(backend, Color.Blue)}, node=${colorize(nodeVersion, Color.Green)})`;
