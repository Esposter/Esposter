...
export const formatVirrunCacheHit = (command: readonly string[] | string): string =>
  `${colorize(colorize("[virrun]", Color.Cyan), Color.Bold)} ${colorize("cache hit", Color.Green)}, replaying "${colorize(typeof command === "string" ? command : command.join(" "), Color.Yellow)}"`;
