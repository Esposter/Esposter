...
// Than spawning wsl.exe here, on the cold-boot-tolerant WSL probe tier: reporting `not found on PATH` for a
...
export const readSandboxProbeOutput = (file: string, args: readonly string[]): string | undefined =>
  readProbeOutput(() =>
    process.platform === "win32"
      ? execWsl(["--exec", file, ...args], { timeout: WSL_PROBE_TIMEOUT_MS })
