import type { VirrunResult } from "#src/models/cli/VirrunResult";
...
export const formatVirrunResult = ({ command, durationMs, exitCode }: VirrunResult): string => {
  const exitColor = exitCode === 0 ? Color.Green : Color.Red;
  return formatVirrunLine(
    `"${colorize(joinCommand(command), Color.Yellow)}" exited ${colorize(String(exitCode), exitColor)} in ${colorize(`${durationMs}ms`, Color.Green)}`,
  );
};
