...
// The dimmed one `formatVirrunDebug` uses. Every surface that reports a failure renders through this (the cache
// Commands, warm, a run whose backend threw, the doctor's unavailable verdict), so one failure reads identically
// Wherever it is raised instead of each site re-deciding that a caught error is red.
