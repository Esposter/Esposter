...
describe(formatVirrunNetworkHint, () => {
  // Color is a no-op under vitest, so the assertions read as plain text.
  test("names the command and points at the native / --no-cache escapes", () => {
    expect.hasAssertions();

    expect(formatVirrunNetworkHint("pnpm outdated -r")).toMatchInlineSnapshot(`
  ...
  test("joins an argv command for display", () => {
    ...
    expect(formatVirrunNetworkHint(["tsx", "scripts/checkDependencies/index.ts"])).toContain(
      `"tsx scripts/checkDependencies/index.ts" tried to use the network`,
    );
  });
});
