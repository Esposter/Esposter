...
export interface OverlayLayers {
  ...
  // Host directory that captures writes as real files (a capture run); must be paired with workDirectory. Omitted →
  // Writes go to an invisible tmpfs upper.
  upperDirectory?: string;
  // Empty scratch directory on the same filesystem as upperDirectory, required by overlayfs whenever upperDirectory is
  // Set.
