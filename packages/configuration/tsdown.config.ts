...
const tsdownConfiguration: UserConfig = mergeConfig(getTsdownConfigurationNode(), {
  ...
  // The `eslint/`, `types/` and `vitest/` trees are published surface no bundle produces, and generating an `exports` field
  // Is what encapsulates a package: without these, every `@esposter/configuration/eslint/*.js` import — the shared
  // Flat config each package symlinks — and the app's `types/global.d.ts` import resolve to nothing, and neither does the Vue hook every Vitest worker imports. Every project
