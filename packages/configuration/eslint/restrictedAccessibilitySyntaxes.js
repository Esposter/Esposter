...
// Altogether, so a proxy file input behind its labelled button is the one place the pair is honest. A bound `hidden`
// Counts only as the literal `true`, since Vue drops the attribute for a falsy value.
export default [
  {
    message:
    ...
    selector:
      "VStartTag:has(VAttribute:matches([key.name='aria-hidden'], [key.argument.name='aria-hidden'])):has(VAttribute:matches([key.name='tabindex'], [key.argument.name='tabindex'])):not(:has(VAttribute:matches([key.name='hidden'], [key.argument.name='hidden'][value.expression.value=true])))",
  },
];
