...
describe("@esposter/xml2js", () => {
  test("bundle size", () => {
    ...
    if (isWindows) expect(getFileSize(distFile)).toMatchInlineSnapshot(`"index.js: 1117.27 KB (1144087 bytes)"`);
  ...
  test("types size", () => {
    ...
    if (isWindows) expect(getFileSize(distDtsFile)).toMatchInlineSnapshot(`"index.d.ts: 35.81 KB (36669 bytes)"`);
