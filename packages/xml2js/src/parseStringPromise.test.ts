...
describe(parseStringPromise, () => {
  ...
  test("keeps every repeated child when explicitArray is off", async () => {
    ...
    const result = await parseStringPromise<{ a: { b: string[] } }>("<a><b>a</b><b>b</b><b>c</b></a>", {
      explicitArray: false,
    });

    expect(result.a.b).toStrictEqual(["a", "b", "c"]);
  });

  test("collapses whitespace runs in every text child when normalize is set", async () => {
    expect.hasAssertions();

    const result = await parseStringPromise<{ a: { $$: { $$: Record<string, string>[] }[] } }>(
      "<a><b>a  b</b><b>b  a</b></a>",
      { charsAsChildren: true, explicitChildren: true, normalize: true, preserveChildrenOrder: true },
    );

    expect(result.a.$$.map(({ $$ }) => $$)).toStrictEqual([
  ...
  test.each([
    ...
    const result = await parseStringPromise<{ a: { b: string[] } }>("<a><b> a  b </b></a>", options);

    expect(result.a.b).toStrictEqual([expected]);
  });

  test("applies a value processor to the normalized text, not the raw text", async () => {
    expect.hasAssertions();

    const result = await parseStringPromise<{ a: { b: string[] } }>("<a><b> a  b </b></a>", {
      normalize: true,
      valueProcessors: [(value) => value.toUpperCase()],
    });

    expect(result.a.b).toStrictEqual(["A B"]);
  });

  test("merges attributes onto the node when mergeAttrs is set", async () => {
    expect.hasAssertions();

    const result = await parseStringPromise('<a><b c="a"/></a>', { mergeAttrs: true });
  ...
  test("drops attributes when ignoreAttrs is set", async () => {
    ...
    const result = await parseStringPromise('<a><b c="a">a</b></a>', { ignoreAttrs: true });
  ...
  test("applies the attribute name and value processors", async () => {
    ...
    const result = await parseStringPromise('<a><b c="a"/></a>', {
  ...
  test("processes the value a qualified attribute wraps when xmlns is set", async () => {
    ...
    const result = await parseStringPromise('<a xmlns:c="a"><c:b c:d="a"/></a>', {
      attrValueProcessors: [(value) => value.toUpperCase()],
      xmlns: true,
    });
  ...
  test("drops a blank char key", async () => {
    ...
    const result = await parseStringPromise('<a><b c="a"> </b></a>');
