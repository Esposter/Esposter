...
describe("@esposter/azure-functions", () => {
  ...
  test.skipIf(process.platform !== "win32")("bundle size (Windows)", () => {
    ...
    expect(getFileSize(distFile)).toMatchInlineSnapshot(`"index.js: 4658.03 KB (4769826 bytes)"`);
  });

  test.skipIf(process.platform === "win32")("bundle size (POSIX)", () => {
    expect.hasAssertions();
    expect(getFileSize(distFile)).toMatchInlineSnapshot(`"index.js: 4664.34 KB (4776288 bytes)"`);
  });
});
