...
| Channels                   | An MCP server with the channel capability feeds a line from outside into the open session — the one way in that opens no second session, and what makes [channel chat](/docs/proposals/infra/channel-chat) possible. In research preview, a plugin registers only under a development flag that asks a confirmation at every launch. |
