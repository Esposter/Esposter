...
| Plugin settings file       | May set two keys, both about subagents. The status line and the spinner verbs are user settings, so the plugin's skill writes them on request and the persona page says how the path survives an update.                        |
