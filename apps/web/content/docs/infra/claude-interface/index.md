...
| MessageDisplay hook        | Receives each piece of a reply as it is displayed — cut at line breaks, the piece as "delta" and the reply's turn as "turn_id" — and the display waits on its exit, so a script that reads one line of a reply runs before the reply is complete, and does nothing that waits.                                                  |
