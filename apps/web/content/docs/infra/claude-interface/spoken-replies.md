...
1. **The spoken channel.** The output style confines the character to spoken lines: every reply opens with one blockquote line — one or two short sentences, in the first person, with no markup, code or number inside it — the first reply of a session with the card's greeting as written, and a reply may close with one; everything else is plain, and nothing else is written as a blockquote. What is spoken is a channel of the reply rather than a reading of it: short enough to synthesize whole and worth hearing, while the answer reads the way a neutral assistant's would.
...
3. **The resident synthesizer** — one process per machine, spawned detached by whichever sender finds no server listening — holds the loaded engine. It binds the socket before the engine loads, so a second sender finds the address taken rather than loading a second engine, and a request arriving during the load waits for it. Per request it fetches the character's reference clip on first use and encodes it once per process, then **reads the lines in order** — each synthesized whole, checked for speech, given the volume as a gain and handed to one player process spawned for the reading, while the line after it is already being generated. It keeps **one turn pending**: a reply's closing line queues behind its opening line, a reply that lands while another is being read drops what of the older one is still waiting, and tells the reading already under way, which stops at the line it has reached rather than finishing over its replacement. Idle for half an hour, it exits and frees the GPU.
4. **The SessionStart hook** spawns a detached warm request for the session's character, so the load and the first-synthesis cost are paid while the person reads the card and types, not on the first reply. What the warm synthesizes is the card's greeting — the line the output style opens the first reply with — and the synthesizer keeps that clip, so the first reply's opening line plays without a synthesis at all; a character with no card warms on one short word. The hook itself waits on nothing: its stdout is the model's context.
...
sequenceDiagram
    ...
    Start->>Sender: warm — this session's character and their greeting (detached, nothing waited)
    ...
    Server->>Server: encode the reference, synthesize the greeting, keep the clip
    ...
    loop each line of the turn, generated while the one before it plays
        Server->>Server: the kept clip, else synthesize the line
...
flowchart LR
    ...
    Greeting{"The card's greeting?"}
    Kept["The clip the warm kept — played at once"]
    Synthesized["Synthesized whole — one line's wait"]

    Reply --> Rule
    Rule -- yes --> Spoken --> Greeting
    Rule -- no --> Plain
    Greeting -- yes --> Kept
    Greeting -- no --> Synthesized
```
...
3. **The proof.** The verb warms the synthesizer with the current session's character — their greeting, the clip the first reply's opening line then plays from — and has it speak one sentence, so the person hears the voice they set up before the first reply does — and the sentence comes through the resident process, the same path every reply takes.
...
- **A synthesis throws** mid-request: the request is dropped, the reason logged, and the server keeps serving. A line the model rejects does not cost a reload for the next.
...
| `packages/genshin-persona/src/services/getWarmRequest.ts`           | What a warm synthesizes: the card's greeting, else one short word                                       |
| `packages/genshin-persona/scripts/voice.ts`                         | The resident synthesizer: the socket, the load, the turn queue, the kept greeting, the idle exit        |
...
| `packages/genshin-persona/src/services/createLatestWinsQueue.ts`    | One turn's requests in order, replaced whole by a newer turn's, and the running one told                |
...
| `packages/genshin-persona/output-styles/in-character.md`            | The spoken channel's one rule: a blockquote line is a spoken line, and nothing else is written as one   |
