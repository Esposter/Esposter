...
- **The player does not play** — not installed, refusing the file, or stopped by a signal, which stands in for the exit status such a player leaves behind: the reason is logged, and the request still answers `ok`, since the sentence was synthesized. A stock player that is silent leaves that one line where a hook's silence gives none.
