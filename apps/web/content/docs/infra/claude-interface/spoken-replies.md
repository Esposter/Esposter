...
Chatterbox Turbo through the ONNX runtime, a precision per component, each measured by the [reference selection](/docs/infra/claude-interface/reference-selection)'s likeness before it was chosen: the speech encoder in half precision and the language model in its 4-bit variant, which cost no likeness and speak faster; the vocoder in full precision, because its half-precision variant is the one that costs likeness — a tenth to a fifth of it — and it is also the one variant this machine's GPU vocodes into speech rather than silence, so the trade it offers is the top rung's speed for that likeness, declined.
...
The plugin ships code and cards. The engine's runtime is a few hundred megabytes of native binaries and the weights more than a gigabyte, and the plugin install runs a frozen `npm ci` under a one-minute ceiling — so none of it is a dependency of the plugin, and all of it is installed by the verb into the directory the plugin already owns for its state. Run with a dub, it first stops any synthesizer running and clears the rung on file, then does the following in order, each skipped when already done:
...
- **Stream inside a line.** A line is synthesized whole and vocoded once, so its first sample arrives after its last token. Vocoding a few dozen tokens at a time re-runs the reference's tokens through the vocoder on every pass, which on this machine costs seconds per pass, so the streaming stays between lines. What a line that sounds the moment it is written has to meet is the [seamless spoken replies](/docs/proposals/infra/seamless-spoken-replies) proposal.
