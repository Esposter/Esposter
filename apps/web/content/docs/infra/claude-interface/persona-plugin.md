...
flowchart LR
    ...
    Headline --> Context
    Note --> Context
    PersonaCard -- "habits and sign-off" --> Context
    Context --> Asked
    Asked -- yes --> Answer
    Asked -- no --> Silent
```
...
**Of the authored card, the greeting is translated and nothing else is, and the translated one is the only one that shows.** The habits and the sign-off are written for the model, and a model given them in English and asked to answer in Japanese answers in Japanese with the register intact, so they follow the reply language by themselves at no authoring cost — where translating them is the whole roster again in every language, for lines the model reads past. The greeting does not follow by itself: the session-start hook prints it to the terminal as the welcome's third line, before the model has said a word, so it is a label a person reads in the interface language beside a headline and a note already in it. The language's module carries it per character, and a character it has no line for yet is greeted in the card's own words — one line in another script is a queue item, where a spinner mixing two scripts would be a broken one.

The resolved greeting is one field, and both copies of the card carry it. A verb that prints a card — `today`, `use`, `pin`, `unpin`, `language` — writes one stream the model and the person share, with no second channel to show them different lines, so what a person reads there is the interface language's down to the greeting. The earlier shape, where the welcome showed the language's line and every card printed the card's own, is rejected: it greeted twice in two languages inside one session, and the language the card kept was the one nobody had asked for. The earlier rule before that, that the whole card stays English because the greeting is a line the model performs, is rejected too: it was true of the model's copy and false of the person's, and the person's is the one that showed.
