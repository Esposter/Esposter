...
flowchart LR
    ...
    Content["baseSpinnerContent + the card + the lines<br/>verbs in two layers, tips in one"]
    Script["spinner.ts, detached<br/>the card's tips, then every line"]
    ...
    Spinner["Spinner<br/>verbs, tips under the character's name or the tool's Tip"]
...
| `packages/genshin-persona/src/services/getSpinner.ts`              | Both verb layers; one tip layer under one label — the card's tips, then the lines, cut at the tool's caps   |
...
| `packages/genshin-persona/src/services/readSpinner.ts`             | The card and every line of the character's, read into one spinner                                           |
...
| `packages/genshin-persona/src/services/writeSessionSpinner.ts`     | The spinner for a character, where `setup` opted the settings in and the label is not theirs yet            |
| `packages/genshin-persona/src/services/baseSpinnerContent.ts`      | The base Teyvat verbs every spinner shows first, and the tips a character with no tip or line falls back to |
