...
**The translated greeting shows in the welcome and nowhere else, and the card keeps its own.** Carrying the resolved line into the card as well — on the argument that a verb prints one stream the person and the model share, so the person reads an English greeting there — was tried and rejected: the card is one block of the model's own English, three habits and a sign-off, and a localized line between them is the two scripts in one list that the spinner's rules already refuse, for a greeting the person has read once in the welcome already. The split is by reader and not by channel: what the model is given stays as authored whichever surface carries it, and the person's greeting is the welcome's line.
