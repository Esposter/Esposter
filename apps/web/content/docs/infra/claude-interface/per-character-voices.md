...
- The reference is trimmed to a few seconds — about what the engine's vendor calls enough — rather than the whole window the engine conditions on, and a minute-long story line is read from its opening; the measurement embeds the same opening seconds, so what it chose is what the engine hears. The trim was measured with the reference selection's own likeness on a few characters in one dub: a few seconds scores no lower than the whole window and a little higher for some, while a reference shorter still leaves the model no ending to anchor on and the carrier runs on for tens of seconds; and the vocoder runs over the reference's tokens on every unit spoken, so the shorter reference makes every unit cheaper. The map's likeness column reads what the whole window scored until the next roster measurement rewrites it.
