...
flowchart TD
  ...
  Y --> CK{Into another chunk?}
  CK -->|yes| WK[Worker: the missing chunks in range generated and meshed, the ones past it dropped]
  WK --> R
  Z -->|yes| O[Outlined, a label naming E and what it does]
  O -->|E, the use trigger, or a tap on the label| U[Its console tab, or the composer]
  Z --> R
...
- **The room is stamped into it.** The floor, the wall behind, the wall to the left and every object are voxel boxes (`RoomVoxelBoxes`) filled into whichever chunks they fall in. The left wall holds the door's opening, and the door stands swung open outward against the wall beside it, so the player walks out through it onto the ground. The two sides the camera looks in over stay open. Leaving to the app is the pause menu's.
- **It streams in chunks.** Space is split into Minecraft's columns of sixteen by sixteen voxels. A chunk within four of the player's own each way is generated and meshed in a Web Worker, which hands its voxels and its mesh back as transferable buffers, so neither a frame nor the composer waits on the terrain. A chunk is dropped once it is a chunk further out than that, so a player pacing a border never regenerates one, and the chunks are settled again only when the player crosses into another, nearest first. Fog in the clear colour thickens from well past the player to short of the nearest ungenerated ground, measured from wherever the camera's arm has it, so the world's edge fades rather than stops.
- **A chunk is one mesh.** `greedyMesh` turns a chunk's grid into triangles once. A face between two solid voxels is never emitted, and neighbouring faces of one colour and one ambient occlusion merge into one quad. Occlusion is counted from each corner's solid neighbours, and a face is shaded by the way it faces. Both are baked into the vertex colours, so the world has no lights and no shadow map. A chunk's grid carries a one-voxel border of its neighbours' voxels, generated the same way, which is read but not meshed, so a face on a chunk's edge is culled and shaded against what is really beside it.
...
- **Fewer pixels.** The canvas renders at half the device's pixel ratio and is scaled up without smoothing, which is a quarter of the fill cost and is what gives the voxels their pixel look. The world is a draw call per chunk in range, one per figure, six for the player and one per gauge.
- **The terrain costs its render distance, never its size.** A player who walks for an hour holds the same chunks as one who stands at the door, and a frame spent inside one chunk does no terrain work at all. The bench beside `generateChunk` holds that a chunk's time does not grow with how far out it is.
...
| `apps/web/app/components/AgentConsole/World/Chunks.vue`               | The chunks in range around the player, asked of the worker and dropped     |
...
- **Walls closing the room's open sides.** The camera looks into the room over its two open sides, and a wall there would pull the spring arm in to the player at every step inside, so those sides stay open and the door is the way out through the two walls the room has.
