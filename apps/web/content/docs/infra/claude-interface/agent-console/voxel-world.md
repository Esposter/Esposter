...
- **The person is a player figure.** Six boxes — a head, a body, two arms and two legs — in the proportions a voxel person is expected to have: in sixteenths of a voxel, the head eight on a side, the body eight by twelve by four and each limb four by twelve by four, two voxels tall like an agent. It is built from the palette's colours, not from any game's model or skin. Its arms and legs swing in opposite pairs, timed by the distance walked rather than the clock so the feet never slide, and it breathes while it stands. With reduced motion asked for, the limbs hang still. It walks in at the door on every load and is kept nowhere.
- **It walks as the camera sees it.** W, A, S and D or the arrows move it forward, left, back and right relative to the camera's heading, and it turns to face the way it walks. Diagonals are cut to the length of a straight step. On a touch screen a joystick in the lower corner walks it, and a gamepad's left stick does through the browser's Gamepad API. Walking reads the same gate as the world's keys: nothing open over the world, nothing editable focused, no modifier held.
- **Collision is one axis at a time.** The player is a box narrower than a voxel and as tall as the figure. Each step is taken along x and cut short at the first solid voxel or the floor's edge, then along z, so a step into a wall at an angle slides along it (`moveThroughGrid`). A step at walking speed is far under a voxel, so it cannot tunnel. The floor is flat, so there is no gravity and no jump, and the agents walk through the player, so where a person stands never holds the session up.
