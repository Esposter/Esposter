...
- **Event Grid dead-letter** — a `deadletter` container every application subscription dead-letters into under a tightened retry policy, a lifecycle expiry, and a storage system topic that push-triggers the replay with an attempt cap and a quarantine prefix. → [Event Grid dead-letter](/docs/infra/eventgrid-dead-letter)
