...
- [Claude interface](/docs/infra/claude-interface) — the terminal given a personality and a voice without building an interface: a Genshin persona plugin picked by the calendar, and replies spoken in each character's own cloned voice by an engine on this machine.
...
- **Claude interface** — the terminal got a personality and a voice for one workspace package and nothing in the estate: the free-tier Speech account that spoke the first two stages left when the third replaced its catalogue voice with a clone of each character's own, run by an engine the plugin installs into its state directory, with the companion window left behind a named gate rather than built early; nothing was added that a rewrite of the terminal would ever have to carry. → [Claude interface](/docs/infra/claude-interface)
