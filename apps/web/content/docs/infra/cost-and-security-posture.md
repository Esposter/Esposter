...
**The guard tears down the metered subscriptions, not every subscription.** `003`/`004` handle `evgs-…-001` (`ProcessWebhook`) and `-002` (`ProcessNotification`) — the two highest-volume application paths. The remaining application subscription (`-006` blob deletions) is deliberately left alone: it fires once per user action on a path measured in events per day, which at Event Grid's per-operation pricing is indistinguishable from zero. `003` and `004` read one per-stack target list (`DevEventSubscriptionGuardTargets` / `ProdEventSubscriptionGuardTargets`), and `004`'s PUT body is built from the same arguments the Pulumi subscription is declared with, so a subscription cannot be torn down by the one workflow without being recreated by the other, nor recreated differently from how Pulumi declares it. `004` reads each target before writing it and runs the PUT only after a read that failed or timed out — the two are distinct statuses in the workflow language, so naming only the first would skip the recreate after a slow read — and because the body is the declared state, a write after a throttled read re-asserts what is already there rather than diverging from it. The Logic App managed identities hold only the scopes their workflow needs — Website Contributor for start/stop, EventGrid Contributor on the custom topic for delete/recreate, and Storage Account Contributor on the environment's storage account for `004` alone.

Two of those are easy to get subtly wrong. **EventGrid Contributor, not EventGrid EventSubscription Contributor**: the narrower role grants the standalone `Microsoft.EventGrid/eventSubscriptions/*` extension type, while a subscription that lives under a topic is the separate `Microsoft.EventGrid/topics/eventSubscriptions` child type — under the narrower role every read of one returns 403. That one applies to `003` and `004` alike — both read the topic-scoped subscription. And **`004` needs the storage scope because a write authorises its linked scopes as well as its target**: the recreated subscription dead-letters to the storage account, so without write there it fails as `LinkedAuthorizationFailed` while holding every permission the topic itself asks for. `003` needs no storage scope of its own, because deleting a subscription validates no destination. The four assignments still carry Pulumi logical names reading `event-grid-event-subscription-contributor`, from before the role was widened: the logical name is the resource's identity, so correcting it would replace a `protect: true` grant rather than rename it, and the `roleDefinitionId` is what the assignment actually grants.
