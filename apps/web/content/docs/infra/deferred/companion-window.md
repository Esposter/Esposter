...
The shape that stays cheap is a **spectator**: the terminal keeps driving, and the companion window subscribes to the hooks — the Stop hook for what was said, the notification hook for when attention is needed — and reacts by speaking through the [character voice](/docs/infra/claude-interface/per-character-voices) and animating. It holds no session, sends no tokens, and breaks nothing when it is closed.
