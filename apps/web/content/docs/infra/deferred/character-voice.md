...
The game installs its Japanese voice track as Wwise packages under the client's streaming assets — tens of gigabytes, no decoded audio anywhere in it. Turning that into reference clips is **solved and costs two npm packages**, built for the [voice match benchmark](/docs/infra/claude-interface/voice-match-benchmark): the packages are unencrypted, the decoder is pure JavaScript, and every clip is found by the hash of the stem the game data records for its line, so the reference set is labelled by construction. Nothing extracted is ever committed — the rule above is what makes this stage defensible, and a cache breaks it exactly as a commit would.
...
- **Prosody is measurable now, and already has somewhere to go.** Median fundamental frequency and speaking rate are numbers a script reads off a waveform, and [per-character voices](/docs/infra/claude-interface/per-character-voices) already spends both as `pitch` and `rate` on a catalogue voice. Those numbers are the one part of this stage that pays off before the gates open, and the [voice match benchmark](/docs/infra/claude-interface/voice-match-benchmark) is how they are spent: choosing the closest catalogue voice needs the reference audio and no inference engine, so it waited on neither of this stage's gates and is built.
