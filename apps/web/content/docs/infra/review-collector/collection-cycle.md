...
- **Two writers of `ai/queue`, one compare-and-swap.** The lease each side pushes with is [two writers](/docs/infra/review-collector/two-writers)'; a rewrite the session's push beat exits the run idle, since that push fires a run of its own that replays onto what the queue then carries — porting off the stale head instead would hold on a conflict the next run resolves.
