...
- **Two writers of `ai/queue`, one compare-and-swap.** Each side pushes with [two writers](/docs/infra/review-collector/two-writers)' lease; a rewrite the session's push beat exits the run idle, since that push fires a run of its own that replays onto what the queue then carries — porting off the stale head instead would hold on a conflict the next run resolves.
