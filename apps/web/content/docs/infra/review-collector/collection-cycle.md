...
- **Two writers of `ai/queue`, one compare-and-swap.** Each side pushes with [two writers](/docs/infra/review-collector/two-writers)' lease; a rewrite the session's push beat is simply replayed by the next run onto what the queue then carries.
