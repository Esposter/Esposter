...
1. [The collection cycle](/docs/infra/review-collector/collection-cycle) — what the collector reads, the gates it clears, how it merges a release whose review is clean or judges one the bot rates riskier, rewrites `ai/queue` onto the tree the next window is built on and repackages a commit no window could carry, ports the largest prefix under the cap, folds `main` in, pushes, replies with the pushed sha, and opens the release pull request over whatever `develop` then carries unreviewed. One script, `ai:coderabbit:collect`.
...
- **Non-blocking.** A person is told, never waited on. Two cases remain theirs, both posted where they are read: a conflict or a reshaping that failed past its attempt cap, and a release the verdict held.
