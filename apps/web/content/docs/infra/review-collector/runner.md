...
**The one credential it does hold is the Claude Code token, and that is a trade rather than an oversight.** The token authenticates the `claude` process itself, and the process that holds it is the process that spawns every tool the session runs, so no authentication mode Claude Code offers keeps it from a tool subprocess — a helper command or a credentials file is read by the same user the tools run as. What it can spend is the account's own subscription, never anything in this repository: a session steered into exfiltrating it burns the owner's Claude quota until the token is rotated, and a session steered into writing it into a fix commit puts it on `review-fixes`, a branch the runner pushes and a person reads. Closing the channel means an egress allow-list around the runner, which is a heavier control than the blast radius earns while the drain reads one bot's prose about one maintainer's code; it is the first thing to add if the repository ever takes contributions the drain would read.
