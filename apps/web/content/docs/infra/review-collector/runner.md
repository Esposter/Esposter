...
| `REVIEW_COLLECTOR_TOKEN`  | the `gh` CLI login token of the account that runs the working session, refreshed with the `workflow` scope | the collector's own pushes to `develop` and `ai/review-fixes` and its thread replies — never the drain's, and the built-in `GITHUB_TOKEN` cannot do the job |
