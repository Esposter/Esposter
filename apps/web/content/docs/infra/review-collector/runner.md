...
**Those two are handed across by name, never `secrets: inherit`.** The callee is pinned to `ai/queue`, a mutable branch whose drain step runs a permission-skipping Claude session over one bot's prose, so inheritance would put the Azure and Pulumi deployment credentials within reach of any step added to that branch; the list makes the blast radius the two the collector reads. The cost is a sync the trigger cannot be pinned to pay — GitHub reads it from whichever ref the event names, so a secret the reusable workflow grows and the trigger has not mapped arrives empty on a `status` or an `issue_comment` alone — which is why the reusable workflow declares both `required` and `constants.test.ts` holds the two files to each other.
