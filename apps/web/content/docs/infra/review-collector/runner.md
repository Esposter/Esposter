...
**The runner declares the two and the trigger inherits everything.** The declaration is the runner's own interface — the only secrets any step in it spends — and the inheritance is what keeps the secret list out of the cross-ref contract above. Inheriting narrows nothing: `CI.yaml` runs on a push to any branch with the same repository secrets, so a step added to `ai/queue` reaches every one of them through that workflow whatever this call hands over.
