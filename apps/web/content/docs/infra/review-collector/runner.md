...
| `status`                              | the `CodeRabbit` context                                | the same free event from its other end; the body and the status flip arrive as separate deliveries, and either is enough. The context is the only filter, and CodeRabbit writes its status on a renovate pull request it never reviewed, so one of those runs a cycle that exits at the gates |
