...
- **`push` on `ai/queue`** — the allocation event. Fires on every push the session makes, which under a standing sweep is one per commit; almost all of them exit at the gates within seconds of the checkout. The one that arrives when the slot is free and the queue has reached the target is the one that pushes.
...
**One copy of the runner runs, whichever event fired it.** GitHub reads a workflow file from the ref the event names — the pushed branch for a `push`, the pull request's merge ref for a `pull_request_review`, the default branch for a `status` or a comment — and the copy `main` carries is the oldest, a release behind `ai/queue`, while the collector reads remote state whose shape the newest copy defines: ref names, bot logins, the head branch its filter compares. Left to the event, a rename in the collector is a deadlock rather than a lag — the copy on `main` fails on the missing ref and so never ports the commit that would have taught it the new name. So `ReviewCollector.yaml` is nothing but the triggers, the one part that has to exist on the event's ref, and it calls the reusable `run-review-collector.yaml` **at `ai/queue`**, which holds every job, filter and step; that workflow's checkout pins the same ref, because `actions/checkout` follows the caller's event sha rather than the ref the file was read from, and the local actions and the cycle are read from that tree. The queue head is the copy every queue push already runs and the session's own tree. A test (`scripts/src/services/coderabbit/collect/constants.test.ts`) keeps the four places the two files spell the branch — the push trigger, the call, the checkout, the retrigger's dispatch — equal to the constant. What still waits for a release is a trigger added to the shell: it fires from `develop` and the pushed branches at once, and on statuses and comments once the release that carries it merges.
...
The dispatch names the trigger shell at `ai/queue`, which picks the copy of the triggers that runs and nothing else — the shell pins the reusable workflow, and every branch the cycle acts on it reads from `origin`.
...
What the session is handed, and what it does with each finding, is the [drain](/docs/infra/review-collector/drain) page.
...
**The one credential it does hold is the Claude Code token, and that is a trade rather than an oversight.** The token authenticates the `claude` process itself, and the process that holds it is the process that spawns every tool the session runs, so no authentication mode Claude Code offers keeps it from a tool subprocess — a helper command or a credentials file is read by the same user the tools run as. What it can spend is the account's own subscription, never anything in this repository: a session steered into exfiltrating it burns the owner's Claude quota until the token is rotated, and a session steered into writing it into a fix commit puts it on `ai/review-fixes`, a branch the runner pushes and a person reads. Closing the channel means an egress allow-list around the runner, which is a heavier control than the blast radius earns while the drain reads one bot's prose about one maintainer's code; it is the first thing to add if the repository ever takes contributions the drain would read.
...
| `REVIEW_COLLECTOR_TOKEN`  | the `gh` CLI login token of the account that runs the working session, refreshed with the `workflow` scope | the collector's own pushes to `develop` and `ai/review-fixes`, its thread replies and the release pull request it opens — never the drain's, and the built-in `GITHUB_TOKEN` cannot do the job |

**Those two are handed across by name, never `secrets: inherit`.** The repository's other secrets are the Azure and Pulumi credentials that deploy the estate, and the trigger is a caller whose callee is pinned to `ai/queue` — a mutable branch the working session pushes, checked out by a job whose drain step runs a permission-skipping Claude session over one bot's prose. Inheritance makes every one of those credentials reachable from any step added to that branch; the list makes the blast radius the two the collector actually reads. The cost is a sync the trigger cannot be pinned to pay: GitHub reads it from whichever ref the event names, so a secret the reusable workflow grows and `main`'s copy of the trigger has not yet mapped arrives empty on a `status` or an `issue_comment` alone — which is why the reusable workflow declares both `required`, so the gap fails the call instead of the step that spends it, and why `constants.test.ts` holds the two files to each other the same way it holds them to the queue branch.
