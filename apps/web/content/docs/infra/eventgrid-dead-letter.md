...
Every Event Grid subscription on the application topic carries a `deadLetterDestination` pointing at the `deadletter` container in the same-environment storage account, alongside the tightened retry policy `AzureEventSubscriptionRetryPolicy` declares — ten attempts over a one-hour time-to-live. The shorter window means a permanently failing event lands in the container while it is still relevant, rather than after a full day of doomed retries. Event Grid writes each exhausted delivery as a JSON blob holding the original events.
...
So the budget guard leaves this subscription alone, and that is load-bearing rather than an oversight. The guard stops the Function App and deletes the two highest-volume **application** subscriptions; the one it leaves alive (blob deletions) keeps dead-lettering into the container while the app is down, so its payloads survive as blobs rather than being dropped. Deleting the replay subscription for the duration would lose even that, in trade for the delivery attempts it would have made against a stopped endpoint: a per-operation charge bounded by the dead-letter rate, and orders of magnitude below the account-wide ingress described in the notes below, which no subscription change affects.
...
| `apps/functions/src/services/deadLetter/checkIsReplayable.ts`                                                    | The replay cap and handler-idempotency bars a dead-lettered event must pass         |
| `apps/functions/src/services/deadLetter/deleteReplayedBlob.ts`                                                   | Best-effort delete of a handled original, logged rather than rethrown               |
| `packages/db-schema/src/services/azure/function/AzureFunctionIsIdempotentMap.ts`                                 | Which handlers a replay may safely rerun                                            |
| `apps/functions/src/services/deadLetter/writeDeadLetterBlob.ts`                                                  | Copy a payload under a prefix — copy only, the handler owns the delete              |
| `apps/functions/src/services/deadLetter/parseReplayId.ts`                                                        | Split an event id into its original identity and replay count                       |
| `apps/functions/src/services/deadLetter/formatReplayId.ts`                                                       | Write the `<eventId>\|<attempt>` id a republished event is sent with                |
| `apps/functions/src/services/deadLetter/constants.ts`                                                            | `MAX_DEAD_LETTER_REPLAY_ATTEMPTS`                                                   |
| `apps/functions/src/models/deadLetter/ReplayId.ts`                                                               | The parsed `{ eventId, replayAttempts }` pair                                       |
...
| `apps/infra/src/azure/resources/Microsoft.EventGrid/eventSubscriptions/`                                         | The application subscriptions and the filtered replay subscription, per environment |
| `apps/infra/src/azure/constants/AzureEventSubscriptionRetryPolicy.ts`                                            | The retry bound every application subscription and the guard's recreate share       |
...
The replay subscription can only be created after the environment's Function App already hosts `ReplayDeadLetterEvent`: Event Grid validates the endpoint at create time and rejects an unknown function with `Webhook endpoint validation failed … StatusCode: NotFound`. So the order per environment is deploy the Function App release first, then `pulumi up` the subscription.
