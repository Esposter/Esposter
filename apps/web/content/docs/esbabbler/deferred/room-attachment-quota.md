...
**What it would reuse.** Almost all of it: the ledger (`storageLedger` already keys on container plus blob name), the reserve-at-SAS-issuance gate, and the `BlobCreated` reconcile handler — the subscription would take the message-assets prefix back, and the counter would move from the user row to the room row.
