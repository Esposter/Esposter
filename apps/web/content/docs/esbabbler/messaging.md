...
- **`lastMessageAt` is server-only.** It is the slowmode clock and nothing else: `updateUserToRoom`'s input carries `lastReadAt` — the read marker the room list's unread dot and Mark Unread From Here move — so a client can never reset the clock its next send is checked against. Only a send path writes `lastMessageAt`.
