...
- **Account linking** — one account can hold several social providers, listed and managed at `/user/settings`. See [account linking](/docs/user/account-linking).
- **Public profile** — every user has a public page at `/user/[id]` showing their identity, achievement showcase, and posts. See [public profile](/docs/user/public-profile).
- **Sessions** — every active session listed at `/user/settings`, revocable one at a time or all but the current one, with push subscriptions and live connections going with the session. See [session and device management](/docs/user/session-device-management).
...
Open work: [roadmap](/docs/user/roadmap). Decided ideas: [deferred](/docs/user/deferred), [rejected](/docs/user/rejected).
