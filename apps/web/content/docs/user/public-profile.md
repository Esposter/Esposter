...
The `userId` filter composes with the existing clauses on `readPosts`: the `parentId` branch, the lexicographic cursor, and [feed block filtering](/docs/post/feed-block-filtering). Block filtering still applies on a profile — a blocked author's identity stays reachable by direct navigation, but their posts stay hidden from the viewer who blocked them, exactly as in any other feed.
