...
**Monsters** — `MonstersDataMap` defines each species' sprite asset, attack kit, and base statistics (`level`, `maxHealth`, `attack`, `defense`, `baseExperience`). A species states no starting status: every one starts at full health with no experience, so `monstersData` derives the status from the statistics and the two can never disagree. Each species plays a distinct role: Aquavalor is the tank (low attack, high defense and HP), Carnodusk is fast and fragile (highest attack, lowest defense and HP), Frostsaber is the rare prize encounter (strong on every axis, highest `baseExperience`), Ignivolt is a glass cannon, and Iguanignite is the balanced starter. `new Monster(key)` takes the species data from `getMonsterData`, which hands every caller a copy, and assigns a UUID, so every caught monster is an independent instance whose stats then grow by [level-ups](/docs/dungeons/battle). The player's party lives in `save.player.monsters`.
