...
| [Schema forms](/docs/proposals/refactors/ui-library/schema-forms)         | JSON Forms, dressed in the library, for the Zod-generated forms vjsf draws today              | The sheet column and dashboard dialogs match the rest of the app, and the last Vuetify-bound engine goes |
...
Three more stand beside it, each a gap the library still has, which a unit or a feature can take up whenever it needs one: [date ranges](/docs/proposals/refactors/ui-library/date-ranges) in the calendar, for the resource list's custom filter and a sheet's date cells; [data table columns](/docs/proposals/refactors/ui-library/data-table-columns) a reader resizes, pins and packs tighter; and the [event calendar keyboard](/docs/proposals/refactors/ui-library/event-calendar-keyboard), rescheduling without a pointer.
