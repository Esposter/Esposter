...
A component is added to the library when the first stage needs it, never ahead of a consumer. This page is the plan for all of them, so each is built once, in the right shape, rather than grown from whichever page reached for it first. A row names the Vuetify 0 primitive that owns the behaviour; where it names none, the component is presentation only and Vuetify 0's polymorphic base element is enough.

## Every component keeps the same contract

- **Behaviour from the primitive, look from the tokens.** The component spreads the primitive's attributes onto its own element, styles its states through the data attributes those attributes carry, and holds no colour of its own ([design language](/docs/proposals/refactors/ui-library/design-language)).
...
| Menu                    | Popover with roving focus                     | the console's menu, the Vuetify menu, the Styled overflow menu                            | agent console   |
| Select, combobox        | Select, Combobox                              | the console's select, the Vuetify select, autocomplete and combobox                       | agent console   |
| Progress, spinner       | Progress                                      | the console's spinner and loading bar, the linear and circular progress                   | agent console   |
...
| :------------------------------------------------------- | :------------------------------------------------------- |
| `apps/web/app/components/AgentConsole/Panel/Menu.vue`    | Becomes the library's menu, with a real listbox contract |
| `apps/web/app/components/AgentConsole/Panel/Popover.vue` | Becomes the library's popover on Vuetify 0's             |
...
- [WAI-ARIA Authoring Practices: date picker dialog](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/examples/datepicker-dialog/), W3C: the keyboard contract of the date grid.
