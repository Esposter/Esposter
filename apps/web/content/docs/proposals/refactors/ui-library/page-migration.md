...
5. **Checked by eye.** The unit is handed to the user with its inventory and the states to look at, per the `run-app` skill: no browser driven by an agent, and no screenshot suite. Every inventory row must hold. A row that fails sends the unit back to step three, not forward with a note.
...
- The migration is a revamp of the design system, not a repaint. Every unit walks the `ui-library` skill's design pass before it is built and before it is handed over. The first thing that pass owes the library is motion: dialogs, popovers and toasts appear and leave in a step, and a confirmation sits in the middle of the screen rather than high, where the palette keeps its field still.
