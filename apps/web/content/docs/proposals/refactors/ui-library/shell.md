---
title: App shell
description: Proposal — the frame every page sits in, redesigned rather than restyled. The top app bar gives way to a dock, a rail on the left edge on a wide screen and a bar along the bottom on a narrow one; pages own their titles and actions; and every flow the app bar carries today has a written place in the new frame.
...
- **The products are on the dock directly.** There are few enough of them that a grid behind a button only hides them. Each is an icon button with its name as its label and tooltip, and the current product is marked in the accent colour. The games are one entry that opens a small menu, as the grid's nested group does now.
...
flowchart TD
  ...
  RL --> H[Page header: title, breadcrumbs, command bar]
  BB --> H
...
| Products grid, with the games group                                 | The products on the dock, the games as a menu                          |
...
- **By eye**, on a wide and a narrow screen, signed in and signed out: every row of the table above, each product reached from the dock, each drawer opened and closed, a notification arriving, and a dialog opened from a page not yet migrated.
...
| `apps/web/app/components/App/Menu/Button.vue`          | Replaced by the products on the dock                          |
...
| `apps/web/app/services/app/ProductListLinkItems.ts`    | The dock's products                                           |
...
apps/web/app/components/App/Dock/Products.vue    the products, with the games menu
