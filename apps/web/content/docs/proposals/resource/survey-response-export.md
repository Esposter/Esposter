...
- **Columns** are the submission time, then one per question in the survey's order, headed by the question **title** rather than its internal name; a checkbox question's answers join into one cell separated by `;` and a space. In an Identified survey the respondent column comes first ([survey response modes](/docs/resource/survey-response-modes)).
