---
title: Voice setup
description: Proposal — the voice verb installs the engine's runtime and weights into the plugin's state directory, records the reference language, and proves the path by speaking one sentence; the reference clips are fetched from the wiki per character as they are needed and cached beside the weights, never committed.
---
...
3. **The language.** One file, `~/.claude/genshin-persona/language`, holding the dub's code: `en`, `ja`, `zh` or `ko` — the four the wiki hosts. Every request the hooks send carries it, so a running synthesizer honours a switch without a restart. The choice is one for the whole roster: the file is one value, and a mixed roster is not a state this design holds.
4. **The proof.** The verb fetches the current session's character's reference in that language and speaks one sentence through the [resident synthesizer](/docs/proposals/infra/character-voice/resident-synthesizer), so the person hears the voice they set up before the first reply does.
...
flowchart TD
    ...
    Runtime -- yes --> Weights
    Weights -- no --> Fetch --> Language
    Weights -- yes --> Language
    Language --> Proof
```
