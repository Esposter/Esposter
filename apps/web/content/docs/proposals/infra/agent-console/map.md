...
- **A minimap in the corner, turned with the camera.** A round map sits in the top corner under the heads-up display, the player an arrow at its centre. It turns as the camera turns, so up on the map is the way W walks, the way a Minecraft minimap mod draws it by default; a mark on its rim points north, and another points home to the room when the room is off its edge. It shows the ground's colour a column at a time, shaded by height so hills read, and the room drawn as its roof. The agents are left off it: they are inside the room, which it already marks.
