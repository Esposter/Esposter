...
- **Other tooling joins without either side knowing the other.** Any page of the app opens in a side pane with no code, the resource explorer included; any external tool with a UI joins through MCP Apps, the open standard the console hosts; a first-party view is one registry entry ([extensions](/docs/proposals/infra/agent-console/extensions)).
