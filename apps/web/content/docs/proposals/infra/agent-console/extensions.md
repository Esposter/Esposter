...
- **Isolation.** Every app renders in a sandboxed frame with scripts allowed and same-origin denied, under a content security policy built from the origins the app's resource declares; the frame never sees the console's cookies, storage or wire, and the console accepts only messages from the frame it created.
