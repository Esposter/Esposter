...
- **Every animation ends.** A figure walks to a station and stops, and a gate opens and settles. While any motion runs, the world draws frames, and when the last one ends it goes back to drawing nothing. Ambient motion, such as an idle breath, drifting leaves or the element backdrop, is either a shader driven by a time uniform at a capped low frame rate, or it is left out. It runs only while the session is working, so an idle session still draws nothing. It stops completely when the session goes idle, when the tab is hidden, or when the person has asked for reduced motion.
