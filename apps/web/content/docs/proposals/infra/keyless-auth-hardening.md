...
**Today:** Azure Functions already use `DefaultAzureCredential` with least-privilege roles; the Railway-hosted app authenticates to Storage/Search/Event Grid/Service Bus with keys and connection strings (`apps/infra/docs/azure/security-constraints.md` maps each blocker).

**This adds**, per service, in order:

1. **Inventory** — enumerate every key-based client construction in `apps/web/server` + `packages/db` against the constraints doc.
...
| File                                                      | Change                               |
| :-------------------------------------------------------- | :----------------------------------- |
| `packages/db/src/` + `apps/web/server/composables/azure/` | credential-based client construction |
| `apps/infra/src/azure/`                                   | role assignments + auth switches     |
| `apps/infra/docs/azure/security-constraints.md`           | tick off each unblocked constraint   |
