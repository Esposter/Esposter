...
flowchart TD
  ...
  B -->|yes| OK[Slot free, frontier is the head]
...
- **A merge commit on the queue is never owed.** `main` merged into `queue` — the way a dependency bump reaches the working branch — carries nothing the queue authored, a merge cannot be cherry-picked, and its content reaches `develop` by the same `main` sync the git skill already describes. The porter skips it and takes the commits around it.
