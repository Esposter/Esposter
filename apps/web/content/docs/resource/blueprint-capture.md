...
- **A resource whose content was never written captures as an entry with no content**, which deploys back to the same content-less state — see [blueprint resource](/docs/resource/blueprint-resource).
...
- Capture is also the cheapest **agent training set**: a captured manifest is a ground-truth example of a working resource graph in exactly the format an AI author would need to emit ([agentic-by-design note](/docs/resource/blueprint-resource)).
