...
- **Surface** — Overview Essentials gains a **Views** row for published resources. The Survey Overview places it beside the response count ([response management](/docs/resource/survey-response-management)) — the funnel in two numbers.
- **Delete and unpublish** — the partition is cleared by `purgeResource`, not by delete: a soft-deleted resource keeps its counts for the whole [recycle bin](/docs/resource/recycle-bin) window, so a restore restores the history too. Unpublishing leaves history intact as well, so re-publishing continues the same counter. The count is the resource's audience, not the version's.
...
- Per-version stats are out of scope; they would belong to [resource snapshots](/docs/resource/resource-snapshots) if ever needed.
