...
description: The dock's notifications — delivered notifications paged in from the server, this tab's own feedback shown beside them, one snackbar queue and one unread badge.
---

# Notifications Bell

Azure-portal notifications parity: a notifications button on the dock with an unread count, one toast queue, and a panel of what has happened recently. It renders **two kinds of entry**, and the difference between them is the whole design:
...
Both render through one `AppNotification` shape, so the panel and the toast never branch on where an entry came from. Only the store knows, and only where it has to: dismissing a delivered entry is a mutation, dismissing a local one is a filter.
...
- save conflict (stale `contentVersion`) → warning with **Refresh** action ([resource page parity](/docs/resource/resource-page-parity))
...
flowchart TD
  ...
  OPEN["Panel opened / dock mounted"] --> RE
  ST --> PANEL["bell panel — merged, newest first<br/>StyledWaypoint pages the delivered half"]
  ST --> BADGE["unread count on the trigger + PWA icon badge"]
  ST --> SNACK["toast queue (~5s, errors persist)"]
...
- **Bell** (`AppNotificationBell`, on the dock, authed sessions): the unread count sits on its trigger, and the trigger opens a `UiPopover` panel — newest first, severity icon, relative time, per-item dismiss, "Dismiss all". Closing the panel marks everything read. The delivered half is cursor-paginated, with a `StyledWaypoint` at the foot of the list paging the next page in. Empty state: "No notifications".
- **Toasts** (`AppNotificationToast`): one `UiToast` in the app's toast stack rendering the store's queue head (~5s, dismissible, held while hovered); errors persist until dismissed. One queue, never ad-hoc toasts beside it.
- `G N` opens the panel ([global search](/docs/resource/global-search) chords).
...
| File                                                   | Role                                                 |
| ------------------------------------------------------ | ---------------------------------------------------- |
| `app/store/notification.ts`                            | the merged list, the toast queue, the two dismissals |
...
| `app/components/App/Notification/Bell.vue`             | the dock's bell + panel                              |
| `app/components/App/Notification/Toast.vue`            | the one toast rendering the store's queue head       |
...
- The delivered half is bounded by retention rather than by the session ([notifications](/docs/architecture/notifications)); a durable audit trail is still the activity log's job ([activity log](/docs/resource/activity-log)).
