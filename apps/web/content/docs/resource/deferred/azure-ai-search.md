...
The reason to wait is that nothing needs it. Postgres covers current volumes, and [`pg_trgm` relevance](/docs/resource/global-search-relevance) already bought typo tolerance for free — the cheap 80% of what a search index would buy, without spending a slot or adding a second indexer to operate.
