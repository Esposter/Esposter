...
One page per deferred idea (not now — with a revisit trigger). Check here and [rejected](/docs/resource/rejected) before adding a roadmap item or proposal; a decided idea is never re-argued. Grid-editor ideas are decided one level down, in the [sheet editor](/docs/resource/sheet)'s own deferred and rejected pages.
