...
Email opts into the **Publishable** capability, so a compiled email has a public URL: `/view/Email/[id]`. That is the industry-standard "view this email in your browser" artifact, a review link to hand a colleague before a send, and the natural merge target for a web-version link when [email sending](/docs/resource/deferred/email-sending) un-defers.
...
- The same `getEmailHtml` compile backs the personalized HTML export ([email personalization](/docs/resource/email-personalization)) — one compile path, two consumers.
...
- Hosted images inside the published copy come from [resource file assets](/docs/resource/resource-file-assets); external-URL images work unchanged.
