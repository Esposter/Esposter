...
- **The data table's `getValue`.** Each column of the grid reads its cells through `getCellText`, and `UiDataTable`'s search filters rows on what each column reads, so the search goes through the format — there is no second global search path to keep in step.
...
Sorting is then pulled back off that text by a `compare` per column, which the data table uses in place of comparing the text. `compareColumnValues` orders numbers by magnitude, booleans false-first, empty cells ahead of filled ones, and everything else through the shared collator.
...
| `app/services/resource/sheet/column/compareColumnValues.ts` | Underlying-value comparator behind each column's `compare`                  |
| `app/store/resource/sheet/row.ts`                           | `getCellText` plus the grid's columns, which carry `getValue` and `compare` |
