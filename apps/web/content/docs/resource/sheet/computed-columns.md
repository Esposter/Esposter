...
A `ComputedColumn` is an ordinary column (`ColumnType.Computed`) whose `transformation` field is a discriminated union keyed by `ColumnTransformationType`: `Aggregation`, `ConvertTo`, `DatePart`, `Math`, `RegexMatch`, `String`, `StringPattern`, or `StringSplit`. Values are never written to `row.data` — computed columns are read-only. A new row's empty data carries no key for one, and the cell-scoped commands (find and replace, the null strategies, text transformations) walk cells through `collectAffectedCells`, which skips a computed column whatever it is handed: there is nothing stored there to match, and nothing a command may write. `NullStrategy.DropRow` works on whole rows through `getNullAffectedRows` instead, which leaves computed columns out on its own, so a null an older row still carries under one never drops that row.
