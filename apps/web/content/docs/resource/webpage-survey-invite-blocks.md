...
Both GrapesJS editors offer the owner's published surveys as drag-in invite-button blocks — the email editor for a send ([email personalization](/docs/resource/email-personalization)), the webpage editor for a landing page that collects responses from anyone who finds it, with zero send infrastructure.
