...
- **The format is generic; the diff is not.** RFC 6902 defines how to express a patch, not how to compute one. Arrays are addressed by index, so a diff that compares elements by position turns one row inserted at the top of a Sheet into a replace of every row below it — a patch as large as the document. A diff that detects insertions needs a longest-common-subsequence pass. Myers' algorithm keeps that near-linear while the edit is small, but its cost grows with the size of the edit and reaches the product of the two lengths at worst: the wrong shape to depend on for a document large enough to need a patch at all.
