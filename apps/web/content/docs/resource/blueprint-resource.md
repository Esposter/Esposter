...
- **Token grammar** — two tokens, both plain string substitution over the JSON's string values:
  ...
  - `{{entry:<key>}}` — replaced with the **created resource id** of that entry. This is how a dashboard entry binds the sheet entry's dataset, or a program entry binds its email and survey — the same bare-id linking every cross-resource reference already uses ([resources](/docs/architecture/resource)), just late-bound.
...
Its twin, [`captureBlueprint`](/docs/resource/blueprint-capture), joins the same router.
...
- **Deliberately no engine.** No conditions, loops, expression functions, or nested-deploy. If a manifest needs logic, the answer is a second blueprint, not a language. The extension seam is the manifest schema (typed parameters, entry-count growth), same posture as the [Program](/docs/resource/program-resource) content schema.
