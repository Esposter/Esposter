...
Every resource carries a flat `Record<string, string>` of tags: shown in Essentials, edited in place, and filterable on `/resource-explorer/all`. Tags are the lightweight organization layer that keeps [resource groups deferred](/docs/resource/deferred/resource-groups) — cross-cutting labels without a grouping entity to model, migrate, and navigate.
...
- A tags-only update leaves no `Renamed` entry in the [activity log](/docs/resource/activity-log): changing a label is not renaming a resource.
- Free-text tag search inside [global search](/docs/resource/global-search) is not wired up; the `/all` filter pill and the `/resource-explorer/tags` list ([service menu](/docs/resource/resource-service-menu)) are the retrieval paths.
...
- If tag usage grows into "give me a folder", that is the [resource groups](/docs/resource/deferred/resource-groups) revisit trigger, not more tag features.
