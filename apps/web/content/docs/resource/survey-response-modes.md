...
- **Setting** — `responseMode` joins the survey content `settings` section alongside the [response controls](/docs/resource/survey-response-controls) toggle. One settings object, one write path, one `contentVersion`. Default `Anonymous`.
...
- The program is the issuer and the survey is the gate: token validation lives here, issuance lives in the [Program resource](/docs/resource/program-resource).
