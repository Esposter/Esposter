...
The email editor joins the data flow: merge-field blocks bound to a `DatasetReference`, survey invite blocks, and per-row personalized HTML export — applying the [datasets standard](/docs/architecture/dataset) to the email product. Actually sending email is deferred ([email sending](/docs/resource/deferred/email-sending)).

## How it works

An email optionally binds one dataset (`EmailEditor.datasetReference`, stored inside the content blob — every GrapesJS save carries it over since project data doesn't know about it). The bound dataset's columns appear in the block manager as drag-in merge-field blocks (`{{columnName}}`), and the owner's published surveys appear as styled invite-button blocks ([survey invite blocks](/docs/resource/webpage-survey-invite-blocks), shared with the webpage editor). Export compiles the current MJML to HTML once (`mjml-code-to-html`), then zips one personalized `.html` per dataset row with merge fields substituted.
...
| `app/services/emailEditor/getEmailHtml.ts`                 | the one MJML compile, shared with the [web view](/docs/resource/email-web-view) |
...
- A capped dataset read never exports silently — the command stages the dataset and the Editor blade's truncation confirm owns the decision ([dataset row cap warning](/docs/resource/dataset-row-cap-warning)).
- Exported files keep app-origin asset urls (`/api/resource-assets/…`), which resolve only for a request carrying the owner's session cookie — images are blank in a file opened straight off disk. Durable public asset urls come with [email sending](/docs/resource/deferred/email-sending).
