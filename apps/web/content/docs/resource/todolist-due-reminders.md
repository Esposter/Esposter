...
- TodoList items have no completion state yet (`TodoListItemType` is `Todo`-only), so the fire-time check verifies existence and due-date match; [completion](/docs/proposals/resource/todo-list/completion) makes a completed item the third drop condition.
