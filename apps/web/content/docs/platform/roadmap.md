...
- [ ] [Resource version store](/docs/proposals/platform/resource-version-store) — hold version history as content-addressed keyframes and deltas so a version costs the edit rather than a full copy of the document, and the storage meter stops climbing for a resource that did not grow. Its own workspace package, no new Azure resource, and a second phase that retires publish-time asset cloning
