...
- **A Sheet holds exactly one data source**, so nothing in the store or the dataset provider needs a sub-item selector. A sibling resource type for the Vuetify component demos was [rejected](/docs/platform/rejected/vuetify-component-resource).
