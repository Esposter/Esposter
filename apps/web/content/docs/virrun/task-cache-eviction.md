...
The host-global `~/.virrun/tasks/` directory is the one cache surface with no superseded-entry sweep. Every other tier self-prunes — `snapshots/` and `prepare/` evict superseded hash directories on each run, source mirrors reap on origin death — but each `(lockfile, working-tree, command)` state mints a fresh `tasks/<key>` entry carrying a full produced-file payload (a `pnpm build` entry carries `dist/`), and nothing but `cache clean --all` would ever remove one. An age-based prune bounds it.
