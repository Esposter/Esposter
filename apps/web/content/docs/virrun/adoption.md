...
| `virrun doctor`              | Probes each `os`-backend prerequisite (on win32 first that WSL starts at all, naming wsl.exe's own reason when it does not; then bubblewrap ≥ 0.10.0, WSL Linux node, python3, host tar, the real overlay-mount verdict) and prints an aligned per-check report; exits non-zero on a gap. |
...
Esposter is the first consumer — dogfooding _is_ the test corpus, not a separate effort. The committed root `virrun.config.ts` branches win32 → `os`, else `native` ([configuration](/docs/virrun/configuration)), and which root scripts carry the `virrun --` prefix, with the reasons per platform, is [monorepo tooling](/docs/architecture/monorepo-tooling). The mutating dev-loop siblings (`lint:fix` → `oxlint --fix`/`eslint --fix`) carry it too, wrapping each underlying step so their edits flow back to the host ([write-back](/docs/virrun/write-back)). `oxfmt` is off the ladder in both directions, for the reason [monorepo tooling](/docs/architecture/monorepo-tooling) gives, and that is also what lets `format` be the one 🏗️ CI check with no `needs: build-packages`: with no `virrun` binary in the command there is nothing in the job that has to be built first. Watch-mode `watch:packages` and network commands (`outdated:dependencies`) run native: the sandbox can't cache them and would only add the prepare-rebuild + mirror tax. `build:packages` runs native too: no `packages/*` build touches Nuxt, their output is platform-neutral and every downstream consumer reads it from host disk, so sandboxing it would buy an isolation nothing wanted and pay the mirror tax to hand the same `dist` back. The app build (`pnpm -C apps/web build`) is native as well, for a different reason — it is only ever run in anger by Linux hosts, the deploy and CI's build job, where the config resolves native regardless — so on win32 the prefix only ever charged the mirror + write-back round trip for an `.output` nothing local reads. The Linux-generated `.nuxt` prepare layer serves the checks that do carry the prefix: `typecheck` and `test` read it. `pnpm install` is never on the ladder, for the reason [materialize node_modules](/docs/virrun/rejected/materialize-node-modules).
