...
- **`node:vfs`** ([nodejs/node#61478](https://github.com/nodejs/node/pull/61478)) — official core virtual filesystem, in Node 26 behind `--experimental-vfs`. Provider-based, full `fs` API compat, mounts inside a reserved namespace that never shadows a real path, module loading from virtual files. The end state virrun targets.
