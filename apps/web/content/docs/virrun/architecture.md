...
4. **[Task cache](/docs/virrun/task-cache)** — a persist run with unchanged inputs is skipped, keyed by `sha256(environment-key + working-tree-hash + command)`; a hit skips the sandbox and replays the recorded output diff + streams. A dev-loop lever — off in CI, where a fresh commit means ~0 hits.
