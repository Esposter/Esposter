...
- No silent wins: if a speedup only appears in an unrealistic state (everything pre-warmed by the bench itself), say so. There is **no install bench group**, for the reason [materialize node_modules](/docs/virrun/rejected/materialize-node-modules) records.
