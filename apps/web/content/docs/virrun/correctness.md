...
5. **Property/fuzz** — two halves. The vfs seam: fast-check drives randomized read/write/exists sequences against the provider and a real `node:fs` temp directory in lockstep and diffs the full trace (node:fs is the oracle, so no fs semantics are re-implemented; failures shrink to a minimal counterexample). The os half: randomized _command_ sequences through the ephemeral RAM overlay assert the isolation invariants under every ordering — the host disk is never mutated, no write leaks across fresh-per-exec uppers, every command yields a well-formed result. Host-gated, small run counts (each op is a real subprocess).
