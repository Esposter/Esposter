...
- **Stored ranking** — a Reddit-style hot score computed at write time and stored on the row, so the feed is a simple indexed sort. See [feed and ranking](/docs/post/feed-and-ranking).
...
- [Feed and ranking](/docs/post/feed-and-ranking) — the home feed, sort options, cursor pagination, and the hot score.
- [Posts and comments](/docs/post/posts-and-comments) — CRUD, the self-referencing model, rich text.
- [Likes](/docs/post/likes) — the ±1 like model, viewer-scoped reads, and transactional counter updates.
- [Feed block filtering](/docs/post/feed-block-filtering) — blocked users' posts and comments are hidden from feeds.

Open work: [roadmap](/docs/post/roadmap). Decided ideas: [deferred](/docs/post/deferred), [rejected](/docs/post/rejected).
