...
- **`refetch(key?)`** — an unconditional re-read, deliberately **not** single-flight, for the reason [async operations](/docs/architecture/async-operations) gives under the invalidating re-read; it supersedes the in-flight answer on latest-wins instead.
- **`supersede(key?)`** — what a store calls before it assigns a value a subscription pushed, because that value is newer than the read still in flight for the same key: the read is marked stale so it cannot land afterwards and put the older number back. The window is real — the subscription is established before the first read is issued. Ordering two async sources against each other is the primitive's job — a store that grows a pair of counters beside its `ref` has hand-rolled the call id the ban in [async operations](/docs/architecture/async-operations) names.
