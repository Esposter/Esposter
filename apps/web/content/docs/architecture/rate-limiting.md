...
Three procedure builders sit on top: `standardRateLimitedProcedure` (public, standard budget), `standardAuthedProcedure`, and `slowAuthedProcedure`. `AuthedProcedureMap.ts` maps the enum onto the latter two, and the room procedure builder `getPermissionsProcedure`, described in [RBAC](/docs/esbabbler/rbac), takes a `rateLimiterType` parameter defaulting to `Standard` and selects through it, so moving a room procedure onto the slow budget is a one-argument change at its declaration.
