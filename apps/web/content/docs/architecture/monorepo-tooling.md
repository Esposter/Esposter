...
pnpm --filter "@esposter/web..." run build
...
- Use filters instead of Lerna scopes/ignores, and only where a filter says something a path cannot: a set, or a dependency closure. One project is `pnpm -C <directory> run <script>`.
- `build:packages` and each app's own `build:*` are different sets rather than a sequence: the libraries are what CI caches and hands to the checks. `pnpm build` derives both its selectors instead of naming them — `--filter "@esposter/web^..."`, everything the app imports, topologically, plus `--filter "{.}^..."`, what the root itself depends on — and then runs `build:web`. The second one is there because the toolchain is declared at the root and the app does not import it: `virrun`'s committed `bin/cli.js` imports a `dist` only its own build writes, so a deploy building from source rather than from CI's `package-builds` artifact reached `build:web` with no `virrun` to run it with. It is the one place the toolchain is a build input rather than a dependency. `^...` rather than the `{.}...` that `setup-packages` installs, because this is a `run`: matching the root would have `build` recurse into itself, which is the same reason `includeWorkspaceRoot` is refused below. It stays two commands because only the second belongs in the sandbox: a library's output is platform-neutral and read from host disk, so `virrun` would charge it the mirror tax for an isolation nothing wants ([virrun adoption](/docs/virrun/adoption)).
...
Tests run through a single root `vitest.config.ts` with two kinds of `projects` entry: a glob per product root (`apps/*` and `packages/*`, each member by its own config, the app as a Nuxt project via `defineVitestProject`) and the root `scripts/` suite — the dependency graph, the outdated-dependency report, the oxlint plugins and the sweep scans. `scripts/` is not a workspace package, so it needs its own entry with both its `include` and its `benchmark.include` scoped to its own tree; the default `**/*.bench.ts` would otherwise pull every package's benches into it. So `coverage`/`test` are `vitest run` at the root, not a `pnpm -r` fan-out, and a coverage shard can land a script test as readily as a package one. A third `agents` entry over `.agents/**/*.test.ts` existed while the code review ran as a workflow script and went out with it; the agent tree holds no executable now, and a check **about** it lives in `scripts/` like any other.
...
**The `package-builds` entry keeps an exact key.** Its `packages/*/src/**/index.ts` glob also catches the hand-written directory barrel a package may keep under `src` — a tracked file, covered by the key, so an exact hit proves the restored copy is the checked-out copy. Restored by prefix it would be another commit's version of that file, extracted over the checkout and built, green. 🏗️ Pulumi and the two Functions deploys restore this same entry for the libraries they link against and build their own app on top of it, so a prefix hit would hand them another commit's libraries to deploy against. Those two deploys are one definition invoked twice — `deploy-function-app.yaml`, the second reusable workflow beside `build-packages` — so the branch each triggers on and its two resource names are all that differ between the stacks.
