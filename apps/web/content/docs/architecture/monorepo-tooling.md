...
- Pass tool flags as **direct args**: `pnpm coverage --reporter=blob --shard=1/4`, `pnpm test -u`. pnpm appends them to the script's command line verbatim, including the ones it has of its own — `--reporter` is a pnpm CLI option and is still forwarded rather than swallowed. Do **not** use the `pnpm <script> -- <args>` separator form: pnpm forwards the literal `--` into the script's arguments, so the underlying tool treats the trailing flags as post-`--` positionals and silently ignores them (this dropped `--shard`/`--reporter` in CI).
