...
- `build:packages` and each app's own `build:*` are different sets rather than a sequence. The libraries are what CI caches and hands to the checks; `pnpm build` is the closure `--filter "@esposter/web..."`, which builds the app and everything it imports topologically, so nothing has to name the library build first.
