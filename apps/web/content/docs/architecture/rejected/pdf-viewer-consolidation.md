...
**What the full viewer does is on the stop list twice over.** Its product is a document-reading surface: virtual scrolling so a large PDF loads without rendering every page, in-document search, a table of contents, printing, XFA and AcroForm rendering, rotation, keyboard navigation, and ARIA attributes with localized tooltips. Accessibility-shaped packages are never absorbed ([dependency reduction](/docs/architecture/dependency-admission) says why) — and virtual scrolling a hundred-page document is engine-shaped rather than adapter-shaped. The [admission test](/docs/architecture/dependency-admission) refuses this at its second and third gates, before the version-lock argument is ever reached.
