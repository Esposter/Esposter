...
- `app/error.vue` is the boundary itself. Nuxt renders it in place of `app.vue`, for a route nothing matches as much as for a thrown failure, so it is drawn with the UI library alone rather than inheriting the app's frame, and it turns an escaped error into a page that says which of the two things happened — a route that does not exist, or a failure — and offers the way back that fits. It reports nothing anywhere either; it is the surface, not the collection.
