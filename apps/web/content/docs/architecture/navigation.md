...
- **Internal route** — `<NuxtLink :to="RoutePath.Resource(id)">` (or `<NuxtInvisibleLink>` when the link should inherit surrounding styling). Real anchors, so keyboard and middle-/ctrl-click work. A Vuetify component with a plain destination takes `:to` directly — same real-anchor semantics — and an inline `navigateTo` handler is reserved for a target computed at click time; which components, and the `RoutePath` rule behind every target, are the `routing` skill's.
