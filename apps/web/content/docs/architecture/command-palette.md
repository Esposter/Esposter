...
- **The app's own commands are the dock's.** Home, every product, the reader's bookmarked and recent pages and the account menu's entries are registered app-wide, and the account menu reads the same list, so the two never disagree. A condition such as being signed in is part of the list, so it is re-read whenever the palette shows it. Choices among several — the design styles, the theme modes — are listed whole under their heading, the chosen one marked.
