...
- **Fetch on load + manual refresh.** No live subscriptions through this layer ([deferred](/docs/resource/deferred/realtime-dataset-refresh)).
- **No external providers** (HTTP APIs, SQL) until secret storage and injection-safety work is scoped ([deferred](/docs/resource/deferred/api-sql-dataset-providers)) — the enum grows one value per new provider, nothing else changes.
