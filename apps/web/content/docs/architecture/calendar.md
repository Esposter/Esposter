...
The look, the tokens and the keyboard contracts they share with the rest of the library are the [UI library](/docs/architecture/ui-library) page's; this page is what is particular to time.
...
| Outlook         | A double click on an empty day or slot creates there, and a drag moves an event to another day or slot                                      |
...
Every shortcut is a registered command ([UI library](/docs/architecture/ui-library)), listed in the shortcuts dialog and bound only while a calendar is mounted.
...
flowchart TD
  ...
  B[Reader double-clicks an empty day or slot] --> G{Did the page pass onCreate?}
