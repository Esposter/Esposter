...
**An exception granted to an interactive command does not transfer to an unattended one.** A bounded poll can be right when a person is watching and has nothing else to wait on — `ai:coderabbit:probe` posts a comment and waits for the bot's reply, because a terminal cannot subscribe to a webhook. The moment that same code runs inside a runner, the premise is gone: the runner _is_ a webhook consumer, and the wait stops being a convenience and becomes a job holding a concurrency group. So the caller's context is part of the rule, and the same function can be correct in one and banned in the other. The [review collector](/docs/infra/review-collector/runner) is the worked case — the cycle asks and exits, and the answer arrives as the event it always was.
