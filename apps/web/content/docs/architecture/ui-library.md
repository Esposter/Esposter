...
The account menu also holds the [readable-text setting](/docs/architecture/design-language#type), the design style and the theme mode. The dock's command button, after the launcher, opens the [command palette](/docs/architecture/command-palette), and anything on the dock with actions of its own opens them as a [context menu](/docs/architecture/context-menus).
