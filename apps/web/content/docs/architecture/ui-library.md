...
| `UiDateField`       | `UiPopover`, `UiCalendar`             | A day, or a day and a time, read in the reader's zone and picked from a calendar under a field drawn as a select's trigger; it may be emptied                                                             |
| `UiEventCalendar`   | `UiCalendar`, `UiToggleGroup`         | Events laid out by the day, the work week, the week or the month, after Outlook: a navigator, today, the working day shaded, a drag to move and a double click to create                                  |
