...
| Sunk             | Background colour shaded along the bottom                       | A filled tone of the text colour; a field adds a muted hairline along its square bottom, as Material's does |
| Search           | Square, as every voxel corner is                                | A pill: every search field and the button drawn as the field it opens                                       |
| Active indicator | The field's bottom shade lit, a tab's line, a half step thick   | A line two hairlines thick in the accent, along a focused field and under the selected tab; never a ring    |
...
| Focus ring       | A solid outline a step wide                                     | The same outline at half the width, following the corner; a field draws its indicator instead               |
...
- **A focus ring around a field.** An outline around a filled field reads as a second edge; the field's active indicator marks it instead, and the ring stays for everything a keyboard reader moves between.
...
- **`ui-field`** — the shape a text field and a select's trigger take over `ui-sunk`: the style's resting line along a square bottom — Material's filled field, a muted hairline in standard, and voxel's sunk shade — which a focused field's active indicator thickens in the accent. A search is a pill instead and draws no line.
