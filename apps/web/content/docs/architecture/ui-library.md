...
- **The dialog shell** takes the library's words — a title, what confirming does — over the library's dialog, and mounts its body only while open ([dialog shell](/docs/architecture/dialog-shell)). It drops in and stands in the middle, as a confirmation does ([motion](#motion)).
