...
- **`ui-button`** — something pressed, one control height of eight steps that a field and a select's trigger share so a row of them lines up, its content centred and an icon button square: `ui-raised` with a button's hover and disabled states, filled by its variant or while pressed, keyed on `data-variant`, `aria-pressed` and, for a toggle group's choice, `aria-checked`; a quiet toggle, such as an editor's bold, fills while pressed too. A shortcut rather than a component's scoped style, so `UiButton` and `UiButtonLink` wear one look.
