...
| `UiTextField`       | Input                                 | A labelled field of one line or several, its rules checked as the reader types; a search is a pill; it may be disabled, and a `maxlength` stops the typing and counts it under the field                  |
...
- **A text field** is named by its label, drawn above it unless what surrounds the field already says what it is for, as a column's filter under the column's name does. A failing rule marks it invalid and points it at the message under it, which is a polite live region, and the form around it counts the result at once, so a submit button can stand disabled before it is pressed. A disabled field is out of the tab order and takes no input, as a native disabled control is.
...
- **A calendar is ours, over the platform's Temporal.** Vuetify 0 ships no date picker yet, only a date adapter, and that adapter is written against a Temporal polyfill of its own — a second copy of Temporal in the bundle beside the platform's, for arithmetic a plain date already does. `UiCalendar` walks plain dates, so a time zone never moves a day, and `UiDateField` turns the instant it holds into the reader's own day and time and back. Weeks start on Monday, ISO's first day, rather than the reader's locale's, which only the browser knows and the server render would disagree with.
...
| `apps/web/app/services/styled/getUiButtonProps.ts` | A Vuetify button's props in the library button's words, for the dialog shell's confirm button   |
