...
**Confirming is a function the dialog awaits, and the dialog closes itself.** `StyledDialog` takes `confirm`, `StyledFormDialog` takes `submit` and `StyledEditFormDialog` takes `save`, and all three answer through `useDialogAnswer`, as `UiConfirmDialog` does. Awaited, the button is pending until the function settles and a `false` keeps the dialog open with the draft intact, which is what a create form wants. The answer runs through `useMutation`, so a function that rejects keeps the dialog open too, and raises the error alert any other write does. With `isOptimistic`, the dialog goes the moment the function is called, because the write has already shown on screen. A consumer never holds a pending flag or a close callback of its own ([destructive confirmation](/docs/architecture/destructive-confirmation)).
