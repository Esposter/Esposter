...
### The content address depends on key order, and that is settled

Those same bytes are also the key they are stored under: a retained version is addressed by the SHA-256 of the serialized content, so a save of content the store already holds writes nothing and costs nothing ([resource version store](/docs/resource/resource-version-store)). That makes the order `JSON.stringify` emits load-bearing — and it holds by specification. ECMAScript fixes own-property enumeration order (integer-like keys ascending, then string keys in insertion order), `JSON.stringify` walks that order, and there is exactly one producer of these blobs: a Zod parse output built by one code path in one deploy. The same content in gives the same bytes out.
