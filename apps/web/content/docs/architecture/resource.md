...
- **A type's content store composes the resource store rather than loading its own copy** — `useSheetStore`, `useNoteStore`, `useDashboardStore` and the rest hold only their own parsed content and reach the row through `readResource`/`readContent`/`saveContent`. The store cannot be generic over `ResourceType`, so the type parameter moved to `readContent<ResourceType.Sheet>(applyContent)`, the one member whose content shape depends on it — it hands the content to `applyContent` only while the read is the latest one issued for the opening still open, so a read landing after a switch or a restore's re-read reaches no store; everything else about a resource row is identical for every type. One row, one publication, one loading flag, so a rename in the toolbar is the name the editor sees.
- **Content is read once per open resource, not once per blade.** The page reads the row before any blade mounts, and a content store's `loadContent` reads the blob only while `checkIsContentRead()` says the store holds none for this resource. Every blade of a type renders the one content, so switching blades renders from the store with no round trip; what changes the content after that reaches the store without a re-read — its own saves, the type's live subscription, and a restore's `ResourceContentHookMap.Reload`. Closing the resource resets it, so reopening reads afresh. Every content store is built on `createContentData`, which is where the guard and the Reload re-read live. A read that lands after another opening is dropped whole — another resource, or the same one closed and reopened, whose id alone cannot tell the two apart. Each opening carries its own token (`getOpening`), replaced when the row read brings a different resource and when the page clears it, but not by a refresh of the one open: `readContent` hands the content over and stamps the store as read only while the opening that issued it is still the open one and no later read of it was issued, and a write issued before an await goes through `getContentWriter`, bound to the opening it was issued under, so the survey store's saves adopt what lands, and build the next save on a pending one, only for that opening, so a late read never shows another opening's document under the contentVersion the blade holds now.
