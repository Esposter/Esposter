...
**A section that scrolls with the page carries `scroll-margin-top`.** It clears whatever sticks to the top of the page — the docs' toolbar on a narrow screen — which an anchored link would otherwise land behind, and the composable reads the same value back as the top of the visible band — one number, declared where it is visible, doing both jobs. A section inside a panel needs none: the panel's own box is the band, and the panel's header sits **outside** its scroll container, so a section clipped above it is genuinely not visible.
