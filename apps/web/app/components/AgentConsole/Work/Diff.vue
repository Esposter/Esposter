...
<template>
  <div font-mono of-x-auto text-body-small>
    <div v-for="(row, index) of rows" :key="index" grid cols-2>
      <div white-space-pre px-2 :style="DiffRowStyleMap[row.type].old">{{ row.oldLine }}</div>
      <div white-space-pre px-2 b-l-1 b-border :style="DiffRowStyleMap[row.type].new">{{ row.newLine }}</div>
    </div>
  </div>
</template>
