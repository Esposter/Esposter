...
interface Props {
  readLook: (out: Vector2) => void;
}

const { readLook } = defineProps<Props>();
...
onBeforeRender(({ delta }) => {
  ...
  const followRatio = reducedMotion.value === "reduce" ? 1 : 1 - Math.exp(-CAMERA_FOLLOW_SHARPNESS * delta);
  eye.copy(playerState.renderPosition).setY(playerState.renderPosition.y + PLAYER_EYE_HEIGHT);
