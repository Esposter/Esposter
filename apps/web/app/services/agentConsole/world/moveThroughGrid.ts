...
// Whether the player's box, its feet at a point, overlaps a solid voxel or reaches past the floor's edge
const checkIsBlocked = (roomGrid: VoxelGrid, { x, y, z }: Vector3, height: number) => {
...
const checkIsSupported = (roomGrid: VoxelGrid, { x, y, z }: Vector3) => {
  ...
  for (let voxelZ = Math.floor(z - PLAYER_HALF_WIDTH); voxelZ < Math.ceil(z + PLAYER_HALF_WIDTH); voxelZ++)
    for (let voxelX = Math.floor(x - PLAYER_HALF_WIDTH); voxelX < Math.ceil(x + PLAYER_HALF_WIDTH); voxelX++)
      if (getVoxel(roomGrid, voxelX, voxelY, voxelZ)) return true;
  return false;
};
...
const moveAlongAxis = (
  ...
  if (isSideways && isHeldAtEdge && !checkIsSupported(roomGrid, position)) {
    position.setComponent(axis, start);
    step.setComponent(axis, 0);
    return;
  } else if (!checkIsBlocked(roomGrid, position, height)) return;
...
// Fall stops on the floor. A blocked axis's step is zeroed, so the caller stops its velocity along that axis. Held at
...
export const moveThroughGrid = (
  ...
  for (const axis of AXIS_ORDER) moveAlongAxis(roomGrid, position, step, axis, height, isHeldAtEdge);
};
