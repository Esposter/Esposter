...
export const getResourceUpdatedRange = (
  ...
  switch (updatedFilter) {
    ...
    case ResourceUpdatedFilter.Last7Days:
      return { updatedAfter: new Date(Temporal.Now.zonedDateTimeISO().subtract({ days: 7 }).epochMilliseconds) };
    ...
    case ResourceUpdatedFilter.Last30Days:
      return { updatedAfter: new Date(Temporal.Now.zonedDateTimeISO().subtract({ days: 30 }).epochMilliseconds) };
    default:
