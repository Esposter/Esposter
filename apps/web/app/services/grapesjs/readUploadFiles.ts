...
export const readUploadFiles = ({ dataTransfer, target }: Pick<DragEvent, "dataTransfer" | "target">) => {
  const files = dataTransfer?.files ?? (target instanceof HTMLInputElement ? target.files : undefined);
  return files ? [...files] : [];
};
