...
export const useCacheStore = defineStore("cache", () => {
  const cacheTagInvalidatorMap = new Map<CacheTag, Set<() => Promisable<void>>>();
  const registerCache = (tags: CacheTag[], invalidate: () => Promisable<void>) => {
    for (const tag of tags) getOrCreate(cacheTagInvalidatorMap, tag, () => new Set()).add(invalidate);
  };
  ...
  const invalidateTags = async (tags: CacheTag[]) => {
    ...
    const invalidators = new Set(tags.flatMap((tag) => [...(cacheTagInvalidatorMap.get(tag) ?? [])]));
