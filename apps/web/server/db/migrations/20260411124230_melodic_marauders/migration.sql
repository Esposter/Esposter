CREATE TABLE "message"."room_categories" (
	...
	CONSTRAINT "name" CHECK (LENGTH("message"."room_categories"."name") BETWEEN 1 AND 100),
...
ALTER TABLE "message"."app_users" ADD CONSTRAINT "name" CHECK (LENGTH("message"."app_users"."name") BETWEEN 1 AND 100);--> statement-breakpoint
ALTER TABLE "surveys" ADD CONSTRAINT "name" CHECK (LENGTH("surveys"."name") BETWEEN 1 AND 100);--> statement-breakpoint
ALTER TABLE "users" ADD CONSTRAINT "name" CHECK (LENGTH("users"."name") BETWEEN 1 AND 100);--> statement-breakpoint
