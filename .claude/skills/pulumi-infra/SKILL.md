...
- **Don't add `aliases` when the Azure name property changed.** A naming-convention rename changes the Azure resource **name property** (often off a legacy portal-GUID name), not just the Pulumi logical name. An alias only remaps the Pulumi URN, so it cannot prevent the replacement — under the new name it is genuinely a different Azure resource. Those renames are applied with the create/cutover/delete sequences above; once applied, the rename is final and needs no follow-up migration option. Note also that the previous Pulumi logical name is the old Azure resource name (the constructor's first argument), never the old export constant.
  - **A logical-only rename is the opposite case** — the Azure name property is unchanged and only the Pulumi resource/variable name moved. There an alias is the correct tool: it preserves the URN so Pulumi adopts the existing resource instead of replacing it. Don't apply the prohibition above to those.
- **Read the preview before calling an alias suggestion a false positive.** CodeRabbit raises this alias suggestion repeatedly (it reasons from the rename diff and re-flags it on every PR that carries the renamed file, often as a "🔴 Critical … protected resource replacement" outside-diff comment). Don't reason about whether `protect: true` blocks a replacement from the diff alone — **run `pnpm infra:preview` and read the plan.** Close the comment only when the Azure name property changed **and** the preview shows the migration already applied (`unchanged` / `0 to replace`); that preview line is the evidence to paste. If the preview still shows a pending replacement, or the rename was logical-only, the suggestion is live — address it rather than closing it. Claiming a replacement will or won't happen without a preview to back it is itself a false positive.
