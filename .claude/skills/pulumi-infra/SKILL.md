...
- Put declarations under `src/resources/<ProviderNamespace>/<resourceTypes>/` mirroring Azure resource IDs after `/providers/`, e.g. `Microsoft.Web/sites`, `Microsoft.Insights/actionGroups`, `Microsoft.EventGrid/eventSubscriptions`.
...
- Durable infrastructure docs live in `packages/infra/docs/`. Ordered roadmaps in `features/infra/v*.md`; completed roadmaps move to `features/infra/completed/`.
- `docs/naming-conventions.md` — Azure naming rules.
- `docs/overview.md` — package overview and imported resource inventory.
- `features/infra/optimization-review.md` — phase-2 cost/security findings.
- `docs/security-constraints.md` — security hardening blockers and app code references.
- `docs/stacks.md` — stack policy.
