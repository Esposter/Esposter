...
- **Never add `aliases` to a renamed resource.** A naming-convention rename changes the Azure resource **name property** (often off a legacy portal-GUID name), not just the Pulumi logical name. An alias only remaps the Pulumi URN, so it cannot prevent the replacement — under the new name it is genuinely a different Azure resource. Renames are applied with the create/cutover/delete sequences above and are final once applied; a rename already in the codebase needs no follow-up migration option. A review suggestion to add `aliases: [{ name: "<old>" }]` to a renamed resource is **wrong** and should be closed, not applied — note also that the previous Pulumi logical name is the old Azure resource name (the constructor's first argument), never the old export constant.
  - **CodeRabbit raises this alias suggestion repeatedly** (it reasons from the rename diff and re-flags it on every PR that carries the renamed file, often as a "🔴 Critical … protected resource replacement" outside-diff comment). It is **always closed, never applied.** Do not reason about whether `protect: true` blocks a replacement from the diff alone — **run `pnpm infra:preview` and read the plan.** A rename that is already applied shows as `unchanged` / `0 to replace`; that preview line is the evidence to paste when closing the comment. Claiming a replacement will happen without a preview to back it is itself a false positive.
