...
3. **Hook registry** — only when one typed action/event must fan out to side effects owned by _unrelated_ stores (`AdminActionHookMap`/`MessageHookMap`; `topRoleChangeHooks` lets the member store keep its per-role group counts current on every role mutation without the role store importing it). Hooks are for decoupling cross-store side effects, not for keeping shared data in sync — reaching for them where a shared store map suffices adds indirection without fixing ownership.
...
- Keyed variants are plain objects/Records of registries (`MessageHookMap[Operation.Create].register(...)`).
