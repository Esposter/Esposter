...
- **Dependency placement.** `@codspeed/vitest-plugin` is a **`peerDependency` of `configuration`** — `configuration` directly imports it in `getBenchmarkPlugins` (part of its published dist) and externalizes it, and the critical rule is **external imports are `peerDependencies`, never `dependencies`/`devDependencies`** (see [Build › external = peerDependency](../build/SKILL.md)). It **must stay in the shared `external` list** (`/^@codspeed\//u`) — bundling it breaks the plugin, which loads sibling runtime files (`globalSetup`, the mode runners) and its native prebuilds via `__dirname` (a bundled build fails with `Failed to load …/configuration/dist/globalSetup.mjs`). So: external + `peerDependency`, never bundled, never a `dependency`.
- **Every package that runs `vitest` must also declare codspeed as a `devDependency` — not just the bench runners.** This is the subtle, easy-to-miss part: `getBenchmarkPlugins.ts` has a **static top-level `import codspeedPlugin from "@codspeed/vitest-plugin"`**, and `getVitestConfiguration` **unconditionally** calls `getBenchmarkPlugins()`. The `CODSPEED_ENV` gate only blocks plugin _invocation_, not the _import_ — so the codspeed module is loaded whenever a vitest config is evaluated, i.e. on every plain `pnpm test`/`pnpm bench` in every package whose `vitest.config` imports `getVitestConfiguration` (all of them; `app` imports `getBenchmarkPlugins` directly). Each therefore declares `@codspeed/vitest-plugin: "catalog:"` as a `devDependency`, exactly the way each package already declares its own `vitest` devDep — satisfying `configuration`'s peer explicitly instead of leaning on `auto-install-peers`. This was the reason codspeed was originally a `configuration` `dependency` (always resolvable for every consumer); the peer move trades that implicit resolution for the explicit per-package devDep. If you ever make the codspeed import dynamic (`await import` gated behind `CODSPEED_ENV`), this cascade can collapse back to just the bench runners — until then, keep it on every vitest-running package.
