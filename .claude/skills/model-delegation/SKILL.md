...
description: Esposter model-delegation conventions — the main session (Fable) does all thinking (specs, proposals, architecture, review); mechanical implementation is delegated to Opus 4.8 background subagents with self-contained prompts. Apply when deciding whether to implement in-session or delegate, and when writing a delegation prompt.
---

# Model Delegation — Fable Thinks, Opus Implements

Token budgets are the constraint: the main session's model (Fable) is expensive and its context is where design quality lives. Spend it on thinking; delegate execution.

## Division of labor

- **Fable (main session)**: specs, proposals, architecture decisions, triage, naming, docs conventions, reviewing agent output. Anything where judgment compounds.
- **Opus 4.8 (subagent)**: executing an already-written spec — renames, sweeps, migrations, mechanical refactors, well-scoped feature implementation. Launch via the Agent tool: `subagent_type: "general-purpose"`, `model: "opus"`, run in background so the main session keeps working.
