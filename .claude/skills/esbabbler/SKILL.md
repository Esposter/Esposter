...
`MessageTypeOperationPermissionMap` (`shared/services/message/`) is the **single** source of truth for what may be done to a message. It answers two questions that must not be collapsed:
...
The two produce **different failure modes**, and conflating them is what let a client offer actions the server refused:
...
- `getMessageProcedure(schema, operation)` names the operation it guards. Never hardcode a type check in a procedure.
