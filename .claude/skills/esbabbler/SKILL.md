...
- **Mutation through the reactive chain** — obtain a participant via `callSessionParticipantsMap.value.get(callSessionId)?.get(sessionId)`. Storing the result in a local variable (`const participant = ...get(id)`) is fine — the object itself is Vue-proxied, so `participant.isMuted = true` still triggers reactivity. The restriction is against capturing a _stale reference_ before the reactive lookup (e.g., closing over the map itself in a non-reactive context).
