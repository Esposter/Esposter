...
- **`v-html` only on a plain element** — on a component (`<v-card-text v-html="html" />`) the html silently never renders: it compiles to an `innerHTML` prop that the component's own children patch drops, so the element comes out empty in SSR and on the client with no warning. Wrap instead: `<v-card-text><div class="rich-text-content" v-html="html" /></v-card-text>`.
