...
- **Static slot names** — use static slot syntax (`#item.drag`, `#item.actions`) instead of dynamic template literals (`#[`item.drag`]`) when the slot name is a known constant. Only use dynamic bindings (`#[`item.${column.name}`]`) when the slot name is genuinely runtime-dynamic.
