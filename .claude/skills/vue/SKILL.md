...
- **No `current` prefix** — reactive refs/computeds are always the current value by definition; the prefix is redundant. Write `userId` not `currentUserId`, `voteOptionId` not `currentVoteOptionId`.
