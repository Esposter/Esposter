...
- **Computed for reused expressions** — extract a `computed` (named to match the prop) when the same derived value binds to 2+ props; single-use values stay inline to leverage Vue inference. Three things are not single-use findings, and each has bitten a pass that read the use count alone: a **sort**, which stays at one use under the rule above; a **map lookup**, where the finding is the computed wrapping it rather than its use count (the rule below); and a **getter called per `v-for` item**, where inlining calls it once per row instead of once per render — leave it, or hoist it if the row count is unbounded. A computed memoizing a genuinely expensive derivation also stays. Past the use count, the test is whether the expression reads cleanly at the use site: inline it when it does — most valuable when the computed carried an explicit type annotation the binding target would infer anyway — and keep it when it has a statement body, is writable, is consumed as a `Ref` (passed to a composable, `storeToRefs`-style destructuring), or its name is the only thing explaining a non-obvious expression.
