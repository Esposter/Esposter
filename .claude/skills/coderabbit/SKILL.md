...
When a push overshoots the cap the review is skipped outright ("Review skipped: N files exceed the limit of 100"), and the fix is always the same: **shorten `develop`, park the remainder on a queue branch, and drain it one window at a time** — `references/release-pr-cutting.md`. That applies to a PR too big for its first review _and_ to an already-reviewed PR whose incremental cycle overshot, which is the ordinary way it happens: local commits accumulate past ~80 files between pushes and the next push is over. Cutting is cheap and the queue branch loses nothing, so cut as soon as the skip appears rather than pushing more on top.
