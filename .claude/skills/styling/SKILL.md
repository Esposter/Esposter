---
name: styling
description: Esposter UnoCSS Attributify Mode styling conventions — prop-based attributes for all static styles, class only for scoped CSS refs / dynamic bindings / third-party selectors, theme primitives and theme colours over bespoke values, MD3 typography and semantic opacity, text-info links, state variants (hover:/focus-within:) instead of scoped &:hover blocks, slash/fraction utilities in valued attributify, abbreviated utilities (op-, b-, rd-) with the explicit b-solid rule and the width-only global border reset, named over numeric utilities, gap directionality, sentence-like rows staying in inline flow, the parent owning spacing (gap/padding over child margins), absolute positioning within a container, rem over px, and style-block rules, plus deep dives on page/panel/sidebar/region layout with border ownership and on arbitrary bracket values (calc, CSS variables, transitions, !important). Apply when writing or reviewing styles in .vue or .scss files, or laying out a page, panel, sidebar, or border.
---
...
- **Widths are zeroed globally; never restore preset-wind4's `border-style: solid` alongside it.** `uno.config.ts` disables that preset's reset wholesale so it cannot fight Vuetify, and `globals.scss` restores **only** `border-width: 0` (in the `uno-base` layer, so Vuetify's own borders and every `b-*` utility still override it). Both halves of that are load-bearing:
  - Without the zeroed width, `border-width` keeps its CSS initial `medium` (~3px), so an element carrying `b-solid` paints a 3px border on every side it gives no explicit width — a stray hairline frame that reads as unexplained padding. Partial sets are the trap: `b-t-1 b-solid` borders the other three sides too.
  - Adding `border-style: solid` back would make **every** element that takes a width from a later layer without declaring a style of its own suddenly render one — Vuetify's loaders and skeletons among them. It looks like a tidy way to delete `b-solid` repo-wide; it is a visual regression across the component library. Tried and reverted.
