...
- **`width` / `height` are html attributes, not styles.** They feed the image optimizer, so they take bare numbers; a percentage or a rem is silently dropped and the image falls back to its intrinsic size. All sizing is CSS utilities (`w-full`, `max-w-180`, `size-8`), and a computed dimension goes through `:style` on the wrapper.
