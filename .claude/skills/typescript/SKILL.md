...
- **Never `await import(...)`** for code-splitting — always static top-level `import`. Components are already chunk-split per component by the build, so a nested dynamic import only hides the dependency and (in dev) defers Vite discovery. If a heavy dep must be pre-bundled, add it to `optimizeDeps.include` in `packages/app/configuration/vite.ts` instead.
