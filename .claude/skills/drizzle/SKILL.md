...
- **Token-as-PK convention** — when a table has a generated random token that already uniquely identifies the row (e.g. invite tokens, call session tokens), use that token as `id: text().primaryKey()` directly. Do **not** add a separate `uuid` surrogate `id` alongside a `token` column — that wastes bytes and duplicates the identity. The `id` field name is always used (never `token`) so the shape is consistent with all other entities.

  ```typescript
  // CORRECT — token IS the id
  ...
  // WRONG — redundant uuid surrogate alongside a token
  export const invitesInMessage = pgTable("invites", {
    id: uuid().primaryKey().defaultRandom(), // ❌ wasted bytes
    token: text().notNull().unique(),        // ❌ the real identity
    ...
  }, ...);
  ```
