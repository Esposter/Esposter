...
- **`.resolves` is banned** (`no-restricted-syntax`) — `await` the promise into a `const`, then assert: `const count = await caller.count(); expect(count).toBe(x);`. `.rejects.` is unaffected — it stays the only accepted async **error** assertion (see **Error Assertions**).
