...
- **Client-side tRPC calls have no sanctioned fake yet** — so a store or composable test cannot currently drive one. The nuxt env applies the real plugin, whose client issues HTTP nothing answers. Test such a unit through a seam that stops short of the call (`useDataStore`'s Create hook, a service module the composable calls), or leave it to the server-side router tests, which use `createCallerFactory` and need no client at all. Faking it properly wants a network-layer interceptor (msw + `msw-trpc`), not module mocking — until that exists, don't invent a per-file client stub.
