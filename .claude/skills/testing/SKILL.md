...
- **Nothing but functions and hoisted mocks lives at module scope.** Every constant — a literal, a fixture object, an entity built by a factory — is a `const` **inside the `describe` callback**, so it dies with the block instead of being retained for the file's lifetime and reachable from an unrelated `describe`. Module scope is for `import`s, pure helper functions (which hold no state), and the `vi.hoisted` block, which `vi.mock` lifts above the imports and therefore _cannot_ move inward. A constant shared by sibling `describe`s is declared in each — duplicating two lines beats a file-scope binding every block can reach.
