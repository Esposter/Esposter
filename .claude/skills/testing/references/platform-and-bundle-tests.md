...
- **What makes a bundle differ across OSes is the minifier, not line endings.** A bundle large enough to exhaust single-character identifiers hands the two-character names out in module order, and Windows and POSIX resolve that order differently — a couple of bytes, deterministic on each side. Line endings are not the cause: the emitted bundle is LF on both. So the split is a property of size, and only the biggest package needs it; the types bundle beside it usually stays byte-identical and must not be split along with it.
