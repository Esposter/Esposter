...
- **Co-locate single-use event/hook types** — event emitter map types (e.g. `AdminActionHookMap`, `MessageHookMap`) that are used in only one service file may be defined inline in that service rather than extracted to `models/`. Only applies to event/hook maps; do **not** co-locate general type maps (`ColumnTypeColumnMap`, `DataSourceConfigurationTypeMap`, etc.) — those stay in `models/` regardless of consumer count.
