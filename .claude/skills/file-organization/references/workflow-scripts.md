# `.claude/workflows` scripts, their tests, and repo-wide globs

Read when adding or editing a `.claude/workflows/*.js` script or its tests, or when writing any repo-wide glob (Vitest project, tsconfig include/exclude, lint ignore) that reaches into `.claude/`.
...
`.claude/workflows/<name>/` holds `models/`, one helper function per file, `constants.test.ts`, and suites split by phase — every normal rule applies.
...
- **ESLint skips them** via a literal `ignores: [".claude/workflows/*.js"]` in the root `eslint.config.js`, because its parser rejects the top-level `return` the sandbox requires.
...
Agent tools check a full parallel copy of the monorepo out at `.claude/worktrees/<name>/`, so an unbounded glob traverses the whole repo a second time per live worktree and reports at paths belonging to another branch.
