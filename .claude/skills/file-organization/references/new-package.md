...
**Example — `packages/db-mock`**, a test-only node package: `@electric-sql/pglite` is a peer (heavy, not bundled, loaded at runtime by `createMockDb`) and is in the shared `external` list; `drizzle-kit` is a `devDependency` only, used by `scripts/generateSnapshot.ts` (regenerates the committed `src/snapshot.tar.gz` via `pnpm snapshot:gen`) and the verification test, not the shipped `createMockDb` runtime; `eslint.config.js` symlinks to `../configuration/eslint/index.typescript.js`.
