...
| `services/{vuetify,styled,entity,zod,ajv,jsonSchema,dayjs,compiler,shared,azure,cache,file}` + the matching `composables` | 2026-08-15 | `app/services/compiler` is not the swept `shared/services/compiler`; `getFilePickerTypes` and `EMPTY_FILE_MESSAGE` each own their one copy |
