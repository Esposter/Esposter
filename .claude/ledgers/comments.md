...
Excluded: `app/configuration/plugins/fixAjv.ts` + test (numbered step list is a deliberate reference) · `app/shared/types/nuxt.d.ts`, `app/app/types/desmos.d.ts`, `configuration/types/global.d.ts`, `vue-phaserjs/auto-imports.d.ts` (vendored/generated) · `app/util/math/random/getRandomValues.ts`, `db/src/services/azure/table/getTableNullClause.ts` (source-URL references) · `*/rolldown.config.ts`, `@ts-expect-error` / `oxlint-disable` lines (directives).
