...
5. **Commit the coherent chunk, then `git pull --rebase`.** The collector rewrites `origin/ai/queue` behind every window, so the local branch goes stale the moment one ports; a rebase refuses a dirty tree, so the pull follows the commit rather than sitting in front of it, replays only what the session committed since, and a plain `git push` follows — never a bare `--force-with-lease` (`review-queue`). Never push unless asked — but once asked, step 4 does not stand in the way: a review slot costs an hour and the checks cost minutes, so the push goes out and the checks run against the same tree beside it (`coderabbit`). Anything they turn up is a commit in the next window.
