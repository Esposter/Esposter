...
5. **`git pull --rebase`, then commit** the coherent chunk. The collector rewrites `origin/ai/queue` after every window it ports, so a local branch is stale the moment one goes out; the pull replays only what the session committed since, and a plain `git push` follows — never a bare `--force-with-lease` (`review-queue`). Never push unless asked — but once asked, step 4 does not stand in the way: a review slot costs an hour and the checks cost minutes, so the push goes out and the checks run against the same tree beside it (`coderabbit`). Anything they turn up is a commit in the next window.
