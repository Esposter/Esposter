...
- **Is every state designed?** Empty, loading, error, hover, focus, pressed, disabled, narrow and signed out each have a look of their own. A state that is only the absence of another is a missing state — the commonest being a list that shows its empty state while its read is still out: it is skeletons until a read settles, and empty only after one settles with nothing (`SKILL.md`, "Components").
