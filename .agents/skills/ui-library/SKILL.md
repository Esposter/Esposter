...
- **A confirmation before something destructive is a `UiConfirmDialog`.** An optimistic write calls `onComplete()` before it is awaited, so the dialog closes with the change; only a write that waits for the server holds the answer pending (`apps/web/content/docs/architecture/destructive-confirmation.md`).
