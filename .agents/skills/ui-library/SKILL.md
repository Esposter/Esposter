...
- **A search is `UiTextField` with the search type**: a pill with a search mark, its label as the hint inside it and still its accessible name, and its own clear button once it holds text, so a feature never draws one beside it. A surface rule draws and never lays out: `ui-sunk` carries no padding, which the call site gives. Any other field — a text field, a select's trigger, a feature's own textarea — wears `ui-field` over `ui-sunk`, the resting line a focused field's indicator thickens; a bare `ui-sunk` is for a track, a chip or a key cap.
