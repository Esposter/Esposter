...
- **A grid with one tab stop takes its keys from `useGridKeyboard`**: the grid maps a key to its next cell and the composable moves the stop there and focuses it once drawn, so the date grid and the event calendar's views walk alike and none keeps a keydown handler of its own.
