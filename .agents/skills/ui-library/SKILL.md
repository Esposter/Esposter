...
- **A search is `UiTextField` with the search type**: a pill with a search mark, its label as the hint inside it and still its accessible name, and its own clear button once it holds text, so a feature never draws one beside it. A surface rule draws and never lays out: `ui-sunk` carries no padding, which the call site gives. A field draws no line and no ring: `ui-sunk` tints it while focused, and the text field while invalid, so a feature adds neither.
