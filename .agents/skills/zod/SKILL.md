...
- **`.default()`** — never combine `.optional().default(value)` (`.default()` already handles `undefined`). Only use `.default()` in schemas whose TS type is a **class with actual property defaults** (e.g. `class Foo { bar = [] }`). Never add `.default()` to a schema that `satisfies z.ZodType<Interface>` — interfaces have no defaults, so schema and type would misalign. Initialise empties explicitly at the call site (`new MyClass()` or `{ steps: [] }`). **The exception is a field a Vjsf form prefills, and its default stays on the shared schema** — Vjsf reads the emitted JSON schema's `default`, and the `*Form` schema layers presentation through `.safeExtend`, which only accepts a replacement of the same schema type and rejects a `ZodDefault` in place of a `ZodString` (`Type 'ZodDefault<ZodString>' is not assignable to type 'never'`). So the default cannot be moved to the form side without abandoning `safeExtend` and restating the field — don't try it (`vjsf` skill).
