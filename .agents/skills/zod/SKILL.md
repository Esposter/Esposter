...
- **An object key naming another schema's field is inherited, never retyped.** Which form depends on the position, and the three differ in what the compiler actually checks — each verified by mistyping the key and running typecheck:
  - **Default — spread it**: `...schema.shape`, or `...schema.pick({ field: true }).shape` for a subset. The key comes from the source, and a wrong one fails the pick (`TS2561`). Reach for this whenever the field carries over unchanged.
  - **The field is modified** (`.optional()`, `.default()`, `.unwrap()`, `.meta()`, a chained constraint) so it cannot be spread → `[schema.keyof().enum.field]: schema.shape.field.optional()`. The computed key is what holds the key equal to the source field name; a literal key beside a modified value keeps compiling after the two have drifted, and the wire contract silently keeps the old spelling.
  - **`.omit()` / `.pick()` on a generic `z.ZodObject`** — a `schema: z.ZodObject` parameter, as a resolver takes — checks **nothing**: `schema.omit({ typo: true })` type-checks clean and omits nothing at all. The computed key is the only guard that exists there.
  - A key that **deliberately differs** from the source field (`targetUserId: selectUserSchema.shape.id`) stays a literal — it is not mirroring a name, so there is nothing to inherit.
