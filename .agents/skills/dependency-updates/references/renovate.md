...
- **Holds** — `h3`, `@platformatic/vfs` and the unocss trio by `allowedVersions`; `typescript` and `typescript-native-bridge` by `enabled: false`. The reasons are the descriptions, and `references/holding-a-dependency.md` says which catalog range pairs with each.
