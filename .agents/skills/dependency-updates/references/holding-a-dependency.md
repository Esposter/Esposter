...
- **A cap at a major needs only the rule** — the caret already stops a re-resolve. `h3` keeps its caret in the catalog and `overrides:`, and its rule's `allowedVersions` names the next major as the ceiling; `@platformatic/vfs` is the same shape on a 0.x line, where the caret already stops at the next minor and the rule names it; only minor/patch within the major.
