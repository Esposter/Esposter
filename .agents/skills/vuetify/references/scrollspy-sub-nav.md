...
**The scrollspy itself is not yours to write.** `useVisibleSectionIds` and `StyledSlideIndicator` are the two pieces; the mechanism, its guarantees and the instruments it bans (`v-intersect`, scroll handlers, a click-tracked "active section" in a store) are owned by `apps/web/content/docs/architecture/section-navigation.md`. This page is only the Vuetify wiring around it.
