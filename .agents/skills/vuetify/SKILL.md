...
`hideDetails: "auto"` renders the details row exactly when there is a message for it; a bare `hide-details` means `true` and **swallows the validation error a field with rules exists to report**, while reserving no less space. Both the static and the bound form are ESLint errors (`vue/no-restricted-static-attribute`, `vue/no-restricted-syntax`); a layout that genuinely cannot afford the row carries a disable comment stating why.
...
- **A built-in alias first, always** — never reimplement one or its message; a custom alias is earned only where no built-in covers the check, named and worded in Vuetify's own voice as a literal (`references/form-dialogs-and-rules.md`). The `required` HTML attribute is not a Vuetify prop — use `:rules="[rules.required()]"`.
