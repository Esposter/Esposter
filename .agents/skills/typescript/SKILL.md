...
- **Guard clauses first, one guard per outcome, and a chain runs `if/else if/else` from its first branch to its last** — a balanced `if/else` stays balanced, and no negated test sits before a terminal `else` (`no-negated-condition`). A guard whose next statement is the fall-through `return` is the enforced half (`no-restricted-syntax` in `packages/configuration/eslint/restrictedSourceSyntaxes.js`); whether consecutive guards are one chain is read. Writing or reshaping a branch: `references/control-flow.md`.
