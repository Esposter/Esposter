...
- **Prefer non-mutating array methods.** `sort()`/`reverse()` are gone outright (`unicorn/no-array-sort`, `unicorn/no-array-reverse`, restated in `vue/no-restricted-syntax` for template expressions oxlint does not read), and splicing a fresh copy is a `no-restricted-syntax` error — `splice()` stays legal only for **in-place** mutation of a store or reactive array, which is all it is for. What is left to judgement is `arr.with(index, value)` over `[...arr.slice(0, i), value, ...arr.slice(i + 1)]`.
