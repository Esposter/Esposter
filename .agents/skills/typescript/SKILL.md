...
- `.forEach()` is **BANNED** — use `for...of` (see Loops); `unicorn/no-array-for-each` enforces it.
- `type` aliases for object shapes → `interface` (`consistent-type-definitions`).
- **Prefer non-mutating array methods** — `unicorn/no-array-sort` and `unicorn/no-array-reverse` take `sort()`/`reverse()`, so the two that are left to judgement are `arr.with(index, value)` over `[...arr.slice(0, i), value, ...arr.slice(i + 1)]`, and `toSpliced()` whenever the result is the array wanted. `splice()` stays legal for **in-place** mutation of a store or reactive array, which is the only thing it is for: a copy-then-`splice` is always `toSpliced`.
