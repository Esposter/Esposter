...
- **Guard clauses first, one guard per outcome, and a chain runs `if/else if/else` from its first branch to its last** — a balanced `if/else` stays balanced, and no negated test sits before a terminal `else` (`no-negated-condition`). A one-line guard whose next statement is a one-line `return` closes with `else`; a branch that spans lines stays a guard ahead of the happy path, and whether consecutive guards are one chain is read. Writing or reshaping a branch: `references/control-flow.md`.
