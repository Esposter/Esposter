...
- **Default option objects** are constants: export one shared `DEFAULT_*` object from the feature's `services/.../constants.ts` and reuse it everywhere, wrapped in `Object.freeze({ ... } satisfies InterfaceName)` so callers can't mutate the shared default. The freeze is shallow, so a default holding an array or a nested object freezes that value too — otherwise one caller's push is every later caller's default.
