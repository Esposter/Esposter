...
- **An `interface` or `type` lives in its own file under `models/`, never beside the code that reads it** — a service, composable, store or constant file declares no type of its own; an options bag, a return shape or a resource contract is a model file the reader imports, so the next reader finds it where every other shape is. The only local declarations are an SFC's `Props` (the `vue` skill) and a test file's fixture shapes, and those sit together at the top of the block after the imports (and macros), before the runtime `const`/logic — never interleaved between logic blocks.
