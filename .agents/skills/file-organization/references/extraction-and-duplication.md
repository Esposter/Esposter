...
The test is what the caller no longer has to state, never how short the call reads. A helper that still takes every element hand-written at every call site — `getFooAll(a, b, c)` over an inline `[a, b, c]` loop — has moved a loop and absorbed no decision: forgetting one is exactly as easy after. A wrapper carrying no logic, no invariant and no default is a rename with an import, so delete it and leave the inline form. Extraction that pays: the caller passes less than it did, or passes it in a shape that cannot be wrong.
