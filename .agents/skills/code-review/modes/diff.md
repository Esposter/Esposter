...
```javascript
Workflow({ scriptPath: "<repo>/.agents/workflows/code-review.js", args: "<level> [target]" });
Workflow({ scriptPath: "<repo>/.agents/workflows/code-review.js", args: "diff <level> [target]" });
```
...
**`.agents/` is never one of them**, however tooling-shaped the window looks. The review workflow's own script, suite and skill tree are edited nearly every round, and reviewing them is how the pipeline compounds instead of drifting — `references/self-improvement.md`.
