...
- **The fix lives in this skill tree, which no test can pin.** A review instruction has no suite, so the only thing that stops it drifting is the meta pass (`references/meta-pass.md`) and the next round reading it. State the rule with the evidence that produced it, so a later reader can tell a lesson from a preference.
...
- **Added a step ahead of a loop that addresses `HEAD` by shape.** A merge folded in before a retry that drops `HEAD~1` changes what `~1` names — the merge's first parent is the commit the loop meant to keep, so the drop removes the fold and the loop's count runs one behind the tree, with every test green because none ran on a merged head. A relative ref is a claim about the shape of history at that line; a step inserted upstream of it rewrites the claim silently. Order the new step after the loop, or make the loop address the sha it built rather than a distance from the tip.
