...
- **A deployed identity is renamed like any other identifier.** An Azure resource name, a function name, or any string a resource's properties point at (an event subscription's `destination` naming a function) is corrected in place the moment it is wrong — infra being code is what makes that ordinary rather than a migration (`apps/web/content/docs/architecture/no-compatibility-debt.md`). Rename, `pnpm infra:preview`, read the plan. The one exception is a resource holding data — a storage account, a search index — whose rename is a replacement that would drop the data, so it goes through the two-phase create/cutover/delete under Renames above. Hesitating on an unpreviewed guess about what a rename would cost is the same false positive as asserting a replacement without one, and it is the more expensive mistake: it leaves the wrong name in place permanently.
