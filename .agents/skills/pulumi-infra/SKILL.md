...
- **Tying `WEBSITE_NODE_DEFAULT_VERSION` to `.node-version`.** The Functions runtime supports fewer majors than the repo runs on, so the setting stays on the major Functions supports and moves by hand when that support lands; `pnpm update:node` deliberately leaves it alone.
...
One private package, one `prod` stack, `src/` split by provider with a generated barrel, `dist/index.js` as the entrypoint, and no Pulumi operation without the user's say (`references/package-shape.md`).
...
- **A deployed identity is renamed like any other identifier** — rename, preview, read the plan (`references/migrations.md`).
...
The one exception to named imports from libraries, and a review suggestion to switch is closed: why is `references/provider-quirks.md`.
...
- `references/package-shape.md` — when adding a provider folder or a non-resource stack output, or touching the barrel or the entrypoint.
