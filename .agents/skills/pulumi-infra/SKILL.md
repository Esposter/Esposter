...
- `src/azure/outputs/` holds a stack output that is not a resource — a value read off a resource for a person to copy once (`prodSpchEsposter001Key`), one per file, wrapped in `pulumi.secret` when it is a credential. A resource is already a stack output through the barrel, so a file lands here only for a value the resource's own outputs do not carry.
