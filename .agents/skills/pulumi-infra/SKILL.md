...
- `src/` is split by provider: `src/azure/{resources,constants,services,outputs}` and `src/github/…`. The generated `src/index.ts` barrel stays at the top level and covers every provider.
- `src/azure/outputs/` is where a stack output that is not a resource goes — a value read off a resource for a person to copy once, such as an account key listed through a `list<Resource>KeysOutput` call, one per file, wrapped in `pulumi.secret` when it is a credential. A resource is already a stack output through the barrel, so a file lands there only for a value the resource's own outputs do not carry; the folder holds none today.
