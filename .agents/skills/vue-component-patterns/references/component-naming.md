...
- a file sits **beside a folder whose name is its prefix** (`Foo/` + `FooBarBackground.vue`) — always untidy, fold regardless of size. This half needs no judgement, so it is a test: `apps/web/app/components/index.test.ts` fails on the stray and names the folder it belongs in
