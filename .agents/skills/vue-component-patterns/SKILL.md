...
**A `:key` names the thing being rendered, never a counter something bumps.** `:key="reloadCount"` on a mounted subtree is a manual refresh wearing a reactive disguise: nothing about the key says what changed, every caller that changes the data has to remember to bump it, and the remount throws away scroll position, focus and every other piece of local state to re-fetch data the surface could simply be handed. When data changes underneath a surface that is already mounted, the owner of that change **pushes** it — a subscription handler, or a hook registry (`services/shared/createHookRegistry.ts`) the holding stores register into, keyed by whatever decides which of them the change is for.
