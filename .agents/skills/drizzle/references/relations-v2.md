...
- **`r.many` → the junction/child table's own name**: `foosToBars`, `bansInMessage`. The table name is already the plural, so nothing is pluralised again — `bansInMessages` reads as bans across messages, and it is the `r.many.bansInMessage` accessor beside it that the key mirrors.
