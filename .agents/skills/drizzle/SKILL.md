...
- **A column holding another table's id gets `.references()`** — the constraint is what makes the impossible state unrepresentable, so it is the default rather than a decision. Pick the `onDelete` the domain means (`cascade` where the row is meaningless without its parent), and never `restrict` on a parent something outside this repo deletes, because that turns their delete into a failure. A referencing column that a pre-existing row cannot fill is settled in the migration — clear those rows, or give the column an empty sentinel — not by leaving the reference off.
