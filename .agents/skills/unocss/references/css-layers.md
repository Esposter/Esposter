...
- `properties` → `null` — CSS custom property declarations must not be wrapped in a `@layer`: a layered declaration loses to every unlayered one whatever its specificity, so no selector change wins it back
