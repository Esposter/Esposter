...
The guard skips generation when the **file list** is unchanged, hashed into `node_modules/.cache/`. Contents never matter — every barrel here is `export * from` and names no symbol — which is the whole point, since a content hash would miss on every keystroke. It cannot see a file that exported nothing and now exports something; `export:gen` is the answer to that.
