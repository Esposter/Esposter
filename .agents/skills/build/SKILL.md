...
- **Dropping `composite` from `tsconfig.library.json`**, on the reading that nothing consumes it — only the app has `references`, and Nuxt generates those. `isolatedDeclarations` requires it: without `composite` or `declaration`, every package fails `TS5069` before it checks a line. It is what keeps the declaration emit on the per-file path (`references/tsconfig-presets.md`).
