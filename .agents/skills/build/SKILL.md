...
`getTsdownConfiguration` runs ctix from a `build:prepare` hook, so no manifest repeats the command. What a package generates is the `exportsGeneration` option — `"typescript"` by default, `"vue"` for the package that ships components, `"none"` for one that names its own entrypoints and would otherwise get a barrel nothing is an entry for. `export:gen` stays a script for regenerating a barrel without a build.
