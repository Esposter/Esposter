...
The guard skips generation when the **file list, each file's export bit, and the generator** are unchanged, hashed into `node_modules/.cache/`. What a file exports never matters — every barrel here is `export * from` and names no symbol — which is the whole point, since a content hash would miss on every keystroke. _Whether_ it exports does: ctix omits an export-less file entirely, so a file gaining its first export needs a barrel line on a source list that did not change, and that is the one transition where a stale barrel builds green with the new API simply absent. The generator is hashed for the same reason — ctix's own cli, the `.ctirc-*` configs it is handed, and every tsconfig beside the package and beside `configuration`, because the same file list run through a different generator is a different barrel. The tsconfigs are in it because `project` points ctix at one and its resolved `include`/`exclude` decide which files ctix ever sees: widening a shared preset's exclude list changes the barrel while the source list and every export bit stay put. Whole directories rather than the resolved `extends` chain — hashing a file the barrel does not depend on costs one regeneration, missing one ships a barrel that does not match its source. The list excludes **only the barrels ctix writes**, named exactly rather than matched as any `index.ts` — a nested one is a hand-written directory barrel the root barrel carries its own `export * from "./<directory>/index"` line for, so excluding it by shape hides the addition that most needs to regenerate. `export:gen` regenerates unconditionally when a guard is not wanted at all.
