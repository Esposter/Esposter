...
- **A hand-maintained `onlyBundle` allowlist**, to answer tsdown's standing hint. `inlinedDependencies` already records every vendored package in a reviewed diff, and the check runs before the manifest is written, so a new package's first build could never pass ("What gets vendored is recorded, not allowlisted").
