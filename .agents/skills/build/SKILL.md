...
- **A guard that skips barrel generation on a cheap hash.** ctix drops _both_ files when two export the same identifier, so a colliding rename changes the output while moving no path and flipping no export bit (`references/barrels.md`).
