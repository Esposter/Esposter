...
description: Apply when writing or reviewing a composable, a form dialog, browser-aware reactive code, or any state that spans an await, a tick or a mount. Esposter Vue 3 composable patterns — the table of primitives that already own a job and the ban on hand-rolling them (useMutation for ordering reads and writes, useCachedRead().supersede, useSave, a mutation's own isPending, useWorkerInterval, usePanZoom, getOrCreate), a hand-kept count of in-flight anything as the tell, no pass-through composables, minimal public surface, createSharedComposable and module-scope refs banned, single-function composables returning the function, inferred return types, composables called at setup, MaybeRefOrGetter vs plain args, the three validation-rule layers, extracting duplicate mutation blocks, toRawDeep over toRaw, and the least-API-calls rule — plus deep dives on resource cleanup, observing the browser, sequencing and useSave mechanics, form-dialog wiring, and composable lifecycle.
---
