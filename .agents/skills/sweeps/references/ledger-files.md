...
- **The index row** (`.agents/ledgers/README.md`) — the ledger, its rules owner, what one unit is, and the `Scope` pathspecs its domain covers. This is the sweep's whole metadata; there is no second metadata block below it, and a scope in particular never moves into the ledger file (why is `references/standing-resume.md`). What granularity the coverage table is dated at is not carried beside the unit either: it restates the unit in other words, and the promoted ones are the folder link.
