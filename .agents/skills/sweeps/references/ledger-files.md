...
- **Coverage** — a table, one row per unit, ordered by expected payoff: `| Unit | Swept | Notes |`. `Swept` is the date the pass landed and `—` while it is open, so the row carries when as well as whether; `Notes` scopes the unit — a boundary, a carve-out, a warning the next pass needs — and states it in the present tense. A checkbox is one bit and cannot say either.
