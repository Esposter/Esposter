...
- **The index row** (`.agents/ledgers/README.md`) — the ledger, its rules owner, what one unit is, and the `Scope` pathspecs its domain covers. This is the sweep's whole metadata; there is no second metadata block below it, and a scope in particular never moves into the ledger file — a resume reads the index to find which files to run against, so a scope stored past that point cannot be reached without opening the thing it was meant to locate. What granularity the coverage table is dated at is not carried beside the unit either: it restates the unit in other words, and the promoted ones are the folder link.
- **Coverage** — a table, one row per unit, ordered by expected payoff: `| Unit | Swept | Notes |`. `Swept` is the date the pass landed and the model that read it (`2026-09-23 · Opus 5.5`, from the commit's `Co-Authored-By` trailer), and `—` while it is open, so the row carries when and by whom as well as whether — written in by `pnpm ai:sweep:ledger-coverage` from the commit trailers: `Ledger: <ledger> | <unit>` on the pass that swept it, `Reopens: <ledger>` (or `<ledger> | <unit>`) on the rule change that invalidates it. The script raises a cell to the newest sweep, clears it on a reopen that is not older than it, never lowers one otherwise, and reports a trailer naming no row — so a hand-written date still counts, and a reworded unit is a new row open at `—`, the way a split one is. **That report is a question, not a to-do, and nothing suppresses it**: the trailer names a unit the tree no longer has, so once the sitting has checked that its successor row exists and carries a date, the line keeps printing and is read past. A list of answered trailers would be the roster that silently drifts from the thing it describes, which is the trade the whole mechanism refuses. A ledger whose units are the tree's own entries — `docs/skills`, one row per skill — gets its rows from the tree too: the same run opens a row for a unit the tree gained and drops one it lost (`LedgerUnitsMap` in `scripts/src/services/sweeps/ledgerCoverage/`), and `scripts/src/workspace/ledgerUnits.test.ts` fails on a table the tree has outgrown, so nobody adds or removes a row by hand there; `Notes` scopes the unit — a boundary, a carve-out, a warning the next pass needs — and states it in the present tense. A checkbox is one bit and cannot say either.
...
- **Every skill that governs a tree the repo holds has a ledger, however small the tree.** A pass is cheap and getting cheaper, and a skill with no ledger is one nobody reads for — a finding against it waits for someone to trip over it. Rejected: leaving a skill unledgered until its tree is worth a sitting, which is how a third of the skills went unread while their findings were "raised rather than swept". The skills with none govern a session rather than a tree (`git`, `review-queue`, `running-checks` and their kind), or are lenses the `quality` ledger's lane already reads with (`over-engineering`, `fallacies`). A mechanical pass with no owning skill is still just the change.
- **One question per file.** A convention another ledger already asks joins that ledger (below); one that asks something different opens its own, however far its files overlap. What never merges is two conventions with different **units**, because a coverage table can only be dated against one of them.
...
- **A row that could never have been read loses its date in the split.** Inheriting the parent's date onto children the parent never read carries the skim forward as if it were coverage. The date survives only where the whole unit was small enough that the pass really could have read it; everything else reopens at `—`. Why the split happened goes in its commit message, never into the ledger, which carries no history.
