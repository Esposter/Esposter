...
the review window — around 90 files against CodeRabbit's 100-file cap (`coderabbit`) — and that budget is spent on
**findings**, not on files. A unit carrying three fixes fills the window by itself; a unit carrying none costs only
its reading and leaves the window open for the next unit. So as the density of findings falls, the number of units
one window carries rises, and the same fixed budget reaches further into the tree every cycle.
