...
**A reference page opens by naming its trigger and what `SKILL.md` kept.** Its first two lines are "read when X" and "the rule itself is in `SKILL.md`; this page is Y" — without them a reader landing on the page cannot tell whether they are holding the whole rule or its tail, and the always-on half gets restated here the next time someone edits it.
