...
- **A second size number beside the ceiling** — a warn threshold, a split-now line, a percentage, or a second axis such as a line count over the byte one. One number is what there is to maintain, and the sweep's `budget` warning is the whole trigger ("SKILL.md is the always-on layer", which argues it).
