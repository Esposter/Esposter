...
| `pnpm ai:coderabbit:exclusions "<base>..<head>" [<rename-sha> OldName=NewName ...]` | `pnpm -C scripts run ai:coderabbit:exclusions`                            | The `path_filters` lines for the files a range lets out of review — pure renames, import-path-only edits and, given the sweep's own commit, rename-token-only edits — never a protected class (`coderabbit` skill, `references/exclusions.md`).                                                                                                                                                                                                                                                                                                                      |
