...
Before writing or widening any regex that finds something inside content a user authored (a blob url, a `{{variable}}`, a blueprint alias), read `packages/app/content/docs/architecture/content-token-rewriting.md` — it is canonical. The four rules that are broken:
