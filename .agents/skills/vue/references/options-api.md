...
JSON Forms' Vue components were that case: its `JsonForms` root and `DispatchRenderer` and every vanilla renderer are Options API, and the schema form threw on render in every dialog while its tests passed. The engine stayed and its Vue shell became the library's own (`apps/web/content/docs/architecture/schema-forms.md`).
