...
- **`defineProps` takes a named `interface <ComponentName>Props`** — never an inline object-literal type or a plain `interface Props`. Name it after the component's identity (file/folder name, stripping `Index`): `Foo/Index.vue` → `FooProps`; `Foo/BarItem.vue` → `BarItemProps`.
  - **How many path words that is, is settled by the folder rather than re-argued per file**, and the outlier is whichever file disagrees with its neighbours — in either direction. A tree whose siblings all carry the folder makes the bare file word the outlier (`ContainerProps` beside `ForwardSendButtonProps`); a tree that spells the whole auto-import path makes the short name the outlier, and there the long spelling is right rather than merely tolerated (`DraftsAndSent/` writes `MessageDraftsAndSent*Props` throughout). A categorisation folder the siblings drop (`Type/Poll.vue` → `PollProps`) stays dropped. **An `export`ed props interface takes the whole auto-import path whatever its folder does**, since it is read at import sites with no folder context to supply the missing words (`ClickerSvgGroupProps`, imported by four icons) — and an interface nothing outside its own file reads is not exported at all. What is never a judgement call: **two different shapes may not share one name in one folder** — `OptionsMenu/Index.vue` and `OptionsMenu/More.vue` both writing `MessageOptionsMenuProps` is a defect however the words are counted.
