...
- **`defineProps` takes a locally declared `interface Props`** — the file path already spells the component (the `props-interface` oxlint plugin enforces the name and the inline-object-literal ban). A shape another file reads moves to its own `.ts` beside the component that owns it (`Styled/DialogActivatorSlotProps.ts`), named after its single export; `props-interface/no-exported-type` enforces the export half, and a shape shared outside its own subtree usually belongs to a model.
- **Prop shorthand naming** — when binding a simple local `ref`/`computed` directly to a prop, name it to match that prop so the `:prop` shorthand works (`const fooType = ref(...)` → `:fooType`). Doesn't apply to complex expressions (`:src="session.user.image"`) or named `defineModel` variables. **A module-scope constant is not a local**: it keeps its casing and the bind is written out (`:items="PIN_ITEMS"`), because the casing says the value is fixed for the life of the process.
...
- **Computed by cost and identity, never by use count** — a computed is a cache with a price, earned on **reuse**, **work** or **identity**; comparisons, booleans, template literals, property reads and map lookups inline. Extracting or inlining any `computed`, the traps, and the cadence question: `references/computed-extraction.md`.
...
- `watchDeep`/`watchImmediate` replace the option object on `watch` (`no-restricted-syntax`, in `.ts` as much as `.vue`). Both are VueUse via Nuxt auto-imports, so they exist in `apps/web` only — a published package keeps the option object behind a disable rather than take on VueUse for an alias. Both flags: `watchDeep(source, cb, { immediate: true })`.
...
Ours are `<script setup>` only (`vue/component-api-style`), and `vue.optionsApi` is **off** — a _dependency_ shipping Options API components mounts and dereferences off `undefined` with nothing thrown. **Adding one, or a `node_modules` component rendering blank**, is that page.
