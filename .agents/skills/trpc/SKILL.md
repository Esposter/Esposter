...
- **A router file holds its `router({ ... })` and nothing else.** Every helper it needs — a repeated where-fragment ("not cancelled and not completed"), a `require*` guard bound to one entity type, a typed client wrapper, a transaction two procedures share — is one export per file under `server/services/<feature>/` (`file-organization`), never a module-level `const` above the router: a private helper is a second definition waiting to happen the moment a sibling router needs the same predicate, and `getRoomMembershipWhere` was written twice that way. **A parameterised fragment is a function, so it is named `get*Where`** — `getRoomMembershipWhere(roomId, userId)`, matching `getCursorWhere` and `getNotBlockedWhere`; only a fragment that takes no arguments is a bare `*Where` const (`activeScheduledMessageJobWhere`), because that name is then a value rather than a call.
