...
**A caller never writes that check itself** — `createErrorAlert` is the one way a caller alerts a rejection, and it asks `checkIsAlertedByErrorLink` before reaching the alert store. Spelled out per site, the guard is a line four callers can each forget, and each one that does ships a double toast nothing catches.
