...
- **`A` prefix for abstract classes only** — never on interfaces, and `no-restricted-syntax` fails the declaration that tries. `AColumn` (abstract class) ✓, `SlashCommand` (interface) ✓, `ASlashCommand` ✗. An interface holding the shape its implementors share takes `Base*`, matching the `base*Schema` it is usually declared beside: `BaseColumnForm`, `BaseExecuteAdminActionInput`
