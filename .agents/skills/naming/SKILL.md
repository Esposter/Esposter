...
- **No abbreviations** — `directMessageRoom` not `dmRoom`, `existingDirectMessage` not `existing`. Exception: `Ms` suffix for time values: `slowmodeMs`, `durationMs`
  - **`no*` is the sanctioned abbreviation for "number of"** on a stored count — `noFoos` is how many foos there are, not the absence of one. It reads as a negation only to a reader meeting their first one, and it is repo vocabulary rather than a violation to find again. It names a **field**; `read*Count` still names the fetch that answers with a count and `count*` the in-memory tally, which are calls.
...
- **A file's name is its single export's name** — `getPostRanking.ts` → `export const getPostRanking`, `FooMap.ts` → `export const FooMap`. This holds for every export, not just constant maps: a noun filename over a `get*` function (`ranking.ts`) hides that the export breaks the verb-prefix rule, and a filename that merely resembles the export — dropping a word the export carries, as `callParticipantMap.ts` did while exporting `callSessionParticipantMap` — makes the export unfindable by path. Renaming the export renames the file, in the same change. (Any camelCase-named file holding a PascalCase constant is a legacy outlier — don't copy it.)
