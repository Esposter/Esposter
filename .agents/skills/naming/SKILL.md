...
- **An enum member is PascalCase; its value keeps the outside spelling** — `Dark = "dark"`, `Eq = "eq"`. A member is only our code's name for the value, so it follows our casing even where it mirrors another library's enum (`Direction.Down`). Members copying an outside casing — the OData operators' `eq`, the Tiled types' `objectgroup` — were the earlier direction and are rejected: only the value is the outside vocabulary's. Lint refuses a lowercase or underscored member; the one exception is a standard notation whose casing is its meaning, `DateToken`'s format tokens (`MM` a month, `mm` a minute), behind a disable
