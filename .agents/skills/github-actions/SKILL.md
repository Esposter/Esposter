...
## A reusable workflow pinned to another ref inherits its secrets

A caller and a callee on the same ref may name what they hand across. A callee pinned to a ref the caller does not live on (`run-review-collector.yaml@ai/queue`, called from whichever ref the event names) is a contract between two copies that a release lag pulls apart, and a named secret list is one more clause of it: the moment either side grows a secret the other has not seen, GitHub refuses the call at startup, on the events that read the stale copy and not the others, until the lagging ref catches up — a failure the workflow cannot heal, since healing needs a run. The call is `secrets: inherit` and the callee declares nothing under `on.workflow_call.secrets` — with inheritance a declaration is a second list to keep true, and the steps' own `secrets.*` reads already say what is spent. Inheriting narrows nothing here: `CI.yaml` runs on a push to every branch with the repository's whole secret set, so what a step pushed to the pinned ref can reach is decided by that workflow, not by the call. The clauses that remain — the file name, the inputs, the ref spelled in each file — are held by a test where nothing in either file can import the constant (`scripts/src/services/coderabbit/collect/constants.test.ts`).
