...
- **Confidence gates the escalation, not the answer.** Below `CONFIDENCE_FLOOR` (`scripts/src/services/jev/constants.ts`) nothing acts on the answer — the case goes up a tier. A decision that writes outside the checkout (a push, a merge, a comment, a closed pull request) needs `HIGH_STAKES_CONFIDENCE` instead, because the cost of being wrong is not a retry.
