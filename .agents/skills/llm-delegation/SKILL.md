...
- **Confidence gates the escalation, not the answer.** Below `0.6` nothing acts on the answer — the case goes up a tier. A decision that writes outside the checkout (a push, a merge, a comment, a closed pull request) needs `0.85`, because the cost of being wrong is not a retry.
