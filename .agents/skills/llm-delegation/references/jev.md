...
- **Confidence gates the escalation, not the answer.** Under its gate nothing acts on the answer — the case goes up a tier. Every gate the estate has today decides something with an effect beyond the checkout (an issue's label, a merge), so `HIGH_STAKES_CONFIDENCE` (`scripts/src/services/jev/constants.ts`) is the only threshold there is: the cost of being wrong is not a retry. A lower-stakes consumer names its own floor beside it when it arrives, and not before — a threshold nothing reads is a rule nothing holds.
