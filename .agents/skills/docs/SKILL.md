...
- **Tests enforce the structural half of that** — `apps/web/content/docs/index.test.ts` (links resolve, index pages cover their siblings, diagrams parse, Key Files paths exist) and `DocsSectionGroupsMap.test.ts` beside the map — so a stale tree fails `pnpm test` rather than waiting on an audit. What they cannot see is a name in prose. Three traps when writing: a link to a page you are about to add fails until it exists, `app/shared/models/…` fails because shared models live at `apps/web/shared`, and an elided path (a `...` segment standing in for directories) is a broken path — write it in full.
