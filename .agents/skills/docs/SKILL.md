...
- **Tests enforce the structural half of that**: `apps/web/content/docs/index.test.ts` (every `/docs/…` link resolves, every index links its siblings, every Mermaid diagram parses, every **Key Files** path exists) and `apps/web/app/services/docs/DocsSectionGroupsMap.test.ts` (the sidebar map and a mapped section agree both ways). Three failures the message will not explain: a link to a page you are about to add fails until it exists; `app/shared/models/…` fails because shared models live at `apps/web/shared`; an elided path (`apps/infra/.../foo.ts`) is a broken path — write it in full.
...
- **Never restate a version a manifest declares — name the field** ("the version `engines.node` asks for"): `update:node` rewrites the manifests, never the prose. Enforced for node and pnpm by `apps/web/content/docs/getToolchainVersionRestatements.test.ts`; the same holds, unenforced, for any value a constant already owns — a path segment, a limit, a threshold — where the page names where it is declared or which command prints it.
