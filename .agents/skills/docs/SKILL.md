...
- **Mechanical follow-through.** After a file moves, `pnpm ai:citations:sync` rewrites every backticked citation of it across the docs, the agent tree and the READMEs from the renames git sees (against `HEAD`, or a base ref passed as its argument once the move is committed) — the Key Files test still catches whatever a hand edit misses. After a rename of a **name**, grep for the old term across the same trees: a stale name in prose survives every check there is. After adding a page, register it in the area `index.md` table and, in a section the sidebar map covers, in `DocsSectionGroupsMap.ts` (`references/page-shapes.md`, "Sidebar grouping").
