...
- **Mechanical follow-through.** After a rename, grep for the old term across every hand-written tree that names identifiers — `apps/web/content/docs`, `.agents/skills`, `.agents/ledgers`, and the READMEs — not the docs tree alone: the Key Files test catches a stale **path**, so a stale **name** in prose survives every check there is. After adding a page, register it in the area `index.md` table and, in a section the sidebar map covers, in `DocsSectionGroupsMap.ts` (`references/page-shapes.md`, "Sidebar grouping").
...
- **Tests enforce the structural half of that** — `apps/web/content/docs/index.test.ts` (links resolve, index pages cover their siblings, diagrams parse, Key Files paths exist) and `DocsSectionGroupsMap.test.ts` beside the map — so a stale tree fails `pnpm test` rather than waiting on an audit. What they cannot see is a name in prose. Three traps when writing: a link to a page you are about to add fails until it exists, `app/shared/models/…` fails because shared models live at `apps/web/shared`, and an elided path (`apps/infra/.../foo.ts`) is a broken path — write it in full.
