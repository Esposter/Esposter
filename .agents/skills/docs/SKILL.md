...
- **Never restate a version a manifest declares — name the field.** `engines.node`, `packageManager` and the `pnpm-workspace.yaml` catalog are where a version lives, and `update:node` rewrites them there; a copy in prose is not one of the places it rewrites, so it goes stale in silence and then tells a contributor to install the wrong runtime. Write "the version `engines.node` asks for", never the number. Enforced for node and pnpm by `packages/app/content/getToolchainVersionRestatements.test.ts`, over every hand-written page in the repo. The same reasoning covers any value a constant already owns — a path segment, a limit, a threshold: state where it is declared, and if the page genuinely needs the number, say which command prints it.
