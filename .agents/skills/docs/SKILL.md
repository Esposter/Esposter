...
- **Mechanical follow-through.** After a file moves, `pnpm ai:citations:sync` rewrites every backticked citation of it across the docs, the agent tree and the READMEs from the renames git sees (against `HEAD`, or a base ref passed as its argument once the move is committed) — `scripts/src/workspace/citations.test.ts` still catches whatever a hand edit misses, across the same trees. After a rename of a **name**, `pnpm ai:sweep:stale-names` lists every code name those trees cite that neither the source nor an installed package's type declarations hold — a candidate rather than a defect, since a name only an external system declares and a naming rule's example were never in either, which is why it is a sweep recipe and not a test. After adding a page, register it in the area `index.md` table and, in a section the sidebar map covers, in `DocsSectionGroupsMap.ts` (`references/page-shapes.md`, "Sidebar grouping").
