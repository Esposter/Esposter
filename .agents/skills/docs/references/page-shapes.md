...
**A proposal that borrows ends with a Sources section.** An algorithm, a paper, a library's technique or another product's design that the proposal takes something from is cited there, one bullet per source: a link to the source and the part the proposal takes from it. A proposal is executed cold later, and whoever builds it reads the original rather than a paraphrase of it. The link goes to the most stable address: a DOI for a paper, the project's own docs for a library. When a proposal ships, its sources go with its substance onto the feature page.
