...
only one of them is finished. A trade names what it buys and what it costs ("one query per page rather than a
join — fast enough for a list of ten"); a defect names something that is simply wrong and is waiting for a
condition to make it visible ("with today's single-target effects this is invisible, but a multi-target effect
would silently drop later targets"). The second one is a bug report filed where nothing will
