...
- **Production's rendered markup copied in as the expected value** — an anchor's whole inline style string, a template's every class, a table's padding. The case reads none of it, so every padding or font tweak fails a test about the link. Parse the output and assert the parts the behaviour owes — the href, the text, the colour the constant names — and nothing the case does not read.
