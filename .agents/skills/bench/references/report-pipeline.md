...
- **That string resolves from wherever Vitest is running**, so every directory a run can start in declares `@esposter/shared-node` as a devDependency — the repo root included, since 🏎️ Bench starts its `vitest bench --run` there. The root owns no bench file and imports nothing from the package, so it reads as a leftover and was dropped once with the devDependencies that served only the tooling tree; the cost is `Failed to load custom Reporter from @esposter/shared-node/reporter` a workflow later, because a bench rewrites its own `*.bench.md` and is in no check suite. `scripts/src/workspace/benchmarkReporter.test.ts` enforces it now, over the run directories it discovers.
