...
A bump can also _regress_ a rule that was green the release before. `1.79.0` made `no-redeclare` fire on TypeScript's value+type declaration merging — `export const RoutePath: {…} = {…} as const; export type RoutePath = typeof RoutePath;` — 20 files across `packages` and `packages/app`, all legal TS. It is `"off"` in the root `rules` map: the pattern is the repo's own convention, and a genuine redeclaration is a typecheck error (TS2451) before it is ever a lint one. Pinning oxlint back instead would freeze every other rule with it; confirm a suspected regression with `pnpm dlx oxlint@<previous> …` before reaching for either.
