...
- **Generated JSON is excluded** — migration snapshots, asset blobs and vitest file snapshots are written and re-read by their generators. They are also most of the JSON in the repo, so the exclusion is most of the run, and a tree missing from it is **invisible**: the files lint green, they just take the time. `packages/parse-tmx`'s `__snapshots__` (40 MB over 131 files) held its `eslint .` at over three minutes against twelve seconds without them — the slowest package in `pnpm lint` by an order of magnitude, and the whole recursive step's critical path. So the tell is a **duration**, not a diagnostic: when one package's lint dwarfs the rest, count the JSON it walks (`eslint . -f json`, group the `filePath`s by directory) before looking at its rules. The standing cross-check is `.oxfmtrc.json`'s `ignorePatterns` — the formatter and the linter walk the same trees, so a generated directory named in one and absent from the other is the gap.
...
**The omission is silent, so audit the scoped entries against each other rather than against the top-level one.** An override that forgets a repo-wide ban lints exactly like one that carries it, and the two scoped `no-restricted-imports` entries here had both dropped the `vue-router`/`useRoute` path that way — the ban survived only in `apps/web/app`, which is the one tree that never needed the override. `"off"` is the same bug in its shortest form: `**/*.test.ts` turning `no-restricted-globals` off for `document` and `localStorage` also turned off the polling ban on `waitFor`/`waitUntil`, in the one file kind where polling is written. Restate the entries the scope actually needs instead of switching the rule off wholesale.

**A scoped glob is stale until it is counted.** Two failure shapes, both of which read as passing: a glob matching **no files at all** (`.agents/**` held no TS after the agent scripts left, and `packages/*/src/index.ts` is gitignored, so oxlint never walks it — the entry existing to exempt the generated barrel from the relative-import ban was exempting nothing), and a glob **stopping short of a tree that already follows the rule** (`packages/*/src/**` left `apps/functions` and `apps/infra` uncovered even though both declare `#src/*` and neither has a `paths` block; `apps/infra/src/github/repository.ts` had been carrying a dead `oxlint-disable-next-line no-restricted-imports` for the ban that never reached it). `apps/web` has no `src/`, so `apps/*/src/**` is exactly the two non-Nuxt apps and reads as the twin of the packages glob. Count the files a glob matches before trusting it, and treat a disable directive found outside a ban's scope as evidence the scope is wrong rather than that the directive is.
