...
**An unimported config file under `eslint/plugins/` is not automatically dead.** `plugins/json.js` is deliberately left out of `plugins/index.js`: it stages `json/recommended` and `json/sort-keys`, which the tree does not satisfy today, so it is kept ready to switch on rather than rewritten when it is. Adding it to the `defineConfig(...)` call in `plugins/index.js` is the whole of enabling it — the `json/json` language it needs is already registered by `plugins/depend.js`. Check a file for a rule set staged this way before calling it unreferenced.
