...
**Creating the PR is itself the start of a review, so the moment after `gh pr create` is an in-flight one.** The check has not appeared yet — the review is queued rather than reporting — and an absent row is the table's unrecognised case (**wait**), never clearance. The easy mistake is the author's own correction: a wrong claim in the body, a stale comment, a typo. It feels like tidying before anyone looks and is a push into a live review, which cancels it, loses its findings, spends the next slot and cancels the CI run. Fix the PR **body** freely (`gh pr edit` touches no sha); a code correction is committed locally and pushed once the review lands.
