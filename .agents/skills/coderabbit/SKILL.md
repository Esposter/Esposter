...
**Creating a PR against the default branch, and every push to one, starts a review** — so the PR is asked for every time, and the moment after `gh pr create` is already an in-flight review. What has to be settled before the ask, and what an author's own correction costs right after opening, is that page.
...
- **Reply to every finding, including rejected ones** — the verdict in the first line (`Agreed, fixed in <sha>` / `Not a real issue, no change`) and the evidence; a silent skip is indistinguishable from an oversight.
- **Push the fix commits first, then reply** — a reply citing a sha the remote does not have is one CodeRabbit cannot resolve.
- **Verify before accepting** — CodeRabbit reasons from names and prior "learnings" and asserts semantics the code does not have; check the implementation, and grep for the repo's convention before taking a suggested diff.
- Where each kind of finding lives (nitpicks and outside-diff-range findings in the review body, the Merge Risk verdict in the walkthrough), reconciling against the stated counts, answering an invalid class in `.coderabbit.yaml`, and why an enforcer over the repo's files does not constrain a pure function's parameter: `references/answering-findings.md`.
