...
1. **`git pull --rebase` before a unit is started, and again before every push.** The collector rewrites `origin/ai/queue` after every window it ports — the ported commits gone, the rest re-parented onto the fixes — so the local branch is stale the moment a window goes out, and a unit written on the stale base is one more commit the pull must replay. **A rebase refuses a dirty tree** and nothing here sets `rebase.autoStash`, so the pull goes where the tree is clean — before the unit's first edit, and after its commit rather than between the two; a unit already in progress catches up at its commit. The pull's fork-point finds where the local branch left the old remote head and replays only what the session committed since; the rewritten commits are never replayed. A conflict here is between the session's own unpushed commits and the collector's fixes, the one kind only the session can settle — resolve it, `git rebase --continue`, and prove the result with `git diff <old head>`, which must show nothing but what the remote brought.
2. **Commit a unit on `ai/queue`.** Run the finishing checks in the background and commit their repairs as their own commit behind the unit — the collector cuts at commit boundaries, so every cut must be green on its own.
3. **`git push` after every commit, plain.** A push refused as non-fast-forward is the collector's rewrite landing since the pull: pull again, push again. **Never a bare `--force-with-lease`** — its lease is whatever the last fetch brought, so a local branch built on the rewrite's predecessor overwrites the rewrite with nothing reporting it; the only lease the session may push with names a sha it read and rebuilt on itself. A queue push spends nothing, so the standing rule that a push is asked for every time is `develop`'s — which the session never pushes.
