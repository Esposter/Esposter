---
name: review-queue
description: Apply when about to git commit or git push, when deciding what branch a commit lands on, when the collector has moved develop or rewritten ai/queue, when a CodeRabbit finding is being fixed in the session, when a collector run is red, or when the collector is switched off. Esposter's review queue — the session's side of the review collector: one permanent branch ai/queue, git pull --rebase before every commit and push because the collector rewrites the queue onto each window, a plain push after every commit and never a bare force-with-lease, a sweep's moves committed apart from its repairs, the Answers / Drains trailers, and the collector run by hand only with its workflow disabled.
---
...
1. **`git pull --rebase` before every commit and before every push.** The collector rewrites `origin/ai/queue` after every window it ports — the ported commits gone, the rest re-parented onto the fixes — so the local branch is stale the moment a window goes out, and a commit made on the stale base is one more commit the pull must replay. The pull's fork-point finds where the local branch left the old remote head and replays only what the session committed since; the rewritten commits are never replayed. A conflict here is between the session's own unpushed commits and the collector's fixes, the one kind only the session can settle — resolve it, `git rebase --continue`, and prove the result with `git diff <old head>`, which must show nothing but what the remote brought.
