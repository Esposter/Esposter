...
1. `\b\d+ (files|packages|pages|components|stores|routers|tests|rows|procedures)\b` for what the repo can
   ...
   Its noise is a status code read as a unit (`404s`), a CSS duration in a sample, and a timeout the reader is
   being told to pass
...
`scripts/src/workspace/restatedPointers.test.ts`, the sidebar map beside itself in
