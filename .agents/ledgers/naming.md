...
| `app/shared/models` — the editor and game trees                        | 2026-09-02 | `BuildingWithStats` spelled out. `noPoints` is the sanctioned `no*` abbreviation and the chart's `dataLabels` is ApexCharts' own option name — neither is a finding, and `flowchartEditor` mirrors `@vue-flow/core` field for field for the same reason    |
| `app/shared/models` — the rest                                         | 2026-09-03 | `DelimiterCharMap` spelled out, `Environment`'s lowercase members onto PascalCase (the values are `MODE`'s, the member names ours), the leaderboard's `self` onto `myEntry`, and `PropertyCondition` taking the family word its three union siblings carry |
