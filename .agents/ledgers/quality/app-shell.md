...
| `services/{vuetify,styled,entity,zod,ajv,jsonSchema,compiler,shared,azure,cache,file}` + `util/date` + the matching `composables` | 2026-08-20 | `app/services/compiler` is not the swept `shared/services/compiler`; `getFilePickerTypes` and `EMPTY_FILE_MESSAGE` each own their one copy |
