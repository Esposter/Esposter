# Tooling

The repo's own scripts and the agent tree — code no product area claims, and the gap the eight product areas
left when this ledger was written.

| Unit                                      | Swept      | Notes                                                                                                                                                                                                                 |
| ----------------------------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scripts/checkDependencies`               | 2026-09-02 | Every manifest read once through `PackageManifest` rather than twice behind `Record<string, unknown>` casts; the change levels, the field list and the registry concurrency named; the timeout a `Temporal.Duration`. |
| `scripts/oxlint`                          | —          |                                                                                                                                                                                                                       |
| `scripts/updateNode`                      | —          |                                                                                                                                                                                                                       |
| `scripts/services`, `scripts` root files  | —          |                                                                                                                                                                                                                       |
| the repository root — config and Markdown | —          |                                                                                                                                                                                                                       |
| `packages/app/configuration`              | —          |                                                                                                                                                                                                                       |
| the app's root config files               | —          |                                                                                                                                                                                                                       |
| `packages/app/content`                    | —          |                                                                                                                                                                                                                       |
