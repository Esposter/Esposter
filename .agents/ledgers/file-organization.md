...
| `packages/azure`, `apps/functions`, `packages/azure-mock`                          | —          | cross-package placement: an Azure helper two packages need lives in `db`; whether `apps/functions/src/services` — one feature folder against a root of loose files — has a `shared/` to name is a judgement, not a move |
...
| `packages/parse-tmx`, `packages/xml2js`                                            | —          | barrel contents are `ctix` output — regenerate, never hand-edit; `parse-tmx/src/models` is a layer that never grew features rather than one whose shared bucket is unnamed                                              |
...
| `scripts/src/coderabbit`                                                           | 2026-09-13 | four verbs; what two share sits in `services/coderabbit/shared`                                                                                                                                                         |
