...
- **`ManageInvites` is defined, described and granted by default, and nothing reads it** — `createInvite` is a plain
  member procedure, so revoking the permission changes nothing. The call is whether the permission is enforced or
  deleted; enforcing it is a behaviour change, so it is not this sweep's to make.
