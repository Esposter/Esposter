...
| Unit                                         | Swept      | Notes                                               |
| :------------------------------------------- | :--------- | :-------------------------------------------------- |
| `app/services`, `app/util`                   | 2026-09-15 |                                                     |
| `app/store`                                  | 2026-09-15 | `message/input` — the Map is the storage            |
| `app/composables`                            | 2026-09-15 | holds the standing exception below                  |
| `app/components`, `app/pages`, `app/plugins` | 2026-09-15 | phases, plus one genuine fork                       |
| `app/middleware`, `app/layouts`              | 2026-09-15 | the only browser reach is a `.client.ts` middleware |
...
2. `checkIsServer` — the one that still needs reading. A hit is either a genuine fork (both branches real, keep) or a
   leaf that has already been answered by a ref or a phase (convert).
3. `window\.` over `app/` — the wider class: `document`, `navigator`, `matchMedia`. Its deterministic half is now
   ...
   says which. The two shapes to look for: a call at a **setup root**, and a call inside a **computed**, which
   survives SSR only while nothing reads the computed during the server render.
