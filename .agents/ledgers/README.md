...
Still unledgered, and known to be: `pagination`, `invariants`, `runtime-efficiency`, `build`, `github-actions`,
`pulumi-infra`, and the product skills (`esbabbler`, `esbabbler-call`, `routing`, `slash-commands`, `tiptap`,
`grapesjs`, `azure-table`, `string-utils`, `vue-phaserjs`) — each too small for a sitting today, so a finding
against one is raised rather than swept. The process skills (`git`, `review-queue`, `running-checks` and their
kind) govern a session rather than a tree, so they have nothing to sweep.
