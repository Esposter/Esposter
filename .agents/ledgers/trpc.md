...
| `server/trpc/routers/achievement`, `app`, `clicker`, `dungeons`, `notification`, `pushSubscription`, `session`, `storage` | 2026-08-31 | clean — the two game saves are the shared blob-state builders, and everything else reads the caller's own id off the session                                                                                                                              |
