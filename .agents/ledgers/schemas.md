...
| Unit                                                       | Swept                 | Notes                                                                                                |
| ---------------------------------------------------------- | --------------------- | ---------------------------------------------------------------------------------------------------- |
| `packages/db-schema/src/schema.ts` + `relations`           | 2026-09-25 · Opus 5.5 | a `r.many` key is the child table's own name; an `r.one` off a role column is the role               |
| `packages/db-schema/src/schema` — the message tables       | 2026-09-25 · Opus 5.5 | the `*InMessage` family                                                                              |
| `packages/db-schema/src/schema` — the rest                 | 2026-09-25 · Opus 5.5 | the twin `friends`/`friendRequests` blocks stay, and the `drizzle` skill says why                    |
...
| `shared/models/db` — the room family                       | 2026-09-25 · Opus 5.5 | `room`, `roomCategory`, `roomEmoji`, `role`, `moderation`, `webhook`                                 |
| `shared/models/db` — the rest                              | 2026-09-25 · Opus 5.5 | `blueprint`, `friend`, `notification`, `post`, `searchHistory`, `user`, `userSettings`, `userToRoom` |
| `shared/models/resource/sheet`                             | 2026-09-25 · Opus 5.5 | the split transformation's form defaults stay on the shared schema — `zod` says why                  |
| `shared/models/resource` — the rest                        | 2026-09-25 · Opus 5.5 | the per-type content shapes and the capability types                                                 |
| `shared/models/dungeons`                                   | 2026-09-25 · Opus 5.5 |                                                                                                      |
| `shared/models` — the editor and game trees                | 2026-09-25 · Opus 5.5 | the GrapesJS subclasses re-declare the catchall a `.shape` spread drops, pinned by a test            |
| `shared/models` — the rest                                 | 2026-09-25 · Opus 5.5 | `achievement`, `message`, `pagination`, `dataset`, `entity`, `compiler` and the singles              |
