...
| `shared/models/db` — the room family                       | 2026-09-22 | `room`, `roomCategory`, `roomEmoji`, `role`, `moderation`, `webhook`                                 |
| `shared/models/db` — the rest                              | 2026-09-22 | `blueprint`, `friend`, `notification`, `post`, `searchHistory`, `user`, `userSettings`, `userToRoom` |
| `shared/models/resource/sheet`                             | 2026-09-22 | the split transformation's form defaults stay on the shared schema — `zod` says why                  |
| `shared/models/resource` — the rest                        | 2026-09-22 | the per-type content shapes and the capability types                                                 |
| `shared/models/dungeons`                                   | 2026-09-22 |                                                                                                      |
| `shared/models` — the editor and game trees                | 2026-09-22 | the GrapesJS subclasses re-declare the catchall a `.shape` spread drops, pinned by a test            |
| `shared/models` — the rest                                 | 2026-09-22 | `achievement`, `message`, `pagination`, `dataset`, `entity`, `compiler` and the singles              |
| `app/models`, `app/services/*/…` form schemas              | 2026-09-22 | the Vjsf-rendered ones carry extra rules                                                             |
| `packages/db`, `packages/db-mock`                          | 2026-09-22 | neither declares a table or a zod schema; the mock's snapshot is generated                           |
| `packages/shared`, `packages/parse-tmx`, `packages/xml2js` | 2026-09-22 | `parse-tmx` and `xml2js` depend on zod nowhere                                                       |
