...
| `sheet-editor/`, `infra/`                                      | 2026-08-21 | diagram pass — the compute-map fan-out listed six computers the key files table already lists; only the nested Aggregation map is a second level and stays. `eventgrid-dead-letter` keeps both of its diagrams: the flowchart is the delivery path, the state diagram is the event's own lifecycle. Earlier prose pass: two pages renamed onto what they describe rather than the migration that produced them; the dead-letter page's operator remediation was cross-referenced twice and stated nowhere, so it is now written; bare /docs routes given prose link text                                                                                                                                        |
| `clicker/`, `dungeons/`, `posts/`                              | 2026-08-21 | diagram pass — clean. The battle state machine and the posts/comments fan-out both earn their density: every edge is a transition or a guard that differs. Earlier prose pass: split from `users/` to fit a review window. Two pages owed a diagram and had none — likes (three procedures, one transaction, an optimistic store) and dungeons saves (two load paths, a manual save, the achievement trigger)                                                                                                                                                                                                                                                                                                   |
| `users/`                                                       | 2026-08-21 | diagram pass — clean, two sequence diagrams and a fan-out. Earlier prose pass: a roadmap logging what shipped instead of holding open work, and three notes recording that a change needed no migration — the standing fact each was carrying is now stated on its own                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `vue-phaserjs/`, `achievements/`, `fluid-simulator/`, `anime/` | 2026-08-21 | diagram pass — clean. Earlier prose pass: the two exhaustive component inventories on the vue-phaserjs index replaced by the command that answers them; a shipped-changelog section cut. The pass found the escaped-line-break defect and it is now enforced, so grep 8 is retired                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `proposals/`, `docs/index.md`, every area `index.md`           | 2026-08-21 | diagram pass — clean; the proposals' diagrams were written under this rule. Earlier prose pass: the drift this row was opened for did not exist — all 40 index pages list every page beside them, and `content/docs/index.test.ts` now holds that, so the concern is enforced rather than re-read                                                                                                                                                                                                                                                                                                                                                                                                               |
