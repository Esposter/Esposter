...
| `proposals/`, `docs/index.md`, every area `index.md`           | 2026-08-20 | the drift this row was opened for did not exist — all 40 index pages list every page beside them, and `content/docs.test.ts` now holds that, so the concern is enforced rather than re-read                                                                                                                                        |
...
Enforceable next, all in `content/docs.test.ts` where the link, index-coverage, Key Files,
mermaid-parse and both label-line-break checks already live. Greps 4, 5 and 6 are exact and decide themselves — but the link-text half of 4 can only land
