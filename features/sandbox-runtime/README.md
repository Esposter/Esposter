...
- `vfs` backend **Step B1** shipped: `BackendType.Vfs` runs `node -e`/`--eval` in the current process — a shell-aware tokenizer parses the invocation, inline code runs via `vm.runInThisContext` with patched process streams/exit + injected `require`, capturing stdout/stderr/exit-code. Anything it can't run faithfully in-process (shell features, other flags, file runs, syntax errors, async) falls back to the native backend, so results always match baseline. Next up in [roadmap.md](roadmap.md): **Step B2** — mount the `FsProvider` around the run (module hooks + fs interception) and `node <file>` + differential tests vs native.
