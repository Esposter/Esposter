...
- `vfs` backend **Step B1** implemented (not yet gate-passed): `BackendType.Vfs` runs `node -e`/`--eval` in the current process — a shell-aware tokenizer parses the invocation, inline code runs via `vm.runInThisContext` with patched process streams/exit + injected `require`, capturing stdout/stderr/exit-code. Anything it can't run faithfully in-process (shell features, other flags, file runs, syntax errors, async) falls back to the native backend. **It is not "shipped" until the gates pass** — this is the first backend that can regress, so the next step is the differential (native vs vfs identical) + benchmark (in-process vs spawn must beat native) pass. Then **Step B2** mounts the `FsProvider` (module hooks + fs interception) and adds `node <file>`. See [roadmap.md](roadmap.md).
