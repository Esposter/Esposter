...
- [x] **Speed gate** — `createVfsBackend.bench.ts` ([bench/results.md](../../packages/sandbox-runtime/bench/results.md)): in-process `node -e` runs orders of magnitude faster than native spawn (the node-startup cost removed); the fall-back path ties native within noise (parse-and-delegate adds ~0). Absolute timings are machine-dependent and never pinned here — the regression check is regenerating + diffing `results.md`, where each task's `vs base` multiplier (native = `1.00×`) makes a regression obvious. → [specs/benchmarking.md](specs/benchmarking.md)
