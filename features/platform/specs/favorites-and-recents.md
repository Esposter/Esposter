...
The portal Home shows Recent (last opened by you) and Favorites (starred) — both per-user, cross-device. Our Home approximates recents with `updatedAt` desc, which is wrong the moment anything autosaves, and has no favorites at all. Favorites are a small Postgres table; recents start as localStorage (per-device) with a table upgrade only if cross-device recall matters.
...
- Favorites group in the empty-query search dropdown ([global-search.md](global-search.md)) — optional follow-up
