...
- Purge protocol — shared by `purgeResource` and the timer, ordered for idempotent retry: delete the `{id}/` blob directory first, dependent Azure Table partitions next (e.g. the [activity log](activity-log.md)'s), and the Postgres row **last**. A partial failure leaves the row behind as the durable marker that re-drives the whole sequence; blob and table deletes treat already-gone as success, and failures rethrow so the timer retries.
