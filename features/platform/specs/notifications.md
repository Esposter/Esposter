...
The portal bell is the feedback channel for every operation — a toast on completion plus a session history panel. Ours has no equivalent: publishes and deletes succeed silently. This is client-only — a Pinia store, an app-bar bell with badge, and snackbar toasts. No backend, no persistence (the portal's bell is effectively session-scoped too).
...
- save conflict (stale `contentVersion`) → warning with **Refresh** action ([resource-page-parity.md](resource-page-parity.md))
...
- `G N` opens the panel ([global-search.md](global-search.md) shortcuts).
...
- Deliberately not persisted (no table, no localStorage) — the value is immediate feedback + a session trail; durable history is the activity log's job ([activity-log.md](activity-log.md)).
