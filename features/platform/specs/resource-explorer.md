...
Azure-portal-faithful **stacked blade**: clicking a resource keeps the list visible and stacks the resource shell **on top of it** (both visible, like the portal), so `/resources/[id]/[[blade]]` renders the shared `ResourceListView` as its base plus the blade as an **absolutely-positioned elevated surface panel** over the list's right side. `inset-y-0 right-0`, `left` responsive (full-width on mobile; leaving a ~`24rem` list strip on `smAndUp`), `bg-surface` + `shadow-lg` + inline-start border for the layered look — this is the deliberate bit of positioning CSS. The list strip stays interactive: clicking another row swaps the blade; the **close ✕** (`:to`) returns to `/resources/all`. Inside the panel: a left blade nav + a content pane, framed by the breadcrumb and a resource-level command bar.

- **Breadcrumb trail**: `Home › Resource Explorer › All › {name}` — `AppBreadcrumbs` takes a `crumbs` prop for the intermediate `All` (→ `/resources/all`) between the matched product and the `{name}` leaf. Navigation uses `:to` (real `<a>`, keyboard + ARIA) everywhere the target is static — `navigateTo` only for dynamic targets (search submit, post-create/-delete redirects, data-table row clicks).
- **Left blade nav** (`v-list`, surface panel with an inline-end border): **Overview** first, then **Editor**, then the type's blades from `ResourceBladeDefinitionMap: Record<ResourceType, BladeDefinition[]>`. Each item deep-links via `:to` to `/resources/[id]/[[blade]]`; the active blade is highlighted.
