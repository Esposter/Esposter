...
The portal's Activity log answers "what happened to this resource and when". Ours has nothing — a publish or rename leaves no trace. Events are message-shaped (high write volume, time-ordered, no joins), so they belong in Azure Table Storage per the storage split — no Postgres migration, no new Azure service.
...
Cleanup: Azure Table Storage has no partition-drop operation, so activity deletion enumerates the `resourceId` partition and batch-deletes entities in transactions of up to 100 (the existing batch convention). Soft delete ([recycle bin](recycle-bin.md)) leaves the partition intact — history survives the bin window and restore appends `Restored` — and the sweep runs at purge time (`purgeResource` and the timer purge), before the resource row is deleted so a failed sweep is re-driven by the surviving row on the next pass.
...
- Activity is the durable trail; the [notifications bell](notifications.md) is the ephemeral session feedback — they share event sources but never storage.
