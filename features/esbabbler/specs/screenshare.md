...
- Starting a share **auto-pops the call into Picture-in-Picture** (Meet/Discord style) so the sharer keeps watching the call while presenting another window. Because `documentPictureInPicture.requestWindow()` needs live user activation and the `getDisplayMedia` picker consumes the click's activation, `toggleScreenShare` sets `isPoppedOut = true` **before** calling `setScreenShare(true)` — i.e. the PiP window opens within the click gesture, then the picker runs. If the user cancels the picker (or the share fails) the pop-out is reverted; on browsers without Document PiP it never pops (`Pip/Host` clears the stale intent). The user can still close the PiP manually. See [`specs/picture-in-picture.md`](picture-in-picture.md).
