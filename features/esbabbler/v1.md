...
- [ ] **Friends system** — bidirectional friend requests (Pending/Accepted/Blocked); `friendshipKey` composite unique constraint prevents duplicate A→B / B→A rows; accepted friends are the recipient pool for the "New Message" DM picker; see [`friends.md`](friends.md)
...
- [ ] **Friends** — bidirectional friend requests with `friendshipKey` idempotency; friends are the DM recipient pool; see [`friends.md`](friends.md)
