...
- **WSL ext4 source mirror** — on win32 the source lower is a WSL-native ext4 mirror (`<cache>/sources/<sha256(hostCwd)>`, per-mirror `flock`, incremental `rsync -a --delete` excluding `node_modules`, `.git`, and an environment's prepare outputs) instead of `/mnt/c`, moving the toolchain's source reads off v9fs (15–64× slower). `--overlay-src`/`--chdir` point at the mirror; write-back's flush target stays `options.cwd`, independent. Because the sandbox now chdir's into the mirror (not `/mnt/c`), the os PATH prepends the mirror's own `node_modules/.bin` so a bare `tsgo`/`eslint`/`oxlint` resolves the overlaid current-platform binary ahead of the win32 host `.bin` the WSL login PATH leaks in. `virrun doctor` probes `rsync`; `cache clean --all` sweeps `sources/`. Bench quantification of the lift is still open (needs a healthy WSL host). → [wsl-source-sync](specs/wsl-source-sync.md)
