...
- **Phase 4 — config allowlist (adoption level 3)**: a committed repo-root `virrun.config.json` (`route`/`backend`/`fallback`) names which commands sandbox. `resolveVirrunConfiguration` walks up via `empathic` (standard parent-dir search, JSON config), `parseVirrunConfiguration` validates + defaults, `matchesRoute` does leading-token matching, and the CLI's `resolveCommandBackend` picks the backend with host-support auto-fallback. Absent config = native no-op. → [config-and-cache](specs/config-and-cache.md) · [adoption](specs/adoption.md)
