...
**Quantify the win32 mirror lift — bench re-run.** The WSL ext4 source mirror shipped (see `## Shipped`), including the follow-on fix for the PATH regression it introduced (the sandbox chdir moved off `/mnt/c`, so a bare command's `node_modules/.bin` lookup must be re-anchored at the mirror — [wsl-source-sync](specs/wsl-source-sync.md)). The one open item is re-running `pnpm bench` on win32 to record the actual speed lift honestly (no overclaim) — blocked on a healthy WSL host (this host's sandbox errors on capture). Broadening the isolation surface (macOS bridge, Firecracker) stays deferred, not a `Now` item.

**Bench the wins, not just the worst case.** The committed bench measured only single cold `os` runs (0.84–0.95× native — the design-bounded overlay read tax), so the story read as "just slower" while the shipped wins (task-cache hit, warm-fork amortization) sat unbenchmarked. Added the `test - task cache hit vs native` group (primes one entry at module scope, times a pure `replayTaskCache` — the sandbox-skip that should beat native by a wide margin, now regression-gated); the fork groups already carry the warm-fork/repeated-run story, so no separate group was added. Re-running `pnpm bench` to capture the numbers is blocked on the same healthy-os-host requirement above. → [benchmarking](specs/benchmarking.md)
...
- **Bench-truth** — corrected the speed story (no overclaim): cut the non-cashable install / warm-vs-cold-reinstall bench groups (the os install feeds the fork snapshot, not host disk → [out-of-scope/materialize-node-modules.md](out-of-scope/materialize-node-modules.md)) and memoized `computeLockfileHash`. On ext4 the remaining per-command tax is inherent overlayfs read cost. The fork groups (typecheck/build/test) run `os/*` as an independent warm fork per iteration, so their mean is the amortized repeated-run cost vs native; a `test - task cache hit vs native` group then benches the sandbox-skip path (`replayTaskCache`, one primed entry) — the only group expected above `1.00×`. → [benchmarking](specs/benchmarking.md)
