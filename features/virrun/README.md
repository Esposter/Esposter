...
Two pass/fail gates on every backend and speed feature — a violation is not shippable, however clever: **faster than the native baseline**, and **observably correct** (exit code, stdout/stderr, produced files, dependency tree) vs running the command natively. Correctness beats speed; a fast wrong answer is worthless. Both are now **CI-enforced**: differential correctness is a shared Vitest harness (a growing command corpus, an explicit `normalizeExecResult` masking seam with nothing normalized implicitly, and the `assertDifferential` helper both backends run their corpus through) that hard-fails the 🏗️ CI coverage shards on any divergence; speed is tracked by the committed `*.bench.md` offline diff, with 🏎️ Bench running plain `vitest bench` shards every push as an executes-clean smoke signal (a hard wall-clock CI fail is rejected as runner-noise-flaky, and CodSpeed was removed after exceeding its 600 min/month free tier → [out-of-scope/ci-walltime-gate.md](out-of-scope/ci-walltime-gate.md)). Detail: [benchmarking](specs/benchmarking.md) · [correctness](specs/correctness.md).

## Now

Nothing actively queued — see [roadmap.md](roadmap.md). The WSL ext4 source mirror and its win32 bench re-run have shipped (`## Shipped`); everything remaining in the roadmap's `## Next` is trigger-gated. Broadening the isolation surface (macOS bridge, Firecracker) stays deferred.
...
- **WSL ext4 source mirror** — on win32 the source lower is a WSL-native ext4 mirror (`<cache>/sources/<sha256(hostCwd)>`, per-mirror `flock`, incremental `rsync -a --delete` excluding `node_modules`, `.git`, and an environment's prepare outputs) instead of `/mnt/c`, moving the toolchain's source reads off v9fs (15–64× slower). `--overlay-src`/`--chdir` point at the mirror; write-back's flush target stays `options.cwd`, independent. Because the sandbox now chdir's into the mirror (not `/mnt/c`), the os PATH prepends the mirror's own `node_modules/.bin` so a bare `tsgo`/`eslint`/`oxlint` resolves the overlaid current-platform binary ahead of the win32 host `.bin` the WSL login PATH leaks in. Each mirror is a self-contained entry (`tree/` holds the byte-exact repo, an `origin` marker records the host cwd), and `reapAbandonedSourceMirrors` reclaims at os-backend startup any entry whose `origin` host dir is gone (deleted worktree / moved repo) — the one cache entry with no lockfile/source key to supersede it. `virrun doctor` probes `rsync`; `cache clean --all` sweeps `sources/`. Bench captured on win32 (`localMonorepo.platform.bench.win32.md`) — honest numbers: the WSL bridge tax dominates on win32 (`os/wsl` currently trails native per-command), the mirror kills the v9fs read tax but doesn't by itself flip win32 to a net win. → [wsl-source-sync](specs/wsl-source-sync.md)
...
- **Prepare layer (source-keyed generated artifacts)** — an `environment` config preset (`nuxt`) captures a framework's generated artifacts (`nuxt prepare` → `.nuxt`) into a **second** overlay layer keyed by lockfile + source-tree hash, forked _above_ the deps snapshot (last `--overlay-src` wins) so its Linux-generated copy shadows the host's. Fixes a win32→WSL type-aware-lint phantom — the host's win32 `.nuxt` collapsed `Ref`/alias/`zod` types under Linux tsgolint (`no-unnecessary-type-parameters`) though it was clean natively. A source edit rebuilds only this layer (deps reused, no reinstall); write-back masks the outputs like `node_modules`; `cache ls`/`clean --all` cover it. Obviated (and removed) the `virtual-store-dir-max-length=60` install pin. → [snapshot-fork](specs/snapshot-fork.md#prepare-layer-realized--source-keyed-generated-artifacts)
...
- **Config backend selection** — a committed repo-root `virrun.config.{ts,mts,js,mjs,json}` pins the backend, loaded via unconfig; the TS form (`defineConfig` from `virrun/config`) is where `process.platform` branching lives (this repo: win32 → os, else native). The prefix stays the sole per-command switch (no allowlist), with host-support auto-fallback. → [config-and-cache](specs/config-and-cache.md) · [adoption](specs/adoption.md)
...
- **Native-on-Linux CI** — the platform-branched `virrun.config.ts` resolves `native` on the Linux CI runners (the sandbox fixes a win32-only problem), so verify jobs run a plain `pnpm i` (pnpm store cache + `postinstall: nuxt prepare`) with no bubblewrap install and no warm-cache stage on the critical path; the earlier warm-snapshot/prepare `actions/cache` pipeline was removed with it. → [virrun CI](https://github.com/Esposter/Esposter/blob/main/packages/virrun/readme/ci.md) · [snapshot-fork](specs/snapshot-fork.md)
...
- **Concurrency-safe cache** — the host-global cache is shared across repos/worktrees, so every capture/persist/task temp is pid-tagged and reaped only once its owner pid is dead, and each mounted snapshot/prepare hash dir holds a `leases/<pid>` the superseded-dir prune honors. Two live runs — even on different lockfile hashes — never delete each other's in-flight temps or published layers. → [config-and-cache](specs/config-and-cache.md#cleanup--self-healing)
