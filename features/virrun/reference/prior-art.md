...
- Bare `wisp` / `skiff` / `husk` — all taken on npm; `wisp` is also a proxy protocol, `skiff` an ex-email brand. Settled on `virrun` (a contraction of _virtual runner_) — short, unclaimed on npm, and published unscoped.
