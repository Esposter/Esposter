...
The differential (layer 3) gate is wired as shared test infrastructure under `services/exec/differential/`. A backend's `*.differential.test.ts` feeds its corpus to one helper; the corpus and the normalization seam are the source of truth that grows. Layers 1 (unit) and 2 (integration/acceptance) live beside the code they cover (`*.test.ts` / `*.acceptance.test.ts`); snapshot/fork equivalence (layer 4) and property/fuzz (layer 5) are not yet built.
...
| snapshot/fork equivalence (layer 4), property/fuzz (layer 5), CI gate enforcement | —                                                                                                                                                             | planned (CI gate → [deferred/ci-bench-gate.md](../deferred/ci-bench-gate.md)) |
