...
- **PATH re-anchoring (shipped fix).** Moving the sandbox chdir off `/mnt/c` onto the mirror broke bare-command resolution: the WSL login PATH (Windows-interop appends the Windows PATH) still carries `/mnt/c/<repo>/node_modules/.bin`, which pre-mirror was the overlaid dir but is now the raw host tree — so a bare `tsgo`/`eslint`/`oxlint` resolved the **win32** binary and crashed needing its `-linux-x64` sibling (e.g. `Unable to resolve @typescript/native-preview-linux-x64`, or esbuild's "installed for another platform"). Fix: `createOsExecOptions` prepends the mirror's own `node_modules/.bin` (`getWslSourceMirrorPath(cwd)/node_modules/.bin`, overlaid from the snapshot lower = current platform) ahead of the login PATH, so the correct-platform binary wins. Native Linux is unaffected (it overlays at `cwd`, so that PATH entry is already the overlaid dir). Regression-gated by `createOsExecOptions.test.ts`. The correctness gate missed the original break because its corpus is `node_modules`-free **and** no CI host is win32+WSL, so the interop-PATH leak never arose there.
