...
Every cache write is disposable, but it must not _accumulate_. Three mechanisms keep `~/.virrun` (and the shared source-temp root) bounded across a machine's lifetime, so a killed run never leaks disk permanently. All run **off the command's critical path** — detached, best-effort, and gated so a failure never aborts the run — and all bank on the same concurrency model: a snapshot is captured **once** (a dedicated CI job; a single cold dev run) and persist runs are **serial**, so no two live runs hold a temp in the same hash dir.

- **Finalizer teardown (clean exit)** — each run captures into a private `mkdtemp` sibling (`snapshots/<hash>/upper.<rand>` + `work.<rand>`, a persist's `upper.persist.<rand>`, a prepare's `prepare/<key>/upper.<rand>`, or a git/files source clone's `virrun-temp-<rand>`) and its in-process finalizer removes that temp on both success and handled error. The published layer is promoted by an atomic `renameSync` onto the bare `upper`; the temp never survives a normal exit.
...
- **Temp-corpse reap (next run)** — a **hard** kill (SIGKILL, crash, `wsl --shutdown`) skips the finalizer, stranding a temp _inside the live hash dir_ — which the prune deliberately skips (`name !== current`) — or in the shared source-temp root, which nothing else sweeps. `reapStaleTemps` runs beside each prune (and once at `loadSource`) and removes any `upper.`/`work.`/`virrun-temp-` prefixed sibling, never the published bare `upper`/`work`. So the _next_ run reaps the previous one's corpse: the cache is self-healing, bounded to `live entry + at most one in-flight run's temps`.
