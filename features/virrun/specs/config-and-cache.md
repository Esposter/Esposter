...
- **`snapshots/`** (host-global) — "clone + install" frozen once; each routed run `fork()`s the matching snapshot. Keyed by lockfile hash so a dependency change invalidates exactly the affected entry, and shared across repos/CI runs. Only the current lockfile's hash is ever reused, so `ensureSnapshot` sweeps every superseded `snapshots/<hash>` (via `pruneStaleSnapshots`) before it hits or mints one — the cache is pinned to a single live entry rather than accumulating a stale ~2.3 GB dir per dependency bump. It lives in `~/.virrun`, **not** the repo, because a fork stacks the snapshot as an **overlay lower** beside the source and overlayfs rejects a lower that nests inside another. In 🏗️ CI this directory is persisted via `actions/cache` keyed by the lockfile hash (`warm-snapshot.yaml` captures it once; format/lint/typecheck restore it read-only and fork it), mirroring the `build-packages` content-hash cache. → [snapshot-fork](snapshot-fork.md)
...
Every cache write is disposable, but it must not _accumulate_. Three mechanisms keep the host-global `~/.virrun` bounded across a machine's lifetime, so a killed run never leaks disk permanently. All run **off the command's critical path** — detached, best-effort, and gated so a failure never aborts the run — and all bank on the same concurrency model: a snapshot is captured **once** (a dedicated CI job; a single cold dev run) and persist runs are **serial**, so no two live runs hold a temp in the same hash dir.

- **Finalizer teardown (clean exit)** — each run captures into a private `mkdtemp` sibling (`snapshots/<hash>/upper.<rand>` + `work.<rand>`, a persist's `upper.persist.<rand>`, a prepare's `prepare/<key>/upper.<rand>`) and its in-process finalizer removes that temp on both success and handled error. The published layer is promoted by an atomic `renameSync` onto the bare `upper`; the temp never survives a normal exit.
- **Stale-entry prune (next run)** — only the _current_ lockfile hash / source key is ever reused, so `ensureSnapshot`/`ensurePrepareLayer` sweep every superseded `snapshots/<hash>` and `prepare/<key>` before hitting or minting the live one (`pruneStaleSnapshots` / `pruneStalePrepareLayers`). The cache stays pinned to one live entry per axis.
- **Temp-corpse reap (next run)** — a **hard** kill (SIGKILL, crash, `wsl --shutdown`) skips the finalizer, stranding a temp _inside the live hash dir_ — which the prune deliberately skips (`name !== current`). `reapStaleTemps` runs beside each prune and removes any `upper.`/`work.` prefixed sibling, never the published bare `upper`/`work`. So the _next_ run reaps the previous one's corpse: the cache is self-healing, bounded to `live entry + at most one in-flight run's temps`. This is scoped to the **per-hash** snapshot/prepare dirs — where the capture-once/serial model guarantees no live run holds a sibling temp — **not** the shared `os.tmpdir()` git/files source-clone root, which is genuinely concurrent (a blanket sweep there would delete a live sibling run's clone). Those clones are removed by their own finalizer on a clean exit and otherwise left to the OS's tmp reaping (reboot / systemd-tmpfiles).

The prune and reap share one primitive, `sweepStaleEntries(dir, isStale)` — list a cache dir's child directories and detach-remove each the predicate selects — so "iterate + guarded detached teardown" lives in exactly one place; prune passes `name !== current`, reap passes the temp-prefix test.

On the **os backend (win32/WSL)** there is a fourth, process-level leak of the same shape: a run's SIGINT/SIGTERM reaper (`buildWslReapCommand`) group-kills its bwrap tree, but a **hard** kill skips it too, leaving the WSL-side `sh`+bwrap reparented to init and pinning the store/snapshot open. `reapOrphanedWslRuns` runs once at `createWslOsBackend` startup and group-kills exactly those orphans — identified precisely, not by TTL: a live run's shell is parented by the `wsl.exe` `Relay(<pid>)`, so a shell whose parent is **not** a `Relay` has been reparented and is orphaned. A concurrent live run is never touched.
