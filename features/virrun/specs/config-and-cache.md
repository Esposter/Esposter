...
```jsonc
{
  ...
  "environment": "none", // Environment: none | nuxt
}
```
...
- **Every field is optional** in the file: `parseVirrunConfiguration` defaults `backend → auto` and `environment → none`, so a minimal `{}` — or no file at all — is valid and the options exist only to override. Unknown keys throw (a typo fails loud, not silent), and the resolved `VirrunConfiguration` object stays fully populated — optionality is the input surface, not the internal contract.
- **`environment`** selects a framework preset whose generated artifacts (Nuxt's `.nuxt`) the sandbox regenerates into a source-keyed prepare layer, so type-aware tooling reads a platform-correct, fresh copy instead of the host's (`os` backend only). `none` (default) disables it; `nuxt` runs `nuxt prepare` for the git-detected nuxt package. Preset-driven — there are no overrides — and mirrored in `schema.json` like `backend` (the `Environment` enum is the single source of truth). → [snapshot-fork](snapshot-fork.md#prepare-layer-realized--source-keyed-generated-artifacts)
