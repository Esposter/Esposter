...
The prune and reap share one primitive, `sweepStaleEntries(dir, isStale)` — list a cache dir's child directories and detach-remove each the predicate selects — so "iterate + guarded detached teardown" lives in exactly one place; prune selects a superseded dir with no live lease, reap selects a dead-owner temp corpse. Both build on one liveness check, `isProcessAlive(pid)` (`process.kill(pid, 0)`).
