...
- **A "cold install" bench must run over a source with no `node_modules`.** The snapshot install overlays on the source tree (the overlay lower _is_ the source), so a cold task pointed at the live repo installs into an already-populated `node_modules` → pnpm no-ops and "cold" silently measures the warm path (it can even clock in _faster_ than the warm tasks by noise). The cache-layer bench (`localMonorepoCache.platform.bench.ts`) fixes this by cloning HEAD into a gitignored-free checkout (`createCleanRepositoryCheckout`) as its source. It is deliberately **store-warm cold**: the deps-snapshot seed also warms the checkout's pnpm store, so cold installs offline and measures the `node_modules`-materialise cost rather than a flaky network download.
...
- Track results over time (regressions are bugs). The committed results file (below) is the offline diff gate; 🏎️ Bench runs plain `vitest bench` shards every push as an executes-clean smoke signal. A hard wall-clock CI fail is rejected as runner-noise-flaky, and CodSpeed (the former hardware-independent simulation signal with PR regression comments + flamegraphs) was removed after exceeding its 600 min/month free tier — the `CODSPEED_ENV`-gated plugin wiring remains for easy re-enable → [../out-of-scope/ci-walltime-gate.md](../out-of-scope/ci-walltime-gate.md).
