...
- Evicted by lockfile hash, not LRU/RAM budget: only the current lockfile's hash is reused, so the next run's `ensureSnapshot` sweeps every superseded `snapshots/<hash>` (`pruneStaleSnapshots`), sparing any a concurrent run still leases (`hasLiveLease`).
- CI no longer consumes the snapshot: the platform-branched `virrun.config.ts` resolves `native` on the Linux runners (the sandbox fixes a win32-only problem), so verify jobs run a plain `pnpm i` from the pnpm store cache and the former warm-capture/`actions/cache` pipeline was removed. The snapshot remains the win32/local os-backend warm path (`virrun warm` provisions it ahead of time); the copy-mode upper is self-contained, so a fork never reads the repo-local `.virrun/store` (recreated empty if absent). → [virrun CI](https://github.com/Esposter/Esposter/blob/main/packages/virrun/readme/ci.md) · [config-and-cache](config-and-cache.md#virrun-cache--gitignored)
