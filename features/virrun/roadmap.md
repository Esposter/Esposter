...
- [ ] Atomic snapshot publish (hardening, non-blocking) — `createSnapshot` `mkdirSync`s `upperDir` before the in-place capture, while `exists` is just `existsSync(upperDir)`, so a concurrent fork can observe a half-built upper mid-install. Capture into a temp dir, then `rename` onto `<hash>/upper` so readers only ever see a finished layer (optional per-hash ready-marker/lock) → [specs/snapshot-fork.md](specs/snapshot-fork.md) → Concurrency safety.
