...
- [x] **Aggregation columns** — dataset-level aggregates (`PercentOfTotal`, `Rank`, `RunningSum`); separate `AggregationColumn` class and `computeAggregationValue(rows, column, rowIndex)` evaluator; third branch in `resolveValue`
...
- [x] **Format strings** — `format?` on each column class (typed via `ColumnFormatMap`); `formatValue(value, format)` pure display function; `BooleanColumn`, `NumberColumn`, `ComputedColumn`, `AggregationColumn` now carry typed format fields
