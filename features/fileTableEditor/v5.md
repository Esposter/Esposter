...
- [x] **Row description** ⚡ — add `description` to the `Row` model via `Description` (schema + class field). Show a small note icon in `ActionSlot.vue` (filled when description exists, outlined when empty); clicking opens a single-field popover to view/edit. Survives undo/redo via the existing command system.
