...
- [ ] **Configurable footer aggregate** ⚡ — add a `footerStatistic?: FooterStatisticType` field to `NumberColumn` (enum: `Summation | Average | Min | Max | Count | None`, default `Summation`). `FooterSlot.vue` looks up the chosen statistic via `ColumnStatisticsDefinitionMap` instead of hardcoding Σ. The per-column choice is set in the column edit form via a select.
