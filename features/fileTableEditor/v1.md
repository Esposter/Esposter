...
- [ ] **Import preview** — after parsing a file, show a read-only preview of the first few rows before confirming the import; prevents having to undo a wrong-file import; preview lives in local component state inside `ImportDialog`, no store changes needed
