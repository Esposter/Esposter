...
const getSwatch = ({ deepFill, stroke }: PackageRoleColors, isDashed = false): string =>
  `<TD BGCOLOR="${deepFill}" BORDER="1" COLOR="${stroke}"${isDashed ? ' STYLE="DASHED"' : ""} WIDTH="24" HEIGHT="15"></TD>`;
...
export const getLegendLabel = (): string =>
  [
    ...
    getRow(
      getSwatch(PackageRoleColorsMap[PackageRole.Library], true),
      "dashed outline &#183; private, never published",
    ),
