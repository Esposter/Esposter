import { REVIEW_FILE_CAP, WINDOW_FILL_TARGET } from "#src/services/coderabbit/shared/constants";
...
describe("review file cap", () => {
  ...
  test("the fill target is derived from the cap", () => {
    expect.hasAssertions();

    expect(WINDOW_FILL_TARGET).toBeLessThan(REVIEW_FILE_CAP);
    expect(WINDOW_FILL_TARGET).toBeGreaterThan(0);
  });
