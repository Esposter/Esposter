...
export const getRepairPrompt = ({ collectorSha, failedLog, mainSha, runUrl }: RepairPromptInput): string => {
  ...
  return [
    ...
    "",
    `Commit the repair as one commit carrying the trailer \`${trailer}\` exactly, added with \`git commit --trailer "${trailer}"\`. The collector runs the whole check suite on it and pushes it straight to \`${MAIN_BRANCH}\` unread, so the body says what each red was and what answered it — that body is the only review the repair gets.`,
    "",
