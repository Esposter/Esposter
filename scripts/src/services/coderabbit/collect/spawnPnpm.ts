...
// Every `pnpm` the collector waits on — the checks and the installs — is spawned the same way. The exit status is the caller's to read, because what a non-zero one means differs
