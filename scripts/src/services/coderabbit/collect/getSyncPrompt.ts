...
// Sequence run to its end over a clean tree that carries every commit the queue owed (`syncQueue`), a repair to
// Any other queue commit is the session's and the queue's own CI names it, and every minute the session spends
// Is a minute the working session can push into and cost the rewrite its lease.
