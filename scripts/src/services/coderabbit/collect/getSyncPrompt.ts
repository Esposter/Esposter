...
// Already carries its whole change.
export const getSyncPrompt = ({ conflictedPaths, conflictSha, targetBranch }: SyncPromptInput): string =>
  [
    ...
    SESSION_DENIALS,
    "",
    `When the sequence has completed: ${FINISHING_CHECKS_INSTRUCTION} Leave the working tree clean with no cherry-pick in progress.`,
  ].join("\n");
