// The frontier is the last range-naming sha across every body, read from one array: `gh` re-runs a `--jq`
// Filter per page, so a `last` computed there names one page's frontier
