...
import {
  FIELD_SEPARATOR,
  RECORD_SEPARATOR,
  TRAILER_VALUE_SEPARATOR,
} from "#src/services/sweeps/ledgerCoverage/constants";

const TRAILER_REGEX = new RegExp(
  String.raw`^(?<type>${Object.values(LedgerEventType).join("|")}):(?<value>.*)$`,
  "gmu",
);
  ...
  log
    ...
    .flatMap((record) => {
      ...
      return Array.from(body.matchAll(TRAILER_REGEX), ({ groups }) => {
        const type = groups?.type as LedgerEventType;
        const [ledger = "", unit] = (groups?.value ?? "").trim().split(TRAILER_VALUE_SEPARATOR);
        return { date, ledger, type, unit: unit?.trim() };
      }).filter(({ ledger, type, unit }) => ledger !== "" && (type === LedgerEventType.Reopens || unit !== undefined));
    });
