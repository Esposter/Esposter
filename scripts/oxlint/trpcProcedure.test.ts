...
// Both rules only exist as an oxlint JS plugin and @oxlint/plugins ships no RuleTester, so the suite drives the
...
describe("trpc-procedure", () => {
  ...
  beforeAll(() => {
    directory = mkdtempSync(join(tmpdir(), "trpc-procedure-"));
    writeFileSync(
      join(directory, ".oxlintrc.json"),
      JSON.stringify({
        categories: {},
        jsPlugins: [join(import.meta.dirname, "trpcProcedure.ts").replaceAll("\\", "/")],
        plugins: [],
        rules: { [ERROR_RULE]: "error", [RETURN_TYPE_RULE]: "error" },
      }),
    );
  ...
  test("reports nothing but these two rules", () => {
    ...
    expect([...new Set(codes)].toSorted()).toStrictEqual([
