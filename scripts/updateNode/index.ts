...
import { getRegistryLatestVersionForMajor } from "@/services/getRegistryLatestVersionForMajor";
...
// 1. Resolve target node version (strip a leading `v`/`^`); default to the latest stable from npm.
const requested = process.argv[2]?.replace(/^[v^]/u, "");
const version = requested ?? (await getLatestVersion("node"));
...
const typesVersion = await getRegistryLatestVersionForMajor("@types/node", major);
