...
// `apps`, `packages` and `scripts` are the only ignores this file owns: every member lints itself with its own flat config, and a
// Root `eslint .` that walked them would apply this file's rules instead of theirs. Everything else a repo-wide walk has
// To skip — the `.claude` alias, `.agents/worktrees` — arrives from `oxlint.config.ts`'s `ignorePatterns`, which the
...
export default eslintConfiguration.append({ ignores: ["apps", "packages", "scripts"] });
